...
import type {
  EndoscopyExamTypeNames,
  EndoscopyItemDetail,
  EndoscopyItemPayload,
} from "../api/masterClient";
import {
  useEndoscopyDatasetOptions,
  useEndoscopyExamTypeOptions,
  useEndoscopyItem,
  useEndoscopyItemMutations,
  useEndoscopyItemSearch,
  useEndoscopySetItemMutations,
...
import { EndoscopyItemSearchModal } from "../components/EndoscopyItemSearchModal";
...
interface Draft {
  ...
  name_kana: string;
  kind: string;
  // 他のオーダー項目と同じオーダーにまとめられるか。false は単独オーダー。
  groupable: boolean;
  // 検査種別(上部消化管内視鏡・下部消化管内視鏡 など)。
...
function toPayload(draft: Draft): EndoscopyItemPayload {
  return {
...
export function EndoscopyItemPage() {
          ...
          {list.data?.items.map((item) => (
            <tr key={item.id} onClick={() => setEditing(item.id)} className="master-search__row">
              ...
              <td className="rad-item__compact">
                ...
                {/* 単独オーダーは 1 オーダー 1 項目になる。既定(グループ化)は
...
function ItemEditModal({ itemId, onClose }: ItemEditModalProps) {
  const detail = useEndoscopyItem(itemId);
  const examTypes = useEndoscopyExamTypeOptions();
  const mutations = useEndoscopyItemMutations();
  ...
  return (
    <Modal
      title={itemId === null ? "内視鏡オーダー項目を追加" : "内視鏡オーダー項目を編集"}
      onClose={onClose}
    ...
    >
      <form onSubmit={handleSubmit}>
        <div className="lab-order-item__fields">
          ...
          {/* 内視鏡など 1 検査に時間を要する項目は、検査室の枠を 1 件ずつ押さえる
          ...
          <label>
            ...
            >
              ...
              <option value="false">単独(1オーダー1項目)</option>
            </select>
          </label>
          <label>
            予約
          ...
          {/* 予約を取る先の枠表。大腸内視鏡の項目なら内視鏡室の検査予約枠、のように
        ...
        <div className="lab-order-item__fields">
          ...
          {/* 内視鏡には頻用コード表のような初期データ源が無く、項目は 1 件ずつ
...
// テンプレート。オーダー項目ごとに決めておくもので、オーダー時に別のものへ変えられる。
...
// オーダーに載っているオーダー項目のデータセットの明細をまとめて初期表示する。
function DatasetSelect({
  ...
  const datasets = useEndoscopyDatasetOptions();
