...
import type {
  ...
  MedicalMaterial,
  MedicalProcedure,
  EndoscopyDatasetDetail,
  EndoscopyDatasetDetailType,
  EndoscopyDatasetPayload,
} from "../api/masterClient";
import {
  useEndoscopyDataset,
  useEndoscopyDatasetDetailMutations,
  useEndoscopyDatasetMutations,
  useEndoscopyDatasetSearch,
...
import { ENDOSCOPY_ROUTE_OPTIONS } from "../fhir/endoscopyResultHelpers";

// 内視鏡の実施入力用データセット。
//
// 実施入力で毎回登録することになる手技料・薬剤・器材の組み合わせに名前を付けて
// おき、オーダー項目マスタ(内視鏡オーダー項目)に紐付ける。実施入力モーダルは、
// オーダーに載っているオーダー項目に紐付く全データセットの明細をマージして初期表示する。
//
...
function toPayload(draft: Draft): EndoscopyDatasetPayload {
  return {
...
export function EndoscopyDatasetPage() {
  ...
  const list = useEndoscopyDatasetSearch(filters, page);
...
function DatasetEditModal({ datasetId, onClose }: DatasetEditModalProps) {
  const detail = useEndoscopyDataset(datasetId);
  const mutations = useEndoscopyDatasetMutations();
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`${detail.data.name} を削除しますか？（明細とオーダー項目への紐付けも削除されます）`))
      return;
...
const SECTIONS: {
  type: EndoscopyDatasetDetailType;
  title: string;
  addLabel: string;
  /** 数量欄を出すか(手技料は数量を持たない)。 */
  quantity: boolean;
...
function DatasetDetailsEditor({
  ...
  const mutations = useEndoscopyDatasetDetailMutations();
  const [adding, setAdding] = useState<EndoscopyDatasetDetailType | null>(null);

  function add(payload: { detail_type: EndoscopyDatasetDetailType; code: string; default_quantity?: string }) {
    setAdding(null);
  ...
  return (
    <>
      ...
      {/* 内視鏡で使う薬剤は造影剤に限らない(鎮静剤・前処置など)ので、
