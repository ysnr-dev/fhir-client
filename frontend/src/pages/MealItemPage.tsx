...
// 食事オーダー項目マスタ(主食・副食形態)。処置の TreatmentItemPage から、セット構成・
// 実施入力データセット・レセ電算コード・予約枠を全て落とした最小の作り。
// 主食(staple)・副食形態(side_dish_form)を kind で切り替えて 1 画面で管理する。
// 食種は性質(種別・食止め・主成分量)を持つので別画面(MealDietPage)。
        ...
        <tbody>
          {list.data?.items.map((item) => (
            <tr key={item.id} onClick={() => setEditing(item.id)} className="master-search__row">
              ...
              <td className="rad-item__compact">{MEAL_ITEM_KIND_LABELS[item.kind] ?? item.kind}</td>
