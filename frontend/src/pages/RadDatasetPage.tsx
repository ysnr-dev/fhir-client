...
import {
  useRadDataset,
  useRadDatasetDetailMutations,
  useRadDatasetMutations,
  useRadDatasetSearch,
...
function toPayload(draft: Draft): RadDatasetPayload {
  return {
...
export function RadDatasetPage() {
  ...
  const list = useRadDatasetSearch(filters, page);
...
function DatasetEditModal({ datasetId, onClose }: DatasetEditModalProps) {
  const detail = useRadDataset(datasetId);
  const mutations = useRadDatasetMutations();
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`${detail.data.name} を削除しますか？（明細と撮影項目への紐付けも削除されます）`))
      return;
...
function DatasetDetailsEditor({
  ...
  const mutations = useRadDatasetDetailMutations();
  const [adding, setAdding] = useState<RadDatasetDetailType | null>(null);

  function add(payload: { detail_type: RadDatasetDetailType; code: string; default_quantity?: string }) {
    setAdding(null);
...
function DetailRow({
  ...
  return (
    <tr>
      ...
      <td>
        {detail.code}
        {/* 器材は施設内コードで選ぶが、算定はレセ電算の特定器材コードで行うので併記する。 */}
        {detail.detail_type === "material" && detail.receipt_material_code && (
          <span className="lab-order-item__code">（算定 {detail.receipt_material_code}）</span>
        )}
      </td>
      {section.quantity && (
        <td className="rad-item__compact">
