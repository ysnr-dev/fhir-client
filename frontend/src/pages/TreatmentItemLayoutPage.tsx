...
import {
  ...
  useTreatmentItemLayoutCellMutations,
  useTreatmentItemLayoutMutations,
  useTreatmentItemLayouts,
  useTreatmentItemSearch,
} from "../api/masterQueries";
import type { TreatmentItem, TreatmentItemLayoutCell } from "../api/masterClient";
...
export function TreatmentItemLayoutPage() {
  const layouts = useTreatmentItemLayouts();
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [creating, setCreating] = useState(false);
  const layoutMutations = useTreatmentItemLayoutMutations();
...
function LayoutEditor({ layoutId, onDeleted }: { layoutId: number; onDeleted: () => void }) {
  const { data, error } = useTreatmentItemLayout(layoutId);
  const layoutMutations = useTreatmentItemLayoutMutations();
  const cellMutations = useTreatmentItemLayoutCellMutations();
  ...
  const cellsByPosition = useMemo(() => {
    const map = new Map<string, TreatmentItemLayoutCell>();
...
interface CellEditorProps {
  layoutId: number;
  position: Position;
  cell: TreatmentItemLayoutCell | null;
  mutations: ReturnType<typeof useTreatmentItemLayoutCellMutations>;
  onClose: () => void;
}
...
function CellEditor({ layoutId, position, cell, mutations, onClose }: CellEditorProps) {
  ...
  // 目当ての項目が分かっているときはその場で置けるよう、打った語を名称・略称・カナの
  // どちらにも当てる。一覧を見ながら探すときは「項目を選択」の検索モーダルを使う。
  const [query, setQuery] = useState("");
  const hasQuery = query.trim().length > 0;
  const candidates = useTreatmentItemSearch({ keyword: query }, 1, hasQuery);
  ...
  async function handlePlaceItem(item: TreatmentItem) {
    setSearching(false);
    setQuery("");
