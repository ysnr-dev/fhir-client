...
import {
  usePhysioDatasetOptions,
  usePhysioExamTypeOptions,
  usePhysioItem,
  usePhysioItemMutations,
  usePhysioItemSearch,
  usePhysioSetItemMutations,
  type PhysioItemFilters,
...
import { PhysioItemSearchModal } from "../components/PhysioItemSearchModal";
import { KIND_LABELS, renderExamTypeOptions } from "../components/physioItemOptions";
import { TemplateSelect } from "../components/TemplateSelect";
import { questionnaireCanonical } from "../fhir/questionnaireResponseHelpers";

// 生理検査オーダー項目マスタ。放射線の RadItemPage と同じ作りだが、JJ1017 の
// 11 要素・32桁コードのプレビュー・頻用コードからの一括作成は持たない。
// 分類軸は検査種別(master_physio_exam_types)ひとつ。
...
function toPayload(draft: Draft): PhysioItemPayload {
  return {
    ...
    exam_type_code: draft.exam_type_code || null,
...
export function PhysioItemPage() {
  const [inputs, setInputs] = useState<PhysioItemFilters>({});
  const [filters, setFilters] = useState<PhysioItemFilters>({});
  const [page, setPage] = useState(1);
  // 編集対象の id。"new" は新規作成モーダル。
  const [editing, setEditing] = useState<number | "new" | null>(null);

  const list = usePhysioItemSearch(filters, page);
  const examTypes = usePhysioExamTypeOptions();
          ...
          {list.data?.items.map((item) => (
            <tr key={item.id} onClick={() => setEditing(item.id)} className="master-search__row">
              ...
              <td className="rad-item__compact">
                ...
                {/* 単独オーダーは 1 オーダー 1 検査項目になる。既定(グループ化)は
...
function ItemEditModal({ itemId, onClose }: ItemEditModalProps) {
  const detail = usePhysioItem(itemId);
  const examTypes = usePhysioExamTypeOptions();
  const mutations = usePhysioItemMutations();
  ...
  useEffect(() => {
    ...
    setDraft({
      ...
      exam_type_code: d.exam_type_code ?? "",
  ...
  return (
    <Modal
      title={itemId === null ? "生理検査オーダー項目を追加" : "生理検査オーダー項目を編集"}
      onClose={onClose}
    ...
    >
      <form onSubmit={handleSubmit}>
        <div className="lab-order-item__fields">
          ...
          {/* セットは検査そのものではないが、まとめの分類として種別を持たせてよい
              (オーダー画面の見出しはセット自身に種別が無ければ構成項目から採る)。 */}
          <label>
            検査種別
            <select
          ...
          {/* 心エコーなど 1 検査に時間を要する項目は、検査室の枠を 1 件ずつ押さえる
          ...
          <label>
            ...
            >
              ...
              <option value="false">単独(1オーダー1検査項目)</option>
            </select>
          </label>
          <label>
            予約
          ...
          {/* 予約を取る先の枠表。心エコーの項目なら生理検査室の検査予約枠、のように
              紐づけておくと、オーダー画面の予約モーダルでその枠表が最初から選ばれる。 */}
          <label>
            予約枠
            <select
        ...
        <div className="lab-order-item__fields">
          ...
          {/* 生理検査には頻用コード表のような初期データ源が無く、項目は 1 件ずつ
        ...
        {isSet && (
          <p className="rad-code__summary">
            セットは検査そのものではありません。構成する単項目を下で登録してください。
          </p>
        )}

        <ErrorBanner error={detail.error ?? examTypes.error} />
        <ErrorBanner
      ...
      {itemId !== null && isSet && detail.data && (
        <SetItemsEditor
          ...
          examTypes={detail.data.exam_types}
        />
      )}
...
// テンプレート。検査項目ごとに決めておくもので、オーダー時に別のものへ変えられる。
...
// オーダーに載っている検査項目のデータセットの明細をまとめて初期表示する。
function DatasetSelect({
  ...
  const datasets = usePhysioDatasetOptions();
...
interface SetItemsEditorProps {
  setItemCode: string;
  setItems: PhysioItemDetail["set_items"];
  /** 構成項目の検査種別の名称。詳細APIが構成項目の分も添えて返す。 */
  examTypes: PhysioExamTypeNames;
}

function SetItemsEditor({ setItemCode, setItems, examTypes }: SetItemsEditorProps) {
          ...
          <tbody>
            {setItems.map((member) => (
              <tr key={member.id}>
                ...
                <td>
                  {member.member_exam_type_code
                    ? (examTypes[member.member_exam_type_code] ?? member.member_exam_type_code)
                    : ""}
                </td>
