...
import {
  ...
  useEndoscopyItemLayoutCellMutations,
  useEndoscopyItemLayoutMutations,
  useEndoscopyItemLayouts,
  useEndoscopyItemSearch,
} from "../api/masterQueries";
import type { EndoscopyItem, EndoscopyItemLayoutCell } from "../api/masterClient";
...
export function EndoscopyItemLayoutPage() {
  const layouts = useEndoscopyItemLayouts();
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [creating, setCreating] = useState(false);
  const layoutMutations = useEndoscopyItemLayoutMutations();
...
function LayoutEditor({ layoutId, onDeleted }: { layoutId: number; onDeleted: () => void }) {
  const { data, error } = useEndoscopyItemLayout(layoutId);
  const layoutMutations = useEndoscopyItemLayoutMutations();
  const cellMutations = useEndoscopyItemLayoutCellMutations();
  ...
  const cellsByPosition = useMemo(() => {
    const map = new Map<string, EndoscopyItemLayoutCell>();
...
interface CellEditorProps {
  layoutId: number;
  position: Position;
  cell: EndoscopyItemLayoutCell | null;
  mutations: ReturnType<typeof useEndoscopyItemLayoutCellMutations>;
  onClose: () => void;
}
...
function CellEditor({ layoutId, position, cell, mutations, onClose }: CellEditorProps) {
  ...
  // 目当ての項目が分かっているときはその場で置けるよう、打った語を名称・検査種別の
  // どちらにも当てる。一覧を見ながら探すときは「項目を選択」の検索モーダルを使う。
  const [query, setQuery] = useState("");
  const hasQuery = query.trim().length > 0;
  const candidates = useEndoscopyItemSearch({ keyword: query }, 1, hasQuery);
  ...
  async function handlePlaceItem(item: EndoscopyItem) {
    setSearching(false);
    setQuery("");
