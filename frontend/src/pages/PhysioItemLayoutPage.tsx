...
import {
  usePhysioItemLayout,
  usePhysioItemLayoutCellMutations,
  usePhysioItemLayoutMutations,
  usePhysioItemLayouts,
  usePhysioItemSearch,
} from "../api/masterQueries";
import type { PhysioItem, PhysioItemLayoutCell } from "../api/masterClient";
...
export function PhysioItemLayoutPage() {
  const layouts = usePhysioItemLayouts();
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [creating, setCreating] = useState(false);
  const layoutMutations = usePhysioItemLayoutMutations();
...
function LayoutEditor({ layoutId, onDeleted }: { layoutId: number; onDeleted: () => void }) {
  const { data, error } = usePhysioItemLayout(layoutId);
  const layoutMutations = usePhysioItemLayoutMutations();
  const cellMutations = usePhysioItemLayoutCellMutations();
  ...
  const cellsByPosition = useMemo(() => {
    const map = new Map<string, PhysioItemLayoutCell>();
...
interface CellEditorProps {
  layoutId: number;
  position: Position;
  cell: PhysioItemLayoutCell | null;
  mutations: ReturnType<typeof usePhysioItemLayoutCellMutations>;
  onClose: () => void;
}
...
function CellEditor({ layoutId, position, cell, mutations, onClose }: CellEditorProps) {
  ...
  const candidates = usePhysioItemSearch({ keyword: query }, 1, hasQuery);
  ...
  async function handlePlaceItem(item: PhysioItem) {
    setSearching(false);
    setQuery("");
