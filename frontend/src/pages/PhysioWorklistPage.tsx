...
export function PhysioWorklistPage() {
  ...
  // 「実施」で実施入力を開くかどうかは検査項目マスタが決めるので、この日の
  // オーダーに載っている項目をまとめて引いておく。
  const itemCodes = useMemo(
    () =>
      (worklist.data?.rows ?? []).flatMap((row) =>
        physioOrderItems(row.order, row.itemRequests).map((item) => item.code),
      ),
    [worklist.data],
  );
  const items = usePhysioItemsByCodes(itemCodes);
  ...
  function needsPerformInput(row: PhysioWorklistRow): boolean {
    const codes = physioOrderItems(row.order, row.itemRequests)
      .map((item) => item.code)
            ...
            <table className="rad-worklist sticky-table">
              <thead>
                <tr>
                  ...
                  <PatientProfileHeadCells />
                  <th className="rad-worklist__content">検査内容</th>
...
function rowExamTypeCodes(row: PhysioWorklistRow): string[] {
  return physioOrderItems(row.order, row.itemRequests)
    .map((item) => item.examTypeCode)
    .filter(Boolean);
}

function matchesFilters(row: PhysioWorklistRow, filters: Filters): boolean {
  if (filters.examTypeCode && !rowExamTypeCodes(row).includes(filters.examTypeCode)) return false;

  const summary = summarizePhysioOrder(row.order);
  ...
  return (
    <form className="patient-search-form" onSubmit={handleSubmit}>
      ...
      <label>
        ...
        >
          <option value="">すべて</option>
          {PHYSIO_TASK_STATUS_OPTIONS.map((option) => (
            <option key={option.code} value={option.code}>
              {option.display}
            </option>
          ))}
    ...
    <tr className={summary.urgent ? "rad-worklist__row--urgent" : undefined}>
      ...
      <td className="rad-worklist__content">
        <ul className="rad-worklist__items">
          ...
          {entries.length === 0 && <li className="order-select__muted">検査項目なし</li>}
        </ul>
      </td>
