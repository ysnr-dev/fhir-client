...
import type {
  ...
  PhysioDatasetPayload,
} from "../api/masterClient";
import {
  usePhysioDataset,
  usePhysioDatasetDetailMutations,
  usePhysioDatasetMutations,
  usePhysioDatasetSearch,
...
import { PHYSIO_ROUTE_OPTIONS } from "../fhir/physioResultHelpers";

// 生理検査の実施入力用データセット。
//
// 実施入力で毎回登録することになる手技料・薬剤・器材の組み合わせに名前を付けて
// おき、検査項目マスタ(生理検査オーダー項目)に紐付ける。実施入力モーダルは、
// オーダーに載っている検査項目に紐付く全データセットの明細をマージして初期表示する。
//
...
function toPayload(draft: Draft): PhysioDatasetPayload {
  return {
...
export function PhysioDatasetPage() {
  ...
  const list = usePhysioDatasetSearch(filters, page);
...
function DatasetEditModal({ datasetId, onClose }: DatasetEditModalProps) {
  const detail = usePhysioDataset(datasetId);
  const mutations = usePhysioDatasetMutations();
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`${detail.data.name} を削除しますか？（明細と検査項目への紐付けも削除されます）`))
      return;
...
function DatasetDetailsEditor({
  ...
  const mutations = usePhysioDatasetDetailMutations();
  const [adding, setAdding] = useState<PhysioDatasetDetailType | null>(null);

  function add(payload: { detail_type: PhysioDatasetDetailType; code: string; default_quantity?: string }) {
    setAdding(null);
  ...
  return (
    <>
      ...
      {/* 生理検査で使う薬剤は造影剤に限らない(負荷心電図の薬剤負荷など)ので、
