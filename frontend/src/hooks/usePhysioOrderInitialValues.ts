...
import { usePhysioOrderDetail } from "../api/queries";
import { serviceRequestsOf } from "../fhir/labOrderHelpers";
import { isPatientMismatch } from "../fhir/patientHelpers";
import {
  parsePhysioOrderForm,
  physioOrderItemRequests,
  physioOrderResponseIds,
} from "../fhir/physioOrderHelpers";

// 保存済みの生理検査オーダーをフォームの初期値に復元する。編集と DO の双方から使う。
// 明細も ServiceRequest なので、ヘッダと一緒に取得した明細から項目を組み立てる。
export function usePhysioOrderInitialValues(srId: string | undefined, patientId?: string) {
  ...
  const { serviceRequest, itemRequests } = useMemo(() => {
    ...
    return {
      ...
      itemRequests: srId ? physioOrderItemRequests(requests, srId) : [],
    };
  }, [detail.data, srId]);

  const initialValues = useMemo(
    () => (serviceRequest ? parsePhysioOrderForm(serviceRequest, itemRequests) : undefined),
  ...
  return {
    ...
    responseIds: physioOrderResponseIds(itemRequests),
    initialValues: patientMismatch ? undefined : initialValues,
    ready: !detail.isLoading,
    patientMismatch,
    error:
      detail.error ??
      (patientMismatch ? new Error("指定された生理検査は別の患者のものです。") : undefined),
  };
}
