...
// 内視鏡の進捗(受付・実施)。オーダーそのものは ServiceRequest のままにして、
// 「その依頼が部門でどこまで進んだか」を Task で別に持つ。
//
//   ServiceRequest(オーダー) ← focus ── Task(進捗)
//
// オーダーを直に書き換えないのは、依頼した内容(誰が何をいつ検査すると決めたか)と、
...
export const ENDOSCOPY_TASK_CODE = { code: "endoscopy-exam", display: "内視鏡" };

/**
 * 内視鏡の進捗。
 *
 * requested … 依頼済(部門はまだ受け取っていない)
 * accepted  … 受付済(患者が内視鏡室に来て、検査待ち)
 * completed … 実施済
 * cancelled … 中止
 */
export type EndoscopyTaskStatus = "requested" | "accepted" | "completed" | "cancelled";

export const ENDOSCOPY_TASK_STATUS_OPTIONS: { code: EndoscopyTaskStatus; display: string }[] = [
...
const helpers = createTaskHelpers<EndoscopyTaskStatus>({
  taskCode: ENDOSCOPY_TASK_CODE,
  statusOptions: ENDOSCOPY_TASK_STATUS_OPTIONS,
});

export const endoscopyTaskStatusDisplay = helpers.statusDisplay;

/** 一覧の行から押せる操作。進める操作と戻す操作を同じ形で並べる。 */
export interface EndoscopyTaskAction {
  label: string;
  next: EndoscopyTaskStatus;
  /**
 ...
 */
export function endoscopyTaskActions(status: EndoscopyTaskStatus): EndoscopyTaskAction[] {
  switch (status) {
    case "requested":
      return [
...
export const endoscopyTasksByOrderId = helpers.tasksByOrderId;

/** ステータスを変えた Task。まだ無ければ作る(組み立ては taskHelpers を参照)。 */
export const buildEndoscopyTaskUpdate = helpers.buildTaskUpdate;
