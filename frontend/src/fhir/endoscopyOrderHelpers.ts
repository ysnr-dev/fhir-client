...
// 内視鏡オーダー。放射線検査と同じくオーダーヘッダは ServiceRequest で、
// 明細(検査 1 件)も 1 件ずつ独立した ServiceRequest にする。
//
//   ヘッダ ← basedOn ── 明細(単項目・セット) ← basedOn ── セットの構成項目
//
// 明細の各 ServiceRequest は、オーダーした時点の内視鏡オーダー項目マスタの内容
// (項目コード・名称・略称・検査種別)を写して持つ。マスタを直した後に過去の
// オーダーの中身が変わってしまわないよう、参照ではなく写し。
//
// 構造は生理検査(physioOrderHelpers)と同型:
// - JJ1017 を持たない。内視鏡は JJ1017 に収載されていない。
// - bodySite を持たない。「上部消化管内視鏡」「大腸内視鏡」のように項目名が
//   対象を含み、代わる標準の部位コード体系も無い。部位は項目名で表す。
// - モダリティの代わりに、施設が定義する検査種別(上部消化管内視鏡・下部消化管
//   内視鏡 など)を明細の category に出す。部門一覧の絞り込み軸でもある。
//
// 検査目的・特別指示の JED(Japan Endoscopy Database)用語は、専用のマスタや
// coding ではなく Questionnaire テンプレートの選択肢(統一表記の転記)で入れる。
// JED 用語集にはコード体系が無く統一表記が本体なので、選択値が
// QuestionnaireResponse に残れば将来の JED 出力にも足りる。
//
// オーダーの単位(GP)は「単独で選んだオーダー項目 1 つ」または「セット 1 つ」。
...
const PURPOSE_QR_EXT_URL =
  "http://fhir-client.local/StructureDefinition/endoscopy-exam-purpose-questionnaire-response";
const REMARKS_QR_EXT_URL =
  "http://fhir-client.local/StructureDefinition/endoscopy-remarks-questionnaire-response";
...
export interface EndoscopyOrderItemLine {
  ...
  shortName: string;
  /** 検査種別(上部消化管内視鏡・下部消化管内視鏡 など)。オーダー内容をまとめて見せる軸。 */
  examTypeCode: string;
  examTypeName: string;
  /**
  ...
  parentCode: string;
  /**
   * 他のオーダー項目と同じオーダーにまとめられるか(マスタの写しではなく、登録時に
   ...
   * EndoscopyOrderFormValues の同名の項目が担う。groupable と同じく登録時の分割にだけ
...
export interface EndoscopyOrderFormValues {
...
export function emptyEndoscopyOrderForm(
...
export interface EndoscopyOrderEntry {
  item: EndoscopyOrderItemLine;
  members: EndoscopyOrderItemLine[];
}

/** 単独で選んだ項目(セットを含む)。構成項目は除く。 */
export function topLevelItems(items: EndoscopyOrderItemLine[]): EndoscopyOrderItemLine[] {
  return items.filter((item) => !item.parentCode);
}

/** 指定したセットの構成項目。 */
export function membersOf(items: EndoscopyOrderItemLine[], setCode: string): EndoscopyOrderItemLine[] {
  return items.filter((item) => item.parentCode === setCode);
}

/** オーダーを GP(単独項目・セット)の並びにする。選んだ順のまま。 */
export function orderEntries(items: EndoscopyOrderItemLine[]): EndoscopyOrderEntry[] {
  return topLevelItems(items).map((item) => ({ item, members: membersOf(items, item.code) }));
}

/** GP の検査種別。セットは自身に種別を持たないことがあるので構成項目から補う。 */
export function entryExamTypeName(entry: EndoscopyOrderEntry): string {
  return entry.item.examTypeName || entry.members[0]?.examTypeName || "";
}

/** GP の見出し(「上部消化管内視鏡 | 上部消化管内視鏡(経口)」)。区切りはカードの見出し・
 *  メタ行(「外来 | 内科 | 児玉 義憲」)と同じ縦棒に揃える。 */
export function entryLabel(entry: EndoscopyOrderEntry): string {
  return [entryExamTypeName(entry), entry.item.name].filter(Boolean).join(" | ");
}

// ---- FHIR リソースの組み立て ----

/** ServiceRequest が内視鏡オーダーかどうか。処方・注射・検体検査との振り分けに使う。 */
export function isEndoscopyServiceRequest(sr: fhir4.ServiceRequest): boolean {
  return (sr.category ?? []).some((category) =>
    category.coding?.some((c) => c.system === ORDER_TYPE_SYSTEM && c.code === ENDOSCOPY_ORDER_TYPE.code),
  );
}
...
function buildItemEntries(
  ...
  const pushEntry = (item: EndoscopyOrderItemLine, parentReference: string): string => {
    sequence += 1;
...
function buildEndoscopyOrderServiceRequest(
  ...
  const resource: fhir4.ServiceRequest = {
    ...
    category: [
      { coding: [{ system: ORDER_TYPE_SYSTEM, ...ENDOSCOPY_ORDER_TYPE }] },
      ...(values.setting
        ? [
            {
              coding: [
...
export interface EndoscopyOrderEntries {
...
export function buildEndoscopyOrderEntries(
  values: EndoscopyOrderFormValues,
  patientId: string,
  requester: OrderContext,
  serviceRequestId?: string,
...
/**
 * 選んだオーダー項目を、登録する 1 オーダーぶんずつに分ける。
 *
 * CT・MRI などは 1 検査に時間を要し、検査室の枠を 1 件ずつ押さえる必要があるため、
 * マスタで「単独」にした項目は 1 オーダー 1 オーダー項目にする。オーダー画面では他の
 * 項目と一度に選べるが、登録時にここで別のオーダー(別のカルテカード)へ分ける。
 *
 * 入外区分・至急区分・実施日時・対象プロブレムは伝票共通の入力なので
 * 各オーダーへ写す。並びは、まとめられる項目のオーダーを先頭に、単独の項目を選んだ順。
 */
export function splitEndoscopyOrderValues(values: EndoscopyOrderFormValues): EndoscopyOrderSplit[] {
  const groupedLines: EndoscopyOrderItemLine[] = [];
  const soloOrders: EndoscopyOrderSplit[] = [];
  ...
  // オーダー項目が 1 つも無い場合も呼び出し側の検証に任せ、空のオーダーを 1 件返す。
  return soloOrders.length > 0 ? soloOrders : [{ key: "", values: { ...values, items: [] } }];
}

/** 予約必須オーダーの予約内容。キーは splitEndoscopyOrderValues のキー(=オーダー項目コード)。 */
export interface EndoscopyOrderBooking {
 ...
 *
 * 即実施(endoscopyResultHelpers)も実施記録をここに足すので、分割 1 件ぶんの組み立ては
 * この関数に集約する。
 */
export function buildEndoscopyOrderSplitEntries(
  split: EndoscopyOrderSplit,
  patientId: string,
  requester: OrderContext,
  booking?: EndoscopyOrderBooking,
): EndoscopyOrderEntries {
  const selection = booking?.selections[split.key];
  if (!selection) return buildEndoscopyOrderEntries(split.values, patientId, requester);
...
export function buildEndoscopyOrderBundle(
  values: EndoscopyOrderFormValues,
  patientId: string,
  requester: OrderContext,
  booking?: EndoscopyOrderBooking,
  ...
  return transactionBundle(
    splitEndoscopyOrderValues(values).flatMap(
      (split) => buildEndoscopyOrderSplitEntries(split, patientId, requester, booking).entries,
    ),
  );
}
...
export function buildEndoscopyOrderDeleteBundle(
  serviceRequestId: string,
  itemIds: string[],
  responseIds: string[] = [],
  /**
...
export function buildDoEndoscopyOrderForm(
  values: EndoscopyOrderFormValues,
  setting: PrescriptionSetting,
): EndoscopyOrderFormValues {
  return {
...
export interface EndoscopyOrderSummary {
...
export function summarizeEndoscopyOrder(sr: fhir4.ServiceRequest): EndoscopyOrderSummary {
 ...
 */
export function endoscopyOrderItems(
...
export function endoscopyOrderItemRequests(
 ...
 * 日付のみ)は空。
 */
export function endoscopyOrderTime(sr: fhir4.ServiceRequest): string {
  const occurrence = sr.occurrenceDateTime ?? "";
  return occurrence.length > 10 ? occurrence.slice(11, 16) : "";
}

export const endoscopyOrderProblem = orderProblem;

// ---- 編集フォームへの復元 ----

export function parseEndoscopyOrderForm(
  sr: fhir4.ServiceRequest,
  items: fhir4.ServiceRequest[] = [],
): EndoscopyOrderFormValues {
  return {
