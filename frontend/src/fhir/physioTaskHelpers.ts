...
// 生理検査の進捗(受付・実施)。オーダーそのものは ServiceRequest のままにして、
...
export const PHYSIO_TASK_CODE = { code: "physio-exam", display: "生理検査" };

/**
 * 生理検査の進捗。
 *
 * requested … 依頼済(部門はまだ受け取っていない)
 * accepted  … 受付済(患者が生理検査室に来て、検査待ち)
 * completed … 実施済
 * cancelled … 中止
 */
export type PhysioTaskStatus = "requested" | "accepted" | "completed" | "cancelled";

export const PHYSIO_TASK_STATUS_OPTIONS: { code: PhysioTaskStatus; display: string }[] = [
...
const helpers = createTaskHelpers<PhysioTaskStatus>({
  taskCode: PHYSIO_TASK_CODE,
  statusOptions: PHYSIO_TASK_STATUS_OPTIONS,
});

export const physioTaskStatusDisplay = helpers.statusDisplay;

/** 一覧の行から押せる操作。進める操作と戻す操作を同じ形で並べる。 */
export interface PhysioTaskAction {
  label: string;
  next: PhysioTaskStatus;
  /**
 ...
 */
export function physioTaskActions(status: PhysioTaskStatus): PhysioTaskAction[] {
  switch (status) {
    case "requested":
      return [
...
export const physioTasksByOrderId = helpers.tasksByOrderId;

/** ステータスを変えた Task。まだ無ければ作る(組み立ては taskHelpers を参照)。 */
export const buildPhysioTaskUpdate = helpers.buildTaskUpdate;
