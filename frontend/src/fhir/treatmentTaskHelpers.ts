...
// 処置の進捗(受付・実施)。オーダーそのものは ServiceRequest のままにして、
// 「その依頼が部門でどこまで進んだか」を Task で別に持つ。
//
//   ServiceRequest(オーダー) ← focus ── Task(進捗)
//
// オーダーを直に書き換えないのは、依頼した内容(誰が何をいつ実施すると決めたか)と、
// 部門がそれをどう捌いたかは別の事実だから。オーダーの status を進捗に流用すると
// 依頼内容の更新と実施入力が同じリソースの取り合いになる。
//
// Task は最初のステータス変更(受付・実施・中止)で作る。オーダー登録時には作らない
// ので、一覧では「Task が無い = 依頼済(未受付)」として扱う。
//
// 進捗は Task.status(FHIR 固定の値セット)だけで表す。施設ごとのコードを載せる
// businessStatus は、今の 4 状態が標準コードにそのまま対応するので使っていない。

export const TREATMENT_TASK_CODE = { code: "treatment", display: "処置" };

/**
 * 処置の進捗。
 *
 * requested … 依頼済(部門はまだ受け取っていない)
 * accepted  … 受付済(患者が処置室に来て、実施待ち)
 * completed … 実施済
 * cancelled … 中止
 */
export type TreatmentTaskStatus = "requested" | "accepted" | "completed" | "cancelled";

export const TREATMENT_TASK_STATUS_OPTIONS: { code: TreatmentTaskStatus; display: string }[] = [
...
const helpers = createTaskHelpers<TreatmentTaskStatus>({
  taskCode: TREATMENT_TASK_CODE,
  statusOptions: TREATMENT_TASK_STATUS_OPTIONS,
});

export const treatmentTaskStatusDisplay = helpers.statusDisplay;

/** 一覧の行から押せる操作。進める操作と戻す操作を同じ形で並べる。 */
export interface TreatmentTaskAction {
  label: string;
  next: TreatmentTaskStatus;
  /**
   * 押すと実施入力を開く操作。ステータスだけを進める他の操作と違い、実施記録
   * (使った薬剤・器材・手技)を入れてから Task の完了と一緒に登録する。
   */
  opensPerformInput?: true;
  /**
   * 日常の流れではない操作(押し間違いの訂正・処置の取りやめ)。一覧では
 ...
 * 「中止」は処置そのものを取りやめる操作で、別のもの。中止からは依頼済に戻せる。
 */
export function treatmentTaskActions(status: TreatmentTaskStatus): TreatmentTaskAction[] {
  switch (status) {
    case "requested":
      return [
...
export const treatmentTasksByOrderId = helpers.tasksByOrderId;

/** ステータスを変えた Task。まだ無ければ作る(組み立ては taskHelpers を参照)。 */
export const buildTreatmentTaskUpdate = helpers.buildTaskUpdate;
