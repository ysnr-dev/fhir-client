 ...
 *
 * backend のマスタを起こさず、**部屋そのものの拡張**として持つ。並び順は部屋の属性で、
 * 部屋を消せば一緒に消えるのが自然 —— location_id で紐づく別テーブルだと孤児行の掃除が
 * 要る。施設で 1 つの並びになるのも都合がよい(端末ごとの好みではなく、手術室の並びは
 * 手術部で共有された前提)。
 *
 * 上流は独自拡張の Location を素通しし、往復もする。独自拡張で _sort はできないので、
 * 並べ替えは読み手側(sortLocations)。
 */
