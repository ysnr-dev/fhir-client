...
// 生理検査オーダーの内容表示。カルテ画面の詳細モーダルから使う
// (検体検査の LabOrderDetailPanel と同じ構成)。
// 1 GP = 検査項目 1 つ、またはセット 1 つ。

interface PhysioOrderDetailPanelProps {
  serviceRequest: fhir4.ServiceRequest;
  /** オーダーにぶら下がる明細(検査項目・セットの構成項目)。 */
  itemRequests: fhir4.ServiceRequest[];
  problemsById?: Map<string, fhir4.Condition>;
}

export function PhysioOrderDetailPanel({
  serviceRequest,
  itemRequests,
  problemsById,
}: PhysioOrderDetailPanelProps) {
  const summary = summarizePhysioOrder(serviceRequest);
  const entries = orderEntries(physioOrderItems(serviceRequest, itemRequests));

  const problem = physioOrderProblem(serviceRequest);
    ...
    <div className="prescription-detail">
      <fieldset>
        ...
        <dl className="prescription-detail__common">
          ...
          <dd>{physioOrderTime(serviceRequest) || "-"}</dd>
          <dt>入外区分</dt>
      ...
      {entries.length === 0 && <p className="patient-table__empty">検査項目がありません。</p>}
    </div>
  );
}

function GroupDetail({ entry, number }: { entry: PhysioOrderEntry; number: number }) {
