...
import { physioOrderItems, type PhysioOrderItemLine } from "../fhir/physioOrderHelpers";
import {
...
// 生理検査の実施入力。放射線検査(docs/rad-result-design.md)と同じ形。
//
// 入力欄そのものは PhysioPerformInputModal(送信先を持たない)で、使い道は 2 つ:
// - 生理検査一覧の「実施」… PhysioPerformModal が包み、実施記録(Procedure 一式)と
//   Task の完了を 1 つの transaction で登録する。
// - オーダー画面の「即実施」… 入力値だけを受け取り、オーダーの登録と同じ
//   transaction に積む(PhysioOrderForm)。
//
// 初期表示は、オーダーに載っている検査項目に紐付く実施入力データセットの明細を
// マージしたもの。薬剤も器材も無い検査が件数の大半なので、何も足さずに
// そのまま「登録」で終われるようにしている。
//
// 放射線と違い被曝線量の欄は無い(生理検査に電離放射線はない)。器材は施設内の
...
/** 生理検査一覧の「実施」。入力した内容をその場で登録する。 */
export function PhysioPerformModal({ row, onClose }: Props) {
  const register = useRegisterPhysioPerform();

  const items = useMemo(
    () => physioOrderItems(row.order, row.itemRequests),
  ...
  return (
    <PhysioPerformInputModal
      ...
      onSubmit={(values) =>
        register.mutate(buildPhysioPerformBundle(values, row.order, row.task), { onSuccess: onClose })
      }
      onClose={onClose}
    />
  );
...
interface InputProps {
  /** 検査内容。データセット由来の初期明細の取得と、実施対象の表示に使う。 */
  items: PhysioOrderItemLine[];
  /**
   * 入力済みの内容。即実施で入力し直すときに渡す(データセット由来の初期行では
   * なく、前に入れた内容から始める)。
   */
  initialValues?: PhysioPerformFormValues | null;
  submitLabel: string;
  submitting?: boolean;
  submitError?: unknown;
  onSubmit: (values: PhysioPerformFormValues) => void;
  onClose: () => void;
}

export function PhysioPerformInputModal({
  items,
  initialValues = null,
  submitLabel,
  submitting = false,
  ...
  const dataset = usePhysioDatasetLinesForItems(itemCodes);
    ...
    <Modal title="実施入力" onClose={onClose} className="modal--lab-order-item">
      <form onSubmit={handleSubmit}>
        ...
        <p className="rad-perform__items">
          <span className="rad-perform__items-label">検査内容</span>
          {items.map((item) => item.name).join("、") || "検査項目なし"}
        </p>
                  ...
                  <tr key={`${line.medicineCode}-${index}`}>
                    ...
                    <td className="rad-item__compact">
                      ...
                      >
                        ...
                        {PHYSIO_ROUTE_OPTIONS.map((option) => (
      ...
      {/* 生理検査で使う薬剤は造影剤に限らない(負荷心電図の薬剤負荷・気道可逆性
