...
import {
  endoscopyExamTypeName,
  useEndoscopyItemLayout,
  useEndoscopyItemLayouts,
  useEndoscopyItemSearch,
  useEndoscopyItemsByCodes,
  useEndoscopySetMembers,
...
import {
  ...
  topLevelItems,
  type EndoscopyOrderEntry,
  type EndoscopyOrderFormValues,
  type EndoscopyOrderItemLine,
  type EndoscopyOrderPriority,
  type EndoscopyOrderSplit,
} from "../fhir/endoscopyOrderHelpers";
import {
  endoscopyPerformSummary,
...
// SOAP と同じテンプレート(Questionnaire)から記入でき、オーダー項目マスタに既定の
// テンプレートが設定してあればそれを最初から選んだ状態で開く。
//
// 選んだ項目は、オーダー時点のマスタの内容(名称・略称・検査種別)を写して持つ。
// マスタを直しても過去のオーダーの中身が変わらないようにするため。
// 放射線検査と違い JJ1017 コードと部位・左右は持たない(内視鏡は JJ1017 に
// 収載されておらず、部位は項目名で表す)。
//
// 「即実施」を選ぶと、診察室でその場で検査する運用のために、登録と同時に実施記録を作って
// Task を実施済にする。実施入力は内視鏡一覧と同じモーダルで、登録されるオーダー
// 単位に入れる(単独オーダーの項目を混ぜて選ぶとオーダーが分かれるため)。

interface EndoscopyOrderFormProps {
  patientId: string;
  initialValues?: EndoscopyOrderFormValues;
  /**
  ...
  onSubmit: (
    ...
    /**
     * 予約必須オーダーの予約(キーはオーダー項目コード)。至急のオーダーでは取らない。
...
export function EndoscopyOrderForm({
  ...
  const layouts = useEndoscopyItemLayouts();
  ...
  // 伝票・検索結果・選択済みをまとめて引く。
  const layout = useEndoscopyItemLayout(active?.kind === "layout" ? active.id : undefined);
  ...
  const buildLine = useCallback(
    (item: EndoscopyItem, parentCode: string): EndoscopyOrderItemLine => ({
      ...
      examTypeName: endoscopyExamTypeName(catalogResult, item.exam_type_code),
      reasonConditionId: "",
      reasonName: "",
      purpose: "",
      remarks: "",
  ...
  useEffect(() => {
    ...
    for (const set of pending) {
      ...
      const memberRows = memberCodesOfSet
        ...
        .filter((row): row is EndoscopyItem => Boolean(row));
  ...
  function updateItem(code: string, patch: Partial<EndoscopyOrderItemLine>) {
    setValues((current) => ({
      ...current,
  ...
  // オーダー項目の選択・解除。セットを外すと構成項目も一緒に外れ、構成項目だけを外すと
  // そのセットからその検査を除いたオーダーになる。
  function toggle(item: EndoscopyItem) {
  ...
  const splits = useMemo(
    () => splitEndoscopyOrderValues({ ...values, items: refreshedItems }),
  ...
  const booksSlot = useCallback(
    (priority: EndoscopyOrderPriority, code: string) => priority === "routine" && requiresBooking(code),
    [requiresBooking],
  );
  ...
  // 実施入力を開く項目が 1 つでもあるか(内視鏡一覧の「実施」と同じ判定)。
  // セットは検査そのものではなく依頼の束ね方なので、構成項目だけで判定する。
  const needsPerformInput = useCallback(
    (items: EndoscopyOrderItemLine[]): boolean => {
      const codes = items
        .map((item) => item.code)
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (values.items.length === 0) {
      setValidationError("オーダー項目を 1 つ以上選択してください。");
      return;
    }
    ...
    if (editing && solo && groups.length > 1) {
      setValidationError(
        `単独オーダーの項目「${solo.name}」は他のオーダー項目と同じオーダーにできません。`,
      );
      return;
    }
    ...
    if (!editing && !setMode) {
      ...
      // 予約必須の枠は予約が実施日時そのもの。選ばずには登録させない(至急・事後を除く)。
    ...
    if (future) {
      setValidationError("事後のオーダーの実施日には当日以前の日付を入力してください。");
      return;
    }
  ...
  const isSolo = (entry: EndoscopyOrderEntry) =>
  ...
  // テンプレートの既定はオーダー項目マスタが持つ。記入内容は診療記録(SOAP)と同じく
      ...
      >
        ...
        <fieldset>
          ...
          {/* 至急区分・実施日・実施時刻は伝票共通ではなく「選択中」のオーダー枠ごとに
              ...
              入るため)。 */}
          {/* 伝票共通の依頼コメントは持たない。1 オーダーの単位がオーダー項目ごとに
              変わり、伝票全体への申し送りが行き先を持たなくなったため
              (申し送りは GP 単位の特別指示に書く)。 */}
        </fieldset>

        {/* 検査伝票(レイアウト)とオーダー項目検索の切替。伝票が複数あればその数だけタブが並ぶ。 */}
        ...
        <section className="order-select__preview">
          ...
          {entries.length === 0 && (
            <p className="order-select__muted">オーダー項目を選択してください</p>
          )}
              ...
              perform={
                editing || setMode
                  ...
                  : {
                      ...
                      summary: performs[""] ? endoscopyPerformSummary(performs[""]) : null,
                    }
              }
              schedule={
                <FrameDateTime
                ...
                perform={
                  editing || setMode
                    ...
                    : {
                        ...
                        needsInput: (split) => needsPerformInput(split.values.items),
                        canPerform: canPerformNow,
                        summary: performs[code] ? endoscopyPerformSummary(performs[code]) : null,
                      }
                }
                schedule={
                  reserved ? (
...
interface FramePerform {
  split: EndoscopyOrderSplit | null;
  checked: boolean;
  onToggle: (checked: boolean) => void;
  onOpen: () => void;
  needsInput: (split: EndoscopyOrderSplit) => boolean;
  canPerform: (split: EndoscopyOrderSplit) => boolean;
  summary: string | null;
}
...
function OrderFrame({
  ...
  onChangePriority: (priority: EndoscopyOrderPriority) => void;
  /** 実施日時の入力、または予約の操作。 */
  schedule: ReactNode;
  perform: FramePerform | null;
  /** このオーダー(枠内のオーダー項目すべて)を選択から外す。 */
  onRemove: () => void;
  children: ReactNode;
}) {
    ...
    <div className="rad-order-frame">
      <div className="rad-order-frame__head">
        ...
        <label className="rad-order-frame__priority">
          ...
          <select
            value={priority}
            onChange={(e) => onChangePriority(e.target.value as EndoscopyOrderPriority)}
          >
...
function GroupEditor({
  ...
  onChange: (code: string, patch: Partial<EndoscopyOrderItemLine>) => void;
...
interface SelectProps {
  ...
  onToggle: (item: EndoscopyItem) => void;
}

// 検査伝票のグリッド。マス割りはレイアウトマスタの定義そのままで、
// オーダー項目のマスにチェックボックスを重ねる。
function LayoutSelectGrid({
  ...
  layout: ReturnType<typeof useEndoscopyItemLayout>["data"];
  error: unknown;
  catalogByCode: Map<string, EndoscopyItem>;
}) {
  if (!layout) {
...
function ItemSearchTab({
  ...
  const result = useEndoscopyItemSearch({ name, active: true }, page, name.trim().length > 0);
        ...
        <>
          <ul className="order-select__search-list">
            ...
            {data && data.items.length === 0 && (
              <li className="order-select__muted">該当するオーダー項目がありません</li>
            )}
          </ul>
