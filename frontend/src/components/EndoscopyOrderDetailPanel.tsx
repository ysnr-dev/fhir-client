...
// 内視鏡オーダーの内容表示。カルテ画面の詳細モーダルから使う
// (検体検査の LabOrderDetailPanel と同じ構成)。
// 1 GP = オーダー項目 1 つ、またはセット 1 つ。

interface EndoscopyOrderDetailPanelProps {
  serviceRequest: fhir4.ServiceRequest;
  /** オーダーにぶら下がる明細(オーダー項目・セットの構成項目)。 */
  itemRequests: fhir4.ServiceRequest[];
  problemsById?: Map<string, fhir4.Condition>;
}

export function EndoscopyOrderDetailPanel({
  serviceRequest,
  itemRequests,
  problemsById,
}: EndoscopyOrderDetailPanelProps) {
  const summary = summarizeEndoscopyOrder(serviceRequest);
  const entries = orderEntries(endoscopyOrderItems(serviceRequest, itemRequests));

  const problem = endoscopyOrderProblem(serviceRequest);
    ...
    <div className="prescription-detail">
      <fieldset>
        ...
        <dl className="prescription-detail__common">
          ...
          <dd>{endoscopyOrderTime(serviceRequest) || "-"}</dd>
          <dt>入外区分</dt>
      ...
      {entries.length === 0 && <p className="patient-table__empty">オーダー項目がありません。</p>}
    </div>
  );
}

function GroupDetail({ entry, number }: { entry: EndoscopyOrderEntry; number: number }) {
