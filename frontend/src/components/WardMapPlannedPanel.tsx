...
// 日付未定の予定も同じパネルに別の見出しで並べ、当日ぶんと同じように実施できる。
...
export function WardMapPlannedPanel({
  ...
  /** 日付未定の入院予定。当日ぶんの下に「日付未定」の見出しで出す。 */
  undated: fhir4.Encounter[];
}) {
  return (
    <div className="ward-map__panel ward-map__planned">
      ...
      {encounters.length === 0 ? (
        ...
        <PlannedList encounters={encounters} {...listProps} />
      )}
      {undated.length > 0 && (
        <>
          <h3>
            日付未定 <span>{undated.length}</span>
          </h3>
          <PlannedList encounters={undated} {...listProps} />
        </>
      )}
    </div>
  );
...
function PlannedList({ encounters, patientsById, onPointerDown, onExecute }: PlannedListProps) {
  return (
