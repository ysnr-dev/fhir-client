  ...
  return (
    <div className="appointment-panel">
      ...
      {/* 元の予約と同じ診療科・担当医から探し始める。ここに来るのは診察予約と
          リハビリ・栄養指導の予約(検査予約の日時は放射線オーダーの編集から、撮影日時と
          一緒に変える。予約タブ側で入口を出していない)。枠表の種別が違うと候補が 0 件に
          なるので、元の予約の種別に合わせて探す。 */}
      <AppointmentSlotPicker
        scheduleType={
          isRehabAppointment(appointment)
            ? "rehab"
            : isNutritionGuidanceAppointment(appointment)
              ? "nutrition-guidance"
              : "consultation"
        }
