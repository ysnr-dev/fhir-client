...
- **経過表(フローシート)**: POMR 5 要素の 1 つで、「経過表」タブに出します
  ...
  - 取得は検査結果の時系列表示と同じ流儀で、まず `$distinct-dates`(precision=full)で直近 N 個の
    測定日時を集計し、いちばん古い測定日時以降の Observation をまとめて取ります(1 回の測定が
    8 件前後の Observation に分かれるため、件数ではなく測定回数で区切ります)。
  ...
  - **イベントの帯**: グラフの真上に、入院・退院・転棟・外出泊・帰院(`Encounter`)、手術の実施
    (ハブ `Procedure`)、放射線・内視鏡・生理検査のオーダー(`ServiceRequest.occurrenceDateTime`)を
    ▼ と短いラベルで出し、グラフに破線の縦線を落とします(`fhir/flowsheetEventHelpers.ts`)。
    - 列は「測定 1 回」で**時間に比例しない**ので、イベントは列の中央ではなく**列の境目**に
      置きます(中央に置くと、その測定のときに起きたように見えるため)。表示数が少ないと
      同じ境目に何日ぶんも集まるので、重い順(手術 → 入退院 → 検査)に積み、同じ種類の検査は
      1 行にまとめます。3 種類を超えたら「他N件」に畳み、内訳は `title` で読めます。
    - **検体・細菌・病理検査は出しません**。患者が動かないうえ、毎日の採血で帯が埋まります
      (時系列は検体検査時系列タブに表があります)。未来の予定も出しません。
    - 帯を**選ぶ**とその境目のイベント一覧をモーダルで出します(`FlowsheetEventModal`)。
      帯では「放射線×7」にまとまるものも、一覧では 1 オーダーずつ日時つきで並べます。
      検査名はヘッダの `ServiceRequest` ではなく明細にあるので、`_revinclude:iterate` で
      一緒に取って「内容」に出します。手術・検査は「詳細」から**カルテと同じオーダー詳細
      モーダル**(`detail=` パラメータ)を開きます。入退院はカードにならないので導線を
      持ちません。
  ...
  - **病日・術後日数**: 時刻の見出しの下に出します。病日は入院日を 1 日目とし、入院前・退院後の
    列は空です。術後は手術当日を「当日」とし、90 日を超えたら出しません(既往になるため)。
