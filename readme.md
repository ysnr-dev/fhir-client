...
- カルテのファイル取込: カテゴリの追加・並べ替え、PDF・PNG・テキストの一括登録(診療日とカテゴリの一括設定)、一覧が診療日の降順で並ぶこと、カテゴリでの絞り込み(上流の `category` 検索)、PDF・画像・テキストのプレビューとダウンロード、表示名・診療日・カテゴリの編集で並びが変わること、削除、7MB 超のファイルが取込時に弾かれて他のファイルは残ること、ドロップでの追加と枠の強調を Docker 環境の画面と上流 REST で確認
