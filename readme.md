...
病棟で絞らず**院内の入院中をすべて取ってから**ベッド id で突き合わせます
（`Encounter?status=in-progress&class=IMP&_include=Encounter:subject`）。病棟のベッド id を
...
- 選んだ病棟は URL の `?ward=` に持ちます。カルテへ渡す戻り先は検索文字列を含むので、
  カルテから「← 戻る」で同じ病棟が開きます。
- 病棟が未指定なら先頭の病棟を自動で開きます（履歴は汚さず replace で書き換え）。
- 「患者選択」→ 患者を検索して選ぶ → 診療科（必須）・主治医・入院日（既定は今日）・特記事項で登録。
...
- **診療科・主治医で絞り込めます。** 選択肢は院内の入院中の Encounter から作るので、実際に
