...
化学療法の治療計画の雛形(レジメン)を施設共通のマスタとして登録します。詳細は `docs/chemo-regimen-design.md`。
患者への適用(レジメンオーダー)は未実装です。
...
- **API**: `GET/POST /master/regimens`、`GET/PATCH/DELETE /master/regimens/:id`(id でもレジメンコードでも可)、`POST /master/regimens/:id/copy`。
  本体と子を 1 リクエストで読み書きし、子は送った種別だけ配列で丸ごと置換します。詳細は薬剤名・用法名をマスタから解決して返します。
