...
- **左ペインのタブ**: 「カルテ」「病名」「アレルギー」「プロファイル」「経過表」「検査結果」「細菌検査」。
  病名・アレルギー・検査結果は一覧・
...
- **経過表(フローシート)**: POMR 5 要素の 1 つで、「経過表」タブに出します
  (`components/VitalFlowsheetPanel.tsx`)。入院患者の 1 週間を 1 枚で読む画面で、上から
  イベントの帯・温度板グラフ・測定項目・食事摂取量・水分出納・注射・内服・看護・検査を同じ
  横軸に並べます。カルテ以外のタブ
  なので、上下分割にすると**上にカルテ、下に経過表**を並べて「SOAP を書きながら推移を見る」
  読み方ができます。**設計は `docs/flowsheet-design.md`**。
  - **横軸は基準日までの期間**(1 週間・2 週間・1 か月から選択。左が古く、右端が基準日で、
    紙の温度板と同じ向き)。「◀ ▶」は期間ぶん送ります。
    測定の無い日も 1 列置き、1 日の中は測定ごとに列が分かれます。日付の見出しは
    `MM/DD(曜)` で、列幅はパネルの幅を使い切ります。**日付を押すとその日の 24 時間表示**に
    切り替わり(枠が 1 日から 1 時間に変わるだけで作りは同じ)、もう一度押すと元の週に戻ります。
    持続点滴は 24 時間表示だと開始〜終了がバーで出ます。
  - **行は測定項目**。手入力のバイタル・テンプレート抽出・看護観察を同じ表に載せます
    (検査値は合流させません。検査結果タブに表・グラフがあるため)。**読み取り専用**で、
    編集はカルテのバイタルカードから行います。
  - **異常値**は施設設定「バイタルの異常値」のしきい値で色付けします。判定は表示時に行い
    `Observation.interpretation` には書かないので、しきい値を変えれば過去の測定にも効きます。
  - **注射**は薬剤の組ごとの行で、予定を空丸・実施を塗り丸で出します。印を選ぶと一覧が開き、
    「詳細」でオーダー詳細、「実施入力」でカルテと同じ実施入力モーダルへ飛べます。
  - **内服**は薬剤の組(RP)ごとの行で、与薬の予定を空丸・与薬を塗り丸・与薬せずを丸+×で
    出します。処方は「1日3回・食後」までしか持たないので、**用法コード(16桁)を復号して**
    施設の食事時刻からの前後(施設設定「内服の与薬の時刻」)で予定時刻を出します。頓用や
    「決まった時刻に」など時刻が決まらない用法は予定を出しません。印を選ぶと押した枠の
    一覧が開き、「与薬入力」でその枠を記録、記録済みなら「与薬を取消」ができます。対象は
    入院処方だけで、進捗 Task(薬剤部の調剤)は動かしません。**設計は
    `docs/prescription-order-design.md`**。
  - **看護観察・看護行為**は指示ごとの行で、欄を分けて出します。印を選ぶと一覧が開き、
    「実施入力」から指示簿と同じ実施入力モーダルへ飛べます(押した指示 1 件だけが出て、
    押した印の日時が記録日時の初期値になります。指示簿から開くとラウンド用に全指示が出ます)。指示は日時ではなく期間と
    頻度で持つので、予定の印は表示している日ごとに頻度から時刻を展開して作ります
    (指示簿の実施入力と同じ設定)。頻度が自由記載の指示は予定を出しません。
  - **食事摂取量**は主食・副食の行で、日の枠の中に朝・昼・夕を並べます(24 時間表示では
    08/12/18 の枠に落ちます)。値は割(0〜10)で、押すとその食事の主食・副食だけの入力が
    開きます。記録は**食事オーダーの実施**(`basedOn` がその食事を出しているオーダー)に
    するので、摂取量の指示を別に立てる必要はありません。オーダーの無い日・食止め・欠食の
    食事は枠を出しません。カルテのカードには出しません(継続オーダーなので「実施済」が
    定まらないため)。
  - **水分出納**は枠ごとの IN・OUT・バランスの合計(mL)です。どの看護観察を IN / OUT に
    数えるかは施設設定で選びます(同じ名前で単位違いの項目があるため MEDIS の管理番号で
    持ちます)。注射は投与量換算マスタで mL に直して IN に足し、換算できない薬剤があれば
    見出しに印を出します。選んでいなければ欄ごと出ません。
  - **検査**は放射線・内視鏡・生理の種別ごとの行です(検体系は出しません)。注射と同じ印で
    予定を空丸・実施済を塗り丸にし、実施した日に置きます。
  - **病日・術後日数**を日付の下に出します。入院日が 1 日目、手術当日が「当日」です。
  - 基準日と全画面は URL の `view` に載るので、リロードしても戻らず、特定の週をリンクで
    共有できます。手入力のバイタルは時刻の見出しからバイタル編集(右ペイン)を開けます。
...
全 16 種別のオーダーで `ServiceRequest` の日付の意味を揃えてあります(2026-09-01。`fhir/shared.ts` 冒頭、
検討の経緯は `docs/order-common-backlog.md` §1)。

- **`authoredOn` = オーダー登録日時**。初回保存時にクライアントがシステム時刻を秒+オフセット付きで入れ
  (`nowFhirDateTime` / `registrationAuthoredOn`)、編集では元の値を引き継いで動かしません。DO(流用)は新規
  扱いです。フォームに入力欄はなく、詳細モーダルの「登録日時」行(`RegisteredAtRow`)で見ます。
- **`occurrenceDateTime` = オーダー開始日(実施予定日)**。種別ごとの「検査日」「撮影日」「実施日時」「注射日」
  「投与開始日」「開始日」「採取予定日時」「投与予定日時」「予定手術日」「希望日」はすべてここに入ります。
  全種別で必須で、未定(値を出さない)を許すのは手術だけです。カルテのカードの位置・診療日ペイン・
  部門一覧の日付軸はすべてこの値です(`orderDay`)。
- **例外は処方一覧の日付軸だけ**で、交付日(登録日の日付、`authoredon`)で引きます。薬剤部は当日交付された
  処方を受け取って開始日前日までに調剤するので、投与開始日で引くと入院の定期処方が開始日まで見えません。
  投与開始日は一覧の列で見せます。注射は 1 日 1 オーダーなので注射日(`occurrence`)で引きます。
...
- **カードを置く診療日**は「開始日(`occurrence`)。無ければ、未定を許す種別なら『日付未定』、それ以外は
  登録日(`authoredOn` の日付)」です(`orderCardDay`)。後者は `occurrence` を書く前に登録された旧データの
  ためのフォールバックで、上流の補完後は起きません。
...
- **本流と先読み**: タイムラインのオーダーは**開始日(`occurrence`)が今日以前のものを新しい順**に
  ページングします(`_sort=-occurrence` + `occurrence=le{today}`、`useKartePrescriptionsInfinite`)。
  カードを置く軸と読む軸が同じなので「この日より新しい日は読み切った」というカットオフ判定が厳密に
  成り立ちます。日付未定のものと開始日が今日より後のものは、種別ではなく**状態で切った別クエリ**
...
- **診療日ペイン**の日付も同じ出どころで、`$distinct-dates` を `date-param=occurrence` の 1 本で当てます。
  看護指示はカードにしないので、本流・先読み・診療日のいずれも `category:not` で外します。
...
処方・検体検査・放射線検査などをまとめて保存し、カルテから一括で出す仕組みです。詳細は `docs/order-set-design.md`。

- **入口**: 「診療業務 > セット登録」(`/order-sets`)で登録・整理、カルテ右ペインの「セット」で適用。
- **持ち主は 3 段階**: 院内共通 / 診療科 / 医師。編集できるのは順に「医師なら誰でも / その科を担当する医師 / 本人」。
  医師かどうかは画面側(PractitionerRole)で判定し、backend が厳密に守るのは医師スコープの持ち主 = ログイン本人だけです。
  医師以外は閲覧のみ。フォルダで任意の深さに整理でき、複製で別の持ち主へ写せます(自分 → 診療科 → 院内共通への昇格)。
- **登録は通常のオーダー登録と同じ画面**。患者と日付に関わる入力(対象プロブレム・投与開始日・依頼病名・予約・即実施など)は
  出ません。保存する中身はフォームの入力値そのもの(backend の `order_set_entries.values`、jsonb)で、患者への参照は保存前に落とします。
- **適用は「全部のフォームを出して、直してから一括登録」**。各オーダーは既定で閉じていて(「すべて開く / すべて閉じる」で一括開閉)、
  除外するものはチェックを外します。入外区分は患者の在院状況に合わせて開きます(セットと食い違うと注意が出ます)。
- **適用日**を上で選ぶと、各オーダーの開始日(投与開始日・検査日・撮影日など)にまとめて入ります。既定は当日で、
  予約が必要な検査だけは予約した枠の日時が優先されるため動きません。個別に直したいオーダーは開いて日付を変えられます。
- 1 つの transaction なので途中まで登録されることはなく、来歴(Provenance)も 1 回の適用につき 1 件です。
  1 件でも入力検証に落ちると何も登録されません。
- **登録されたオーダーには出どころのセットが焼き付きます**(ヘッダ ServiceRequest の identifier `order-set-instance` と拡張 `order-set`)。
- 代行入力(医師以外のログイン)でカルテを開くと、「医師のセット」はログインユーザーではなくヘッダーで選んだ**指示医師**のものになります。
- 対応種別は処方・注射・検体検査・放射線検査・生理検査・処置(残りは順次)。
...
- **オーダー項目 → 結果項目の対応**(`master_lab_order_item_results`、1:N)は検査オーダー項目マスタの編集画面の
  「返す結果項目」で決めます。結果登録画面はこの対応表でオーダーの検査項目を結果の行に展開します
  (血液ガス分析 1 件 → pH・PCO2… のように分かれる項目もここで表します)。パネルは構成項目がそれぞれ結果項目に
  対応するので、対応づけるのは単項目だけです。対応の無いオーダー項目は「展開していません」と名前を挙げます。
- **セットの中のパネル**(生化学セットの中の「ナトリウム及びクロール」など)はオーダーの明細に展開されないため、
  対応が無い項目は**パネル構成をたどって**結果項目まで解決します(`expand_panels=true`)。入れ子の深さは問いません。
- **初期投入**は単項目オーダー項目 1 件につき同じコードの結果項目を 1 件作って 1:1 で対応づけます
  (`Master::LabResultItemDerivation`。migration と `db:seed` の両方から呼び、既存行は上書きしません)。
  単項目なのに複数の結果を返す 6 件(尿沈渣・蛋白分画・血液ガス分析・精液一般・髄液一般)は分解した結果項目を
  `db/seed_data/lab_result_items.csv` / `lab_order_item_results.csv` で同梱しています。オーダー項目に JLAC11 があれば
  配布マスタからデータ型・単位・選択肢を写します。新しいオーダー項目には「同じコードで結果項目を作成」の導線があります。
- **`Observation.code` の coding** は 施設の結果項目コード(`lab-result-item`)→ JLAC11 → JLAC10 → 略称 の順です。
  読み出し・時系列の行まとめ・検査分野のグルーピングは施設コードをキーにします。
- **結果項目マスタ導入前の検査結果**(施設コード無し、JLAC11 だけ)は読み出し側で読み替えます。保存済みの 17 桁は
  試薬・機器単位でマスタの代表コードと下 5 桁が違うため、測定物・識別・材料の **12 桁**で結果項目を引き当て、
  時系列では同じ行に合流させます(材料コードまで違う旧結果は別行のままです)。編集して保存し直すと施設コードが付きます。
- 感染症・腎機能・レジメンの検査値チェックは従来どおり JLAC11 の分析物コード(先頭 5 桁)で突き合わせます。
  結果項目に JLAC11 があれば coding に併記されるので動作は変わりません。

### 基準値と H/L の自動判定

数値型の結果項目には**基準値**(`master_lab_reference_ranges`)を性別・年齢帯ごとに複数行持てます
(結果項目マスタの編集画面の「基準値」)。適用する行は、性別が一致(または共通)し、検体採取日の満年齢が
年齢帯に入る行のうち表示順で先のものです。男女別の行と共通の行を混ぜるときは並びで優先を決めます。

- **結果登録画面**は結果値を入れた時点で基準値と突き合わせ、H/L のプルダウンに自動で入れます(下限未満 = L、
  上限超 = H)。手で直せます。基準値の列も並べて出します。基準値の列を足したぶん、登録フォームと内容表示の
  検査項目は**略称だけ**にし(略称が無ければ名称)、正式名称と長い材料名はツールチップで読みます。
  カルテの左ペインでも行が折り返さない幅に収めるためです。
- **保存時**に適用した基準値を `Observation.referenceRange`(`low` / `high`、`type = normal`)に写します。
  内容表示と時系列表示の「基準値」列はこの保存値を読むので、後でマスタを変えても過去の結果の基準値は
  変わりません(時系列は並びが先の結果、つまり最新の基準値を出します)。
- 患者の性別・生年月日が無いときは、性別・年齢帯の指定が無い行だけが当たります。基準値の無い項目や
  文字列・コード型の項目は従来どおり手で選びます。
- **初期値**を `db/seed_data/lab_reference_ranges.csv` に同梱し、`db:seed` で入れます(148 行 / 128 項目)。
  出どころは行の備考に書いてあり、JCCLS 共用基準範囲・代表的なカットオフ値(腫瘍マーカー・BNP・D ダイマー
  など上限だけのもの)・参考値の 3 通りです。**migration からは入れません**。基準値は施設の測定系・試薬で
  決まる値なので、運用前に画面で確認・修正する前提です(ALP は IFCC 法、凝固系は試薬依存、蛋白分画は
  セルロースアセテート膜法の値です)。
- 基準値を入れる項目のうち、配布マスタに JLAC が無くて単位が入らなかったものは
  `db/seed_data/lab_result_item_units.csv` で単位を補います(未設定のときだけ入れます)。

## 化学療法レジメンマスタ

化学療法の治療計画の雛形(レジメン)を施設共通のマスタとして登録し、患者に適用して注射・処方オーダーに展開します。
詳細は `docs/chemo-regimen-design.md`。

- **入口**: 「マスタメンテ > 化学療法 > レジメン」(`/regimens`)。一覧から行を選ぶと 1 レジメン 1 ページの編集画面(`/regimens/:id`)。
- **持ち方**: backend の `master_regimens` と子テーブル(適応疾患・投与ステップ・薬剤・検査基準・副作用)。オーダーセットのような
  持ち主の階層は持たず、診療科と適応疾患(病名マスタ)を分類属性に、承認状態(下書き / 承認済 / 廃止)・承認日・承認者を持ちます。
- **投与ステップ**: 注射オーダーの RP(混注)に相当する単位で、順序・相対日(Day 1, 8, 15 …)・点滴 / ワンショット / 内服・経路・手技・
  ライン・点滴時間・器材を持ち、その中に薬剤(種類・算出基準(体表面積 / 体重 / AUC / 固定量 / 製剤単位)・基準値・上限値)を並べます。
  1 クール = 投与期間 + 休薬期間で、編集画面の Day 表でどの日に何が入るかを確認できます。
- **API**: `GET/POST /master/regimens`、`GET/PATCH/DELETE /master/regimens/:id`(id でもレジメンコードでも可)、`POST /master/regimens/:id/copy`。
  本体と子を 1 リクエストで読み書きし、子は送った種別だけ配列で丸ごと置換します。詳細は薬剤名・用法をマスタから解決して返します。
- **レジメンオーダー(患者への適用)**: カルテ右ペインの「化学療法」で承認済のレジメンを選び、開始日(Day 1)と体格(身長・体重 →
  体表面積)から投与量を出して、クール単位で登録します。適用 1 件はヘッダの `ServiceRequest`(`order-type|chemo-regimen`、
  `intent = plan`)で、日ごとの投与は**通常の注射・処方オーダー**(`requisition` と `regimen-order` 拡張でどのクールの何日目かを持つ)
  として登録されるので、注射一覧・払出・実施入力・経過表はそのまま使えます。カルテ左ペインの「化学療法」タブが月の暦で
  投与スケジュールを出し、適用が複数あればタブで切り替えます。移動・中止・次クールの登録・レジメンの中止は右ペインで行います。
...
## 患者の基本情報（`Patient` の拡張項目）

患者登録・編集画面（`components/PatientForm.tsx`）で、氏名・住所・電話に加えて次を登録できます。
いずれも任意で、新患登録（受付時）の必須は従来どおり氏名・カナ・性別・生年月日だけです。

**入力欄の並びは新患登録モーダルと共通のレイアウト**（CSS の `.patient-fields`）です。窓口が
同じ患者属性を同じ並びで書けるよう、関連する欄を横に並べます。患者登録画面・患者編集画面・
カルテのプロファイルタブの 3 か所すべてで同じ見た目になります。

情報のかたまり（属性 / 住所 / 本人の連絡先 / 緊急連絡先 / かかりつけ医）は
**枠で区切ります**（`.patient-fields__group-box`）。項目が増えるとまとまりの名前だけでは
切れ目が読めなくなるためです。**新患登録モーダルだけは枠を使いません**。受付の最中に書く
画面で項目も少なく、枠を足すと縦に伸びて受付情報まで届かなくなるためです。

| 項目 | 置き場所 | 補足 |
|---|---|---|
| 旧姓 / 通称名 | `name`（`use=maiden` の `family` / `use=nickname` の `text`） | 漢字名・カナ名の後ろに置きます（上流が先頭の `name` を検索用に採るため） |
| 死亡日 | `deceasedDateTime` | `deceasedBoolean` は書きません（併記すると上流が弾きます） |
| 使用言語 / 通訳要否 | `communication.language`（BCP-47） / `preferred` | 言語未指定で通訳だけを立てるときは `language` を `und` にします（1..1 のため） |
| 緊急連絡先・キーパーソン | `contact[]` | 続柄・氏名・電話・住所。何件でも登録できます |
| かかりつけ医・紹介元 | `generalPractitioner` | 連携先医療機関または医師を検索モーダルから選びます |

- **続柄は複数選べます**（v2-0131 の `C` 緊急連絡先 / `N` 近親者 / `BP` 支払・保証人 /
  `E` 勤務先 / `U` 不明）。「キーパーソン」単独のコードが無く、近親者かつ緊急連絡先のように
  重なるためです。「長女」のような補足はコードで表せないので、コードを持たない `text` だけの
  concept として同じ `relationship` に添えます。
- **連絡先の氏名にカナはありません**。`Patient.contact.name` が 0..1 で、本人の氏名のように
  漢字とカナを別の `HumanName` に分けられないためです。
- 死亡した患者は**患者一覧の氏名の後ろと患者帯に赤で出ます**。死亡後のオーダー抑止は
  入れていません（表示のみ）。
- 使用言語と通訳要否は**患者帯にも出ます**（「ポルトガル語 / 通訳必要」）。

## 血液型（`Observation`）

カルテの「プロファイル」タブの **身体** 区画で患者の血液型を登録します。ABO と RhD は
LOINC が別項目（883-9 / 10331-7）なので `Observation` を 1 件ずつ作り、「ABO は分かっているが
RhD はこれから」という状態をそのまま保存できます。保存は片方だけ書かれた状態を作らないよう
transaction でまとめて送ります。コードの値集合は輸血オーダーと共有です。

| 要素 | 内容 |
|---|---|
| `code` | LOINC 883-9（ABO）/ 10331-7（RhD） |
| `valueCodeableConcept` | 輸血オーダーと同じ `transfusion-abo` / `transfusion-rhd` のコード |
| `method` | 情報源。`tested`（検査確定）/ `reported`（本人・家族の申告）/ `referred`（他院からの情報） |
| `effectiveDateTime` | 確認日（検査日または申告を受けた日） |

- **情報源は値と必ず並べて出します**。申告のままの型で製剤は出せないので、検査確定でないときは
  「検査未確定」の印を付けます。
- **患者帯にも出ます**（「血液型 A型 RhD＋」）。輸血・手術で真っ先に確かめるためです。検査で
  確定していない型には「検査未確定」の印を付けます。確認日は帯には出しません（行を増やさない
  ため）。詳細はプロファイルタブで読みます。
- **輸血オーダーの血液型の初期値に入るのは検査確定の型だけです**（新規登録のときのみ。編集や
  DO で写った型は上書きしません）。申告値はオーダー画面に参照として並べるに留めます。
- 患者の型とオーダーで選んだ型が**食い違うときは赤字で知らせます**。選択自体は妨げません。
  意図して別の型を出すことがあるためです。
- 輸血オーダー側の血液型の項目は残してあります。オーダーの型は「何型を出すか」という依頼の
  中身で、患者の血液型と食い違いうるためです。

## 身長・体重と腎機能（読み取り専用）

プロファイルタブの **身体** 区画に、バイタルと検体検査の結果から**最新値を引いて**出します。
本体はそれぞれの画面にあるので、ここでは編集できません（同じ情報を 2 経路に持たないためです）。

| 項目 | 引き当て | 補足 |
|---|---|---|
| 身長 / 体重 / BMI | LOINC 8302-2 / 29463-7 | 測定日を併記します。BMI は両方あるときだけ出します |
| 血清クレアチニン | JLAC11 分析物 `C3002` | 材料・測定法の違いは分析物コードで吸収します |
| シスタチンC | JLAC11 分析物 `C3003` | あれば出します。eGFR の算出には使いません |
| eGFR | 血清クレアチニンから算出 | 日本腎臓学会の推算式 |

- **測定日を必ず併記します**。小児のように値が変わる患者で、いつの測定かで読み方が変わる
  ためです。身長と体重の測定日が離れていても BMI は出します（体重だけ測り直すのが普通です）。
- **eGFR は算出値です**（男性 194 × Cr^-1.094 × 年齢^-0.287、女性はその 0.739 倍）。配布マスタに
  eGFR の項目が無いためです。**性別か生年月日が未登録のときは算出せず、その理由を出します**。
  薬用量の判断に使う値なので、当てずっぽうを出しません。
- 身長・体重は**患者帯には出しません**（プロファイルタブで読みます）。帯の枠は限られており、
  輸血・手術で真っ先に確かめる血液型を優先しています。

## 妊娠・授乳（`Observation`）

血液型と同じ **身体** 区画で登録します。妊娠状態（LOINC 82810-3）と授乳状態（63895-7）は
別項目なので `Observation` を 1 件ずつ作り、「妊娠していないが授乳中」もそのまま保存できます。

| 要素 | 内容 |
|---|---|
| `code` | LOINC 82810-3（妊娠状態）/ 63895-7（授乳状態） |
| `valueCodeableConcept` | SNOMED CT（妊娠中 77386006 / 妊娠していない 60001007 / 不明 261665006、授乳中 413712001 / 授乳していない 169750002） |
| `component` | 分娩予定日（LOINC 11778-8）。妊娠中のときだけ入れます |
| `effectiveDateTime` | 確認日（必須） |
| `category` | `survey`。問診・観察で得る情報なので、検体検査の結果一覧に混ざりません |

- **確認日は必須です**。いつ時点の状態かが分からないと使えないためで、画面は常に確認日の
  新しいものを出します。「不明」を選択肢に残すのは、確認したが分からなかったことと、
  まだ聞いていないこと（登録が無い状態）を区別するためです。
- **処方と放射線検査のオーダー画面の先頭に注意が出ます**（`components/PregnancyNotice.tsx`）。
  催奇形性・乳汁移行と被曝の判断に要るためです。妊娠中・授乳中のときだけ出し、そうでなければ
  何も出しません。常に出すと読み飛ばされ、本当に注意が要るときに目に入らなくなるためです。
- **オーダーは妨げません**。妊婦にも必要な検査・薬はあり、判断は医師のものだからです。

## 感染症（`Observation`）

カルテの「プロファイル」タブの **感染症** 区画で、種類ごとに 1 行で読みます。情報は 2 経路から
集まります。

- **検体検査の結果**（`category=laboratory`）。JLAC11 の**分析物コード（先頭 5 桁）**で
  「この結果は HBs 抗原」と判定し、自動で拾います。陽性・陰性は**値の文字列を先に読みます**
  （「陽性」「(+)」「reactive」など）。検査結果の登録画面は異常フラグ（`interpretation`）を
  選ばなければ `N`（正常）で保存するので、判定を先に見ると「陽性」と入れた結果まで陰性に
  なってしまうためです。異常フラグは値から読めないときの補助に留めます。
  値から読み取れない結果（抗体価などの数値）は「**判定不明**」にします。基準値の持ち方が項目
  ごとに違い、ここでは決められないためで、陰性と決めつけません（橙色で出します）。
- **手入力**（`category=exam`）。検査していないが他院情報・本人申告で分かっている分です。
  `code` はローカルコード（`infection-type`）を先に置き、LOINC を添えます。LOINC だけでは
  種類を引き直せず、ローカルコードだけでは標準の相手に読ませられないためです。

同じ種類が両方にあるときは**検査結果を優先**します。検査で出た値が正だからです。そのため
**検査結果由来の行は画面から編集できません**。手入力分だけ編集・削除できます。「未検」は
`Observation` を作らないことで表すので、結果の選択肢は陽性・陰性だけです。

**陽性が 1 つでもあると患者帯にバイオハザードのピクトグラムが出ます**（赤）。標準予防策に
加えるかの判断に直結するためです。種類が複数あってもアイコンは 1 つで、件数を右肩に添え、
種類・情報源・確認日は押して開く吹き出しに出します（帯にバイオハザードが並ぶと、どれが何か
読めないまま場所だけ取るためです）。この図柄は**注意区分マスタの選択肢には出しません**。
感染症は注意として登録するものではなく、二重管理になるためです。

### 分析物コードの粗さ

対応づけは `fhir/infectionHelpers.ts` の定数（`INFECTION_TYPES.analytes`）に持ちます。分析物
コードは配布マスタが決める全国共通のコードで施設ごとに変わらず、種類の一覧自体も定数なので、
施設マスタにはしていません。

- **材料（血清 / 血漿）と測定法は分析物コードでは分かれません**。1 コードでその種類の検査を
  全部拾えます（HBs 抗原は 229 行が 1 件で済みます）。
- **大項目まで粗くすると誤ります**。大項目「HBs」は HBs 抗原と HBs 抗体の両方を含みますが、
  抗原陽性は感染、抗体陽性はワクチンか既感染で意味が逆です。
- **HTLV-1・MRSA・結核は検査結果から拾いません**（手入力のみ）。配布マスタの項目名から分析物を
  特定できなかったためで、分かった時点で定数に足します。
- 上流のコード検索は完全一致で前方一致が使えないため、患者の検体検査の結果を新しい順に 500 件
  引き、突き合わせは画面側で行います。

## 診療上の注意（`Flag`）と注意区分マスタ

カルテの **「プロファイル」タブ** に、患者の「現在の状態」を区画ごとに読むタブがあります
（[`docs/patient-profile-design.md`](docs/patient-profile-design.md)）。時系列ではないので
タイムラインには出ません。現在の区画は **診療上の注意** と **基本情報** で、身体（血液型・妊娠）・
感染症・生活などの区画を後から足す前提の構造になっています。

基本情報の「患者情報を編集」は、**患者編集画面へ遷移せずタブの中で開きます**（注意の登録・編集と
同じ挙動）。カルテを見ているところから離れず、戻り先を見失わないためです。楽観ロックと競合時の
扱いは患者編集画面と同じです。

注意は 1 件が 1 つの `Flag` です。

| 要素 | 内容 |
|---|---|
| `status` | `active`（有効）/ `inactive`（終了）。画面から `entered-in-error` は書きません |
| `category` | `http://fhir-client.local/CodeSystem/flag-category`。`safety`（安全）/ `clinical`（臨床）/ `advance-directive`（意思）/ `administrative`（事務） |
| `code` | `http://fhir-client.local/CodeSystem/patient-caution`。コードは注意区分マスタの `code`、`display` は登録時点の表示名。`text` に内容の自由記載 |
| `subject` | `Patient/{id}` |
| `period` | 開始日・終了日（どちらも任意）。終了日が無ければ継続中 |
| `author` | ログイン中の医療従事者（無ければ省略） |

- **削除は用意していません**。誤登録も「終了」（`inactive` ＋ 終了日）で残します。一覧の既定は
  有効なものだけで、「終了したものも表示」で全件になります。
- 上流は JP Core が Flag をプロファイルしていないため、HL7 の基本定義で登録されます
  （`docs/server-improvement-backlog.md` の 2026-09-05 の項）。

### 注意区分マスタ（`/patient-cautions`）

「何を注意として選べるか」と「患者帯にどう出すか」を決める施設マスタです（マスタメンテ > 共通 >
注意区分）。列は コード / 表示名 / 区分 / **ピクトグラム** / 表示順で、seed に初期値 15 件が入って
います（転倒リスク・暴力/粗暴歴・離院リスク・体内金属・造影剤注意・抗凝固薬内服・認知症/せん妄・
車椅子・聴覚障害・視覚障害・DNAR・輸血拒否・未収金・要配慮・その他）。

- ピクトグラムの図柄は `frontend/src/components/icons/cautionPictograms.tsx` の 1 モジュールに
  まとまっており、**選択肢もそのキー一覧から作ります**（定義を 2 か所に持たないため）。backend
  にも同じ一覧を検証用に持っていますが、正は TS 側です。
- **ピクトグラムが空の区分は患者帯に出ません**（「帯に出すか」の真偽値は別に持ちません）。

### 患者帯のピクトグラム

患者帯（`PatientHeader`）は行を増やさないルールなので、注意は **文字ではなくピクトグラム**で
出します。有効な注意のうち、マスタにピクトグラムがあるものだけを並べます。

- 図柄は区分ごとに色分けします（安全=赤 / 臨床=橙 / 意思=紫 / 事務=灰）。大きさは 20px で、
  帯の文字（14px）より少し大きくしてあります。離れた席からでも図柄を読めるようにするためです。
  帯全体は baseline 揃えですが、アイコンには基準線が無く下寄りに見えるので、行の中央に
  合わせています。
- 同じ図柄が複数あれば 1 つにまとめ、右肩に件数を添えます。並びはマスタの表示順です。
- **押すと吹き出しが開き**、名称と内容が読めます。もう一度押す・外側を押す・Escape で閉じます。
  吹き出しの中のリンクから、対応する画面（プロファイルタブ・アレルギータブ）へ移動します。
  ホバーの `title` ではなくクリックにしているのは、帯のアイコンは離れた席からも見るもので、
  内容を読むのに正確なホバーを要求したくないためです。読み上げには同じ文言を `aria-label` で
  持たせています。
- マスタに無いコードの注意は帯に出ません（タブには出ます）。マスタのピクトグラムが未知のキーの
  場合は汎用の三角（`alert`）で描き、黙って消しません。

**注意以外もここに並びます**（いずれも赤で、注意区分マスタの選択肢には出しません）。

- **アレルギー**。薬剤（カプセルに禁止記号）とそれ以外（皿に禁止記号）で図柄を分けます。
  薬剤禁忌は処方・注射で真っ先に確かめるもので、食物・環境のアレルギーとは見るべき場面が
  違うためです。活動性のものだけを出し、解消済み・非活動は出しません。吹き出しの
  「アレルギーを開く」でアレルギータブへ移動します。
- **感染症**（バイオハザード）。陽性が 1 つでもあれば出ます。
...
## 病棟マップ（master_ward_maps + Encounter）

入院患者一覧の別の見え方。病棟の間取り(設備・病室・ベッドの配置)の上に、その日の
入院患者をベッドごとに出す。設計は `docs/ward-map-design.md`。

### レイアウトの持ち方

- backend の `master_ward_maps` に 1 病棟 1 行。`ward_location_id`(FHIR Location の id、
  一意)・`ward_name`(表示用の写し)・`layout`(jsonb)・`note`。API は
  `/master/ward_maps`(index は `?ward_location_id=` で絞れる。show/create/update/destroy)。
- `layout` はキャンバスの大きさ(マス数)・1 マスの px・オブジェクト配列。オブジェクトは
  `fixture`(設備。種別・任意の表示名・回転・色)/ `room`(病室 Location の id)/
  `bed`(ベッド Location の id と所属病室)。座標・大きさはマス数の整数。形は backend
  (`Master::WardMap#layout_shape`)と frontend(`normalizeLayout`)の両方が守る。
- ベッドが何床あるかは Location だけが正。レイアウトは「どこに描くか」しか持たず、消えた
  Location は欠番、置いていない病室・ベッドは未配置として扱う。

### 画面

- **病棟マップ**(`/ward-map?ward=&date=`、患者一覧メニュー)。病棟と日付を選ぶと、
  レイアウトの上に入院患者のカード(番号・患者番号・氏名・性別・年齢・診療科 / 主治医・
  入院日数・注意区分 / アレルギー / 感染症のピクトグラム・退院予定 / 転棟予定 / 外出泊中の
  札)が出る。カードを押すとカルテ(戻り先を保つ)。ケバブから転室・転床 / 外出泊 /
  転科・転棟予定 / 退院予定 / 退院、空床からは入院登録。マップが未作成の病棟は病室と
  ベッドを機械的に並べて出す(保存はしない)。
- **転床(移動プラン)**。今日の表示では患者カードを掴んで空床や別の患者の床へ運べる。
  落としても書き込まず「保留中の移動」に積む(占有床へ落とせば入れ替え)。
  「一時退避」に置けば床を空けられるが、退避のままでは確定できない。「転床を確定」で
  転床日を決め、直前に入院を引き直して他端末の変更が無いことを確かめてから、全件を
  1 つの transaction で書く。病棟・日付を変えるとプランは捨てる。
- **本日の入院予定**。この病棟宛て(または病棟未定)の当日予定を並べ、空床へ落とすと
  入院実施モーダルがその床を選んだ状態で開く。
- **病棟マップ編集**(`/wards/:wardId/map/edit`、病棟一覧の行メニュー「マップ編集」と
  マップの「マップを編集」)。左に設備と未配置の病室・ベッド、中央にキャンバス、右に
  キャンバスの大きさと選んだオブジェクトのプロパティ。掴んで移動・縁でリサイズ
  (設備と病室)・回転・複製(設備)・削除・矢印キーで 1 マス(Shift で 5)・
  Ctrl+Z / Ctrl+Y・Ctrl+D・R・空白を掴んでスクロール・Ctrl+ホイールで拡大縮小。
  「自動配置」は病室とベッドを機械的に並べ直す、「整列」は病室内のベッドを並べ直す。
  保存はレイアウト全体を PATCH。未保存の変更はリロード・タブを閉じる前に止める
  (画面内のリンクでは止まらない)。

### 検証(2026-09-09、Docker 環境)

- 病棟マップ編集: 自動配置で 8 室が並ぶ、設備の追加・移動・保存・再読み込みでの復元、
  Undo、ベッドを病室の外へ出したときの警告色。
- 病棟マップ: 入院患者がベッドに出る、氏名からカルテへ行き「戻る」で `?ward=` 付きで
  戻る、空床への移動 1 件・占有床への移動で入れ替え 2 件・一時退避を積んで確定
  モーダルで「退避中のまま」が検証で止まる、同室内の移動 1 件を確定して Encounter の
  location が「先頭 active + 旧床 completed」になる、当日の入院予定を空床へ落として
  入院実施モーダルに床が入る、ピクトグラムの吹き出しとケバブが開く。
- backend: `spec/models/master/ward_map_spec.rb`・`spec/requests/master/ward_maps_spec.rb`
  (全体 1237 例、失敗なし)。
