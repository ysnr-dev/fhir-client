...
### 任意の profile(院内エージェント / 日レセ)

レセコン連携(後述)を通しで動かすときだけ要るコンテナは profile で切ってあり、既定では起動しません。

| profile | サービス | 中身 |
| --- | --- | --- |
| `agent` | `agent` | 院内エージェント(別リポジトリ `../fhir-client-agent`)。レセコンの通知をカルテへ中継する |
| `orca` | `orca` | 日レセ検証環境(別リポジトリ `../weborca-dev`)。`http://localhost:8000/` |

隣にそれぞれのリポジトリを clone したうえで、profile を指定して起動します(`--profile "*"` で全部)。

```bash
cp .env.example .env         # RECEIPT_INBOUND_TOKEN を入れる(.env は git 管理外)
docker compose --profile "*" up -d --build
```

- `RECEIPT_INBOUND_TOKEN` は管理画面「外部システム連携 > 医事会計」で発行した受信トークン。未設定だとエージェントが起動時に落ちます。`.env` に書く代わりにコマンドの前に置いて渡すこともできます
- 日レセは `weborca-dev` 側の compose と**同じコンテナ名 `weborca`・同じボリューム**を使うので、片方だけ動かします。向こうで起動していたら先に `cd ../weborca-dev && docker compose down`(`stop` だけだとコンテナが残り、名前の衝突で起動できません)
- 別プロジェクトが作ったボリュームを指すため、起動のたびに `volume "weborca-dev_pg_data" already exists but was created for project "weborca-dev"` という警告が出ますが無視して構いません
- 停止時も profile を付けます(付けないと `agent` / `orca` が残ります)
...
- **左ペインのタブ**: 「カルテ」「病名」「アレルギー」「プロファイル」「経過表」「検査結果」(検体検査・
  検体検査時系列・細菌検査・病理検査をまとめたドロップダウン)「食事」「化学療法」「パス」「指示簿」
  「予約」「ファイル」。タブの定義は `karteUrl.ts` の `KARTE_TABS` 1 か所で、足すときはここと
  `KartePage.tsx` の `renderTabPanel` に 1 行ずつ加えます。病名・アレルギー・検査結果・ファイルは一覧・
...
### 退院時サマリー(退院時要約)と文書作成の督促

右ペインの「退院時サマリー」から、**入院 1 件に 1 つ**の退院時サマリーを書きます(設計は
`docs/discharge-summary-design.md`)。診療記録と同じ `Composition` の器で、type は LOINC `18842-5`、
`encounter` に対象の入院を持ちます。対象は入院中・退院済のどちらでも選べ(退院前に用意する運用のため)、
選んだ入院に既にサマリーがあれば編集に切り替わります。

- **セクションは固定の 8 つ**(C-CDA on FHIR Discharge Summary のコード)。退院時診断・手術処置・退院時処方・
  アレルギーは候補のチェックリストで、選んだものを `section.entry` に参照として持ち、`text` はその表示名から
  生成します。入院経過・主な検査所見・退院時の状態・退院後の方針は診療記録と同じリッチテキストで、
  テンプレート記入・シェーマも同じ機構で使えます(施設独自の項目はテンプレートの埋め込みで吸収します)。
- **下書きは入院期間のデータから自動で集めます**。オーダーは入院(`Encounter`)を参照していないので、
  経過表と同じく患者 + 入院期間の日付範囲で引きます(病名・手術・処置・検査・退院処方(区分「退院」)・
  アレルギー・入院イベント)。「下書きを集め直す」で候補だけを集め直し、書いた本文と選択は残ります。
- **転帰(退院先)**は標準要素 `Encounter.hospitalization.dischargeDisposition` に持ち、退院モーダルの
  任意欄とサマリーの転帰欄のどちらからも書けます(サマリーの保存で変えたときは同じ transaction で
  `Encounter` を PUT します)。
- 確定・署名・修正済み・変更履歴・削除は診療記録と同じです。カードのバッジは「退院時サマリー」、
  左ペインの種別では「診療記録」と分けて選べます(`?card=note:discharge-summary`)。退院時診断の
  `entry` が標準の `entry` 検索に乗るので、プロブレムの絞り込みにもそのまま出ます。
- **文書作成の督促**: 退院の transaction で期限付きの通知 Task(`document-due`)を主治医あてに作ります
  (期限 = 退院日 + 施設設定「文書作成の督促」の日数、既定 14 日。`Task.restriction.period.end`)。
  定期実行の主体がシステムに無いので、N 日後にサーバーが作るのではなく退院の時点で作り、**期限超過は
  通知一覧の表示時に判定**します。サマリーを確定で保存すると同じ transaction で対応済みになり、
  退院取消で取り下げます。確定済みのサマリーがある入院と、未対応の督促が残っている入院には作りません。
  通知の「カルテ」は `?open=discharge-summary:<入院の id>` でカルテを開き、右ペインをその入院の
  サマリーで始めます(一回限りの引数で、読んだら URL から消します)。入院患者一覧の退院患者の
  操作メニューにも同じ入口があります。
...
- **処方区分の初期値**: 入外区分で選択肢が違う(入院=定期/継続/臨時/退院/緊急、外来=院外/院内)ので、
  初期値も入外区分ごとに施設設定(管理 > 施設設定「処方区分の初期値」)で持ちます。フォームを開いた
  ときと入外区分を選び直したときに入り、未選択(その都度選ばせる)のままにもできます。登録済みの処方には
  区分が焼き付いているので、設定を変えても過去の処方は動きません。
...
- 対応種別は病名と 16 種のオーダー(処方・注射・検体検査・細菌検査・病理検査・放射線検査・生理検査・内視鏡・処置・手術・食事・
  輸血・リハビリ・栄養指導・他科依頼・看護指示)。看護指示と食事は入院向けなので外来の患者に適用すると注意が出ます。
  食事はセットから出しても前の食事を終わらせません(食事タブで終えます)。手術はセットから出すと手術室の重なり確認を
  通らないので、登録後に手術カレンダーで確かめてください。
...
- **測定法**(`method_name`)は試薬・機器の名称で、結果登録時に `Observation.method` へ写します
  (「配布JLACマスタから引き当て」で配布マスタの測定法名称が入ります)。
...
|---|---|---|---|---|
| `lab-panic` 緊急異常値 | アラート | 検査結果(`DiagnosticReport`) | オーダーの依頼医 | 確認 |
...
- **強度**は「どれだけ急いで見てほしいか」で、`Task.priority` に乗せて **アラート**(`stat`)/
  **注意**(`urgent`)/ **お知らせ**(`routine`)の 3 段階に写します(`asap` もアラートとして読みます)。
  種別ごとの固定値ではなく通知 1 件ごとの属性なので、同じ種別でも状況によって変えられます
  (対応表は `fhir/notificationHelpers.ts`)。オーダーの至急区分(通常・至急・事後・緊急)も同じ
  `priority` を使いますが、あちらは別の対応表です。
- 種別ごとの「どう見せて、どう対応済みにするか」は `components/notifications/notificationRegistry.tsx` の
  対応表に集めています。新しい通知を足すときは、Task を作るヘルパー(強度はここで指定します)と内容セルを
  書いて対応表に 1 要素足すだけで、一覧・件数・種別フィルタ・一括対応に乗ります。
- **一覧**は未対応のものを新しい順に並べ、**種別と強度**で絞れます。強度は行の列にバッジで出し、
  アラートの行は左端に帯を引きます。**強度では並べ替えません**(いつ届いたかが読めなくなるため)。
  どちらの絞り込みの件数も絞る前の全件から数えるので、2 つのセレクトは独立した軸として読めます。
  既定は自分あてだけで、「自分あてのみ」を外すと宛先の決まらない通知(オーダーに紐付かない検査結果など)も
  出ます。取得は `Task?code=<種別をカンマ区切りで OR>&status=requested&owner=…&_include=Task:subject` の
  1 本で、絞り込みは取得済みの行に対して画面側で行います。
- **ヘッダーのベル**に自分あての未対応件数を常に出します(緊急異常値は連絡が遅れると患者に害が出るので、
  メニューを開かないと気付けない置き方にしません)。未対応があるベルは色を付けて件数を肩に載せ、
  **アラートが 1 件でもあるときだけ赤く**します(お知らせが溜まっているだけの状態と分けるためです)。
  件数は `_summary=count` を 2 本(合計と `priority=stat,asap`)引きます。
...
- 行から**カルテ**(その検査結果を開く)と**確認**ができます。確認すると通知が完了になり、
  誰がいつ確認したかが `Task.note` に残ります。結果が最終報告なら、**検査結果の既読(来歴)も同時に残します**
  (次の節の「緊急の通知と検査結果確認は 1 件にまとめる」)。
- **訂正に追随します**。値を直してパニック値でなくなれば通知を取り下げ(`cancelled`)、訂正でまたパニック値に
  なれば同じ通知を書き換えて未確認に戻します(内容が変わったので改めて見てもらうためです)。
- 部門の進捗(受付・到着)の Task とは別物です。あちらは焦点がオーダーで宛先を持たず、こちらは焦点が
  検査結果で宛先と未確認・確認済みを持ちます(`fhir/labPanicHelpers.ts`)。

### 検査結果の確認(既読)

**最終報告**になった検査結果に、依頼医あての通知(`Task`、`code = task-code|result-review`、強度は
**お知らせ**)が未確認の状態で作られます。対象は検体検査・細菌検査・病理・放射線(読影レポート)の
4 種別です。放射線はカルテにタブを持たないので、確認は読影レポートの詳細モーダルで行い、通知の
「カルテ」もそのモーダルを開きます。

- **確認の記録は来歴**(`Provenance`)が正本です。`target` がそのレポート、`agent.type = verifier`、
  `signature` は代行入力の承認と同じ ASTM E1762 の Verification Signature で、「誰がいつ読んだか」を
  残します(`fhir/provenanceHelpers.ts` の `buildReviewProvenance`)。通知は宛先と未確認・確認済みだけを
  持ちます。確認すると、来歴の作成と通知を確認済みにする PUT を 1 つの transaction で送ります。
- **確認できるのは誰でも**です(緊急異常値と同じ)。宛先は依頼医ですが、当直医や主治医が先に読むことが
  あるので限りません。記録には確認した本人が残ります。
- **緊急の通知と検査結果確認は 1 件にまとめます**。同じ結果に緊急異常値(検体検査)・重要所見(読影)の
  通知が未確認で出ている間は、検査結果確認の通知を作らず、未確認のものがあれば取り下げます。宛先が同じ
  依頼医で、2 回確認させないためです。
  - 緊急の通知を**確認すると、最終報告なら既読の来歴も同時に残します**(未確認の検査結果確認があれば
    対応済みにします)。中間報告(暫定報告)のうちは既読を残さず、後で最終報告にしたときに検査結果確認を
    改めて作ります。
  - 逆に、カルテで結果を**確認すると、未確認の緊急の通知も一緒に対応済みにします**(結果を読めば
    緊急の内容も読んだことになるため)。
  - 確認済みの後に訂正して、緊急の通知が未確認に戻れば 1 件(緊急の通知)、戻らなければ 1 件
    (検査結果確認)が出ます。
- **中間報告のうちは通知も確認もできません**。値が変わりうるものを読ませても確認にならないためです。
  確定した後に**訂正すると未確認に戻ります**(通知は同じものを書き換え、画面は確認済みの印を外して
  「確認」に戻します)。判定は確認の時刻とレポートの最終更新(`meta.lastUpdated`)の前後で行います。
- **画面**は 3 か所です。診療業務 > 通知の種別「検査結果確認」、カルテの検体採取日ペイン(未確認の
  結果に小さな印)、確認の操作(3 種別ともカルテタブの見出し行の左端。登録・編集のボタンとは性質が
  違うので間を空けます)。**確認者と確認日時は印のツールチップ**で読みます(検査項目が見える領域を
  行で減らさないためです)。
- オーダーに紐付かない結果は宛先が決まりません(緊急異常値と同じで、一覧の「自分あてのみ」を外すと出ます)。
- 通知を作るのは**この仕組みを入れた後に登録・確定した結果から**です。過去の結果に遡って通知は作りません
  (未確認として一気に溜まると、いま出た結果が埋もれるためです)。過去の結果もカルテから確認はできます。
...
- **パニック値(緊急異常値)**も同じ行に持ちます(下・上)。外れると H/L ではなく **HH / LL** と判定し、
  結果値を反転表示にしたうえで、依頼医あての通知を作ります(次の節)。
- **初期値**を `db/seed_data/lab_reference_ranges.csv` に同梱し、`db:seed` で入れます(148 行 / 128 項目)。
  パニック値は代表的な 13 件(白血球数・ヘモグロビン・血小板数・Na・K・Ca・血糖・血液ガス 3 件・
  PT-INR・フィブリノゲン)に入れてあります。
...
結果登録画面の**報告区分**で 中間報告(`preliminary`)/ 最終報告(`final`)を選びます。確定した結果を
編集して保存すると**訂正報告**(`corrected`)に変わります(遷移は保存時に決まるので、確定済みの結果を
開いたときは選択欄ではなく「訂正報告」と出ます。診療記録・病理と同じ規約です)。病理の「修正報告」が
`amended` なのに対し検体検査で `corrected` を使うのは、値を直したという意味のコードで、
JAHIS 臨床検査データ交換規約(HL7 v2 の OBX-11 = "C" 訂正)に対応するのがこちらだからです。

- `DiagnosticReport.status` と `Observation.status` の**双方に同じ値**を書きます(項目だけ中間・
  レポートだけ確定という状態を作らないためです)。
- 印は 3 か所: 内容表示の「報告区分」(中間・訂正はバッジ)、カルテのカードのバッジ、
  累積表のセルの右肩(**◦ 中間報告 / ▴ 訂正報告**。列を増やさないための印で、凡例を表の上に出します)。
- **旧値**は上流のバージョン履歴(`_history`)に残ります。内容表示で項目にチェックを付け、
  ケバブの「選択項目の変更履歴」から 版 × 値・判定・基準値・コメント の表で読めます。
  履歴の照会は 1 項目 1 リクエストなので、選んだ項目だけを引きます。
- 中間報告でも**パニック値の通知は出します**(値が危険なことは確定前でも変わりません)。

### 結果に付随する情報

- **項目ごとのコメント**(`Observation.note`): 溶血・乳び・再検などの但し書き。登録画面は行末の
  アイコンで開く 1 行の入力欄、内容表示は行の右端のアイコン(押すと吹き出しで中身が出ます)。
  入る項目が限られるので、どちらも列を増やさない形にしています。
- **総合所見**(`DiagnosticReport.conclusion`): 検査室から結果全体に添える文。登録画面は検査項目の
  下に畳んであり(「＋総合所見」で開く)、内容表示は検査項目の表の下に枠で出します。
- **実施施設・実施者**(`DiagnosticReport.performer`): 自院(管理 > 施設設定)とログインユーザーを
  保存時に焼き付けます。入力欄は出さず、内容表示で読むだけです。編集しても最初に登録した実施者が
  残ります。外注(`docs/lab-backlog.md` B-2)が入るまで実施施設は自院固定です。
- **報告日時**(`DiagnosticReport.issued`): 保存のたびに更新するので、訂正した時刻が残ります。
  採取日(`effectiveDateTime`)とは別物です。
- **測定法**(`Observation.method`): 結果項目マスタの**測定法**(`method_name`)を写します。配布の
  共有項目JLACコードマスタの測定法名称(試薬・機器の製品名)から「配布JLACマスタから引き当て」で
  入り、手でも直せます。JLAC11 の測定法コード 3 桁は単独で引ける表が無いので、コードではなく
  名称を `text` に入れています。内容表示では検査項目名のツールチップに添えます。

## 検体検査結果の取込

検査室の分析装置・LIS・外注ラボから受け取ったファイルを読み込み、手入力と同じ形で結果を登録します。
入口は**部門業務 > 臨床検査部門 > 検査結果取込**。対応形式は JAHIS 臨床検査データ交換規約
(HL7 v2.5)の **ORU^R01**(ファイル転送型の正式形)と **OUL^R22**(会話型)です。設計は
`docs/lab-result-import-design.md`。

- **文字コード**は自動で判定します(BOM → ESC シーケンス → MSH-18 の宣言 → UTF-8 として妥当か →
  CP932 の順)。判定に使った根拠を詳細画面に出すので、化けたときに原因を追えます。外れるファイルは
  取込画面で UTF-8 / Shift_JIS / ISO-2022-JP を指定できます。宣言と中身が食い違うファイルは
  取り込まずに指定を促します。
- **結果項目の引き当ては JLAC10 / JLAC11 が正本**です。完全一致 → 17 桁の先頭 12 桁
  (測定物・識別・材料)の前方一致の順で引きます。複数当たった行・当たらなかった行は**保留**に
  なり、詳細画面で結果項目を選びます。そのとき「結果項目マスタに JLAC を書き込む」(既定 ON)で
  選んだ項目の空の JLAC を埋めると、次回からは自動で当たります。取込元ごとのコード対応表は
  持たず、**結果項目マスタの JLAC を育てる**形です。このため運用前に結果項目マスタの JLAC を
  入れておくほど手間が減ります。
- **値も検証します**。コード型(CD / CO)の値が選択肢に無い行、数値型(PQ)に `<5` のような
  数値でない値が来た行は保留になり、画面でその場で選び直す / 打ち直せます。
- **候補**(HL7 の ORC・OBR 群)1 件が検査結果 1 件になります。患者とオーダーは
  ①人が選んだオーダー → ②検体ラベル番号 → ③患者番号 + 採取日 の順で決めます。ラベル番号から
  引いた管がファイルの患者と違うときは採らず、患者番号で照合し直します。採取日のオーダーが
  無ければ「オーダーに紐付けずに登録」、複数あれば画面で選びます。
- **H/L は施設の基準値で付け直します**。基準値を持たない項目だけファイルの異常フラグ(OBX-8)を
  写します。ファイルの基準範囲(OBX-7)は画面に参考として出すだけで保存しません。
- **登録**は手入力と同じ道(`useCreateLabResult` / `useUpdateLabResult`)を通るので、パニック値の
  通知・結果確認の通知・報告区分の遷移がそのまま効きます。既に結果があるオーダーに取り込むと
  既存のレポートへの追記になり、確定済みなら**訂正報告**になります。報告区分はファイルの
  OBR-25 から決めます(`P` → 中間報告、`F` / `C` → 最終報告)。
- OBX-11 が `X`(結果なし)・`D`(削除)の行と、SPM 直下の OBX(検体の状態)は取り込みません。
- 同じメッセージ ID のファイルを取り込むと詳細画面で知らせますが、取込は止めません
  (訂正版が同じ ID で来ることがあるためです)。二重登録は「既に結果があるオーダーは追記になる」
  ことで防ぎます。

## 放射線検査の読影レポート

放射線検査のオーダーに 1 件ずつ、読影レポート(`DiagnosticReport`)を付けます。設計は
`docs/rad-report-design.md`。

- **入口**は 2 つで、同じモーダルが開きます。**部門業務 > 放射線検査一覧**の実施済の行の「読影」
  (登録済みなら「読影編集」)と、カルテの放射線カードのメニュー「読影レポート登録 / 編集」。
  モーダルの上には依頼内容と実施情報(撮影日時・造影剤・被曝線量)を畳んで出します。
- **入力**は報告区分(暫定 / 最終)・所見・診断・重要所見・画像。最終報告は診断が必須です。
  確定済みのレポートを直すと**訂正報告**(`amended`)になり、保存ボタンが「訂正報告として更新」に変わります。
  所見・診断はテンプレートからも記入でき、撮影項目マスタの「既定のテンプレート > 読影所見」で
  所見の既定を決められます。読影医(`resultsInterpreter`)と発行施設(`performer`)は保存時に
  ログイン中の医療従事者と自院を焼き付けます。
- **画像**はいくつでも添付でき、ファイル選択(複数可)・画像欄へのドラッグ&ドロップ・貼り付けの
  3 通りで足します。CT・MR の濃淡画像なので長辺 1600px の JPEG にします。1 枚ずつキャプションを付け、
  上下で並べ替えられます。サムネイルを押すと拡大表示(前後の送り、描き込みと元画像の切り替え)。
- **描き込み**は元画像を書き換えず、合成画像を別の `Binary` に持ちます
  (`extension[rad-report-image]` の `source` と `annotated`)。「描き足す」「元画像から描き直す」
  「描き込みを外す」ができます。暗い画像の上で見えるよう、色は黄・赤・水色・白です。
- **1 回の保存で新しく送る画像の量に上限**があります(base64 で 7MB)。上流が本文 10MB で打ち切るためで、
  超えたら「いったん保存してから追加してください」と出ます。保存済みの画像は再送しないので、分けて
  保存すれば枚数の制限にはなりません。
- **重要所見**をチェックして要点を書くと、依頼医あてに**アラート**の通知を出します(暫定報告でも出します)。
  要点を書き換えると未確認に戻り、チェックを外すと未確認の通知は取り下げます。重要所見が未確認の間は
  検査結果確認の通知を出さず、重要所見を確認すると最終報告なら既読も残ります(「検査結果の確認(既読)」)。
- **カルテ**の放射線カードには暫定・訂正のバッジを出し、メニューの「読影レポート表示」で内容を開きます。
- 読影レポートのあるオーダーは、**実施の取消とオーダーの削除ができません**(レポートの根拠が消えるため)。
  先にレポートを削除します。レポートを削除すると、所見・テンプレートの記入内容を消し、未確認の通知を
  取り下げます。画像の `Binary` は消しません(旧版が参照するため)。
...
疾患・術式ごとの入院診療の標準計画(クリニカルパス)を施設共通のマスタとして登録します。基本仕様は JAMI・JSCP の
ePath(ePath R4 実装ガイド)の概念に準じます。患者への適用と日次評価(カルテの「パス」タブ)まで実装済みで、
ePath 形式の入出力が次の段階です。詳細は `docs/clinical-pathway-design.md`。

- **入口**: 「マスタメンテ > クリニカルパス > パス定義」(`/pathways`)。一覧から行を選ぶと 1 パス 1 ページの編集画面(`/pathways/:id`)。
- **持ち方**: backend の `master_pathways` と子テーブル(対象病名・病日・OAT ユニット・観察項目・タスク)。レジメンと同じ承認制
  (下書き / 承認済 / 廃止、承認日・承認者、承認済は凍結して複製で改訂)で、診療科と対象病名(病名マスタ)を分類属性に持ちます。
- **病日と OAT ユニット**: 入院日を 1、入院前日を -1 とする病日ごとに、OAT ユニット(1 つのアウトカム + それを判定する観察項目 +
  実施するタスク)を並べます。アウトカム・観察項目は BOM(Basic Outcome Master®)または施設ローカルのコードを任意で持ち、
  タスクは ePath のタスク分類(治療 / 検査 / 食事 / 観察項目 / ケア項目 / 教育・指導・説明 / 活動・安静度 / 医療文書と中分類)を持ちます。
  OAT ユニット・観察項目・タスクの識別子(uuid)は適用後まで持ち越すもので、編集で行を作り直しても変わりません。
- **続きと分割**: 同じ識別子のアウトカム・観察項目・タスクを複数の病日に置くと「続き」になり、パスシートでは 1 行に並びます。
  概要表のセルを押して続きを足す・外すことができ、名前や雛形は続き全体で揃います(適正値は日ごと)。続く病日の看護指示は
  1 件のオーダーにまとまり、食事・安静度は次の食事・安静度の直前で終わります。手術当日などは「この日を分割」でステップ(術前 / 術後)に分けられます。
- **パスタブ**: 適用したパスは「日めくり」(1 病日ずつ達成状態・実績・タスクの実施をまとめて記録)と「オーバービュー」(紙のパスシートと同じ格子。既定)で見ます。
  オーバービューの見出し帯に未評価・バリアンス・未実施の件数を出し、押すとその行に絞ります。看護観察に結んだ観察項目には、その日の経過表の値を候補として出します。
  予定外の追加・日程の変更・終了・適用の取り消しは見出し帯のケバブから行います。
- **指示簿のパスのタスク**: 病棟の指示簿の看護指示の表の下に、その病棟の患者の、その日の病日に置かれたオーダーを持たないタスク
  (観察・説明・文書など)を患者ごとに出し、チェックで実施にできます(未指示受けのビューでは出しません)。
- **入院患者一覧・患者ヘッダのパス**: 入院患者一覧の「パス」列と患者ヘッダに、進行中のパス名を出します。パス名からパスタブを開きます。
- **バリアンスの通知**: 重要アウトカム(★)を「未達成」と記録すると、入院の主治医あてに「パスのバリアンス」の通知(強度は注意)が
  届きます。通知の「カルテ」からパスタブのその病日の日めくりを開きます。達成・未評価に直すと未対応の通知は取り下げます。
- **カルテのパス評価**: 記載(S/O/A/P・自由記載・コメント)のあるアウトカムの評価は、カルテのタイムラインに「パス評価」のカードとして
  記録日時の日に出ます。本文にパス名と病日、メニューの「パスで開く」でパスタブのその病日の日めくりを開きます。
- **オーダー雛形**: タスクにはオーダーセットのエントリと同じ形(種別 + フォーム値)の雛形を任意で付けられます。既存のオーダー登録
  フォーム(オーダーセットと同じ 16 種。看護指示・食事・手術・輸血なども含む)をモーダルで開いて内容を決め、適用時に入院日 + (病日 − 1) の
  日付で展開する材料になります。種別の既定はタスク分類から決まります(ケア項目 → 看護指示、食事 → 食事オーダー など)。
  雛形の無いタスクはチェックリスト項目です。
- **概要表**: 編集画面の末尾に、列 = 病日、行 = アウトカムと大分類ごとのタスクの読み取り専用の表を出します。
- **フェーズと分岐**: 病日をフェーズ(連続する病日のまとまり)に分け、フェーズごとに「次に進めるフェーズ」の候補と選ぶ目安を登録できます
  (「＋ フェーズ」、フェーズ見出しのメニュー「分岐を編集」)。患者への適用はフェーズ単位で、最初は開始フェーズだけを登録し、
  パスタブの見出しのアイコン「次のフェーズを適用」で候補から選んで進めます(開始日は前のフェーズの翌日が既定)。選び間違えたらケバブの「フェーズの取り消し」。
  フェーズが 1 つのパスは今まで通り一括で適用されます。サンプルは 900005(市中肺炎・3 日目に効果判定で分岐)。
- **API**: `GET/POST /master/pathways`、`GET/PATCH/DELETE /master/pathways/:id`(id でもパスコードでも可)、`POST /master/pathways/:id/copy`。
  本体と子を 1 リクエストで読み書きし、子は送った種別だけ配列で丸ごと置換します。
- **サンプル**: `db/seed_data/pathways/*.json` の 4 件(900001 腹腔鏡下胆嚢摘出術 4 泊 5 日、900002 内視鏡的大腸ポリープ切除術 1 泊 2 日、
  900003 鼠径ヘルニア根治術 2 泊 3 日、900004 市中肺炎 7 日間)を `db:seed` で下書きとして投入します。900001 は
  タスク 53 件のうち 32 件に 13 種別のオーダー雛形(処方・注射・検体検査・放射線検査・生理検査・病理検査・処置・手術・輸血・
  食事・看護指示・他科依頼・栄養指導)が入っているので、承認して患者に適用すると病日ごとの日付でオーダーがまとめて立ちます。
...
患者ごとに任意のファイル(他院の紹介状・同意書・持参の検査結果・説明書のスキャンなど)を取り込みます。
設計の経緯は `docs/patient-file-design.md`。

- **保存先は上流の `DocumentReference` + `Binary`**。1 ファイルにつき本体を `Binary`、診療日・カテゴリ・
  表示名を `DocumentReference` に持ち、**1 本の transaction Bundle** で保存します(`fhir/patientFileHelpers.ts`。
  Bundle 内の参照は `urn:uuid` のプレースホルダで、上流が実 ID に書き換えるため孤児 `Binary` が生まれません)。
- **取込**は「取込」ボタンから。ファイル選択(複数可)とドラッグ&ドロップの 2 経路で積み、行ごとに表示名を
  直せます。診療日とカテゴリはフォーム 1 つぶんだけ持ちます(同じ日に受け取った同じ分類の書類をまとめて
  入れる使い方が中心のため)。
- **1 ファイルの上限は 7MB**。上流が本文 10MB で 413 を返し、base64 で約 4/3 倍になるためです。画像は
  `normalizeImageFile` で長辺 1600px に縮小してから数え、それ以外は無加工です。**複数ファイルは 1 ファイル
  1 Bundle で順に送る**ので、1 件が弾かれても他は残ります(「N 件を登録しました。◯◯は…」と返し、
  失敗したファイルだけがフォームに残ります)。
- **診療日は `DocumentReference.date`** に、その日の 0 時をローカルのオフセット付き(`2026-09-19T00:00:00+09:00`)で
  入れます。上流が索引している日付が `date` だけで、一覧の並び(`_sort=-date`)と期間検索を診療日で行うためです。
  登録・更新の時刻は `meta.lastUpdated` が持ちます。
- **カテゴリ**は `DocumentReference.category` に `http://fhir-client.local/CodeSystem/file-category` の code で
  入れ、`display` にカテゴリ名も焼き込みます(カテゴリを消しても取り込み済みのファイルの表示名は変わりません)。
  絞り込みは上流の `category` 検索で行うので、ページングと両立します。
- **カテゴリの管理**は「ファイル」タブの「カテゴリ管理」ボタンから、追加・改名・並べ替え・削除を行います
  (backend DB の `file_categories`。テンプレートカテゴリと同じ作りで、`code` は UUID を自動採番)。
- **一覧はどちらの見せ方でも診療日で区切ります**(見出しはグループの上端に貼り付きます)。カルテの
  タイムラインと同じ読み方に揃えるためで、表からは重複する診療日の列を落としています。日付を持たない
  ファイルは末尾に回します。
- **一覧は表とサムネイルを切り替え**られます(ヘッダのアイコン。選んだ側は localStorage に残します)。
  サムネイルで中身を読むのは画像だけで、PDF や Office は MIME ごとのアイコン(書類・表・スライド・
  書庫・音声・動画)を出します(1 ページ目を描くには別のレンダラが要るため)。
- **表示**は種類で出し分けます(画像は `<img>`、PDF は `<object>`、テキストは整形表示、それ以外は
  ダウンロードのみ)。中身は `Accept: application/octet-stream` を付けた `GET /fhir/Binary/<id>` で blob として
  取り、object URL にして描きます(`hooks/useObjectUrl.ts`)。
- **編集**できるのは表示名・診療日・カテゴリだけです。ファイル本体の差し替えはしません(消して入れ直します)。
  **削除は `DocumentReference` だけ**で、`Binary` は消しません(旧バージョンが参照しているため)。
- **タイムラインには出しません**。診療の経過そのものではなく「患者に付いている書類の束」なので、
  予約・指示簿と同じくタブでのみ見ます。
...
内服の与薬の時刻・処方区分の初期値・文書作成の督促・水分出納の対象項目・他科依頼の
依頼先ごとの既定テンプレート）は、**`settings`
...
レセコンをはじめとする外部システムの設定は、管理 > 外部システム連携の一覧に並びます。
システムを選ぶと**そのシステムに応じた設定ページ**が開き、**有効/無効はシステム単位**です。

どんなシステムがあり、設定ページに何を出すかは backend の定義
(`Integrations::ExternalSystems`)が持ちます。画面はその宣言を見て描くだけなので、
特定の製品やシステムの語彙を持ちません。

| 定義が宣言するもの | 意味 |
|---|---|
| `sections: :connection` | 接続先 URL・ユーザー・パスワードと接続テスト(API で繋ぐシステム) |
| `sections: :inbound_token` | 外部から通知を受ける Bearer(院内エージェントに配る) |
| `sections: :code_mappings` | カルテ ↔ 外部システムのコード対応表 |
| `adapters` | 製品(`system_type`)の選択肢。製品固有の設定項目はアダプタが宣言する |
| `fields` | 定義自身が宣言する設定項目。**API で繋がないシステム**はこれだけを持つ |

設定は `external_system_connections` にシステム(`system_key`)ごとの 1 行として入ります。
接続先・ユーザー・パスワード・受信トークンは決まった列に、それ以外の項目は `options`
(jsonb)に入るので、システムを足すときにテーブルは変わりません。パスワードと受信トークンは
暗号化して保持し、パスワードはブラウザへ返しません(受信トークンは配る必要があるので
再発行時にだけ平文で返ります)。

コード対応表(`external_code_mappings`)は**製品(`system_type`)のコード体系に対する表**
なので、同じ製品と繋ぐシステムは同じ表を見ます。

システムを足すときは定義に 1 要素加えます。API で繋ぐなら、あわせてアダプタを 1 つ書いて
`adapters` に登録します。

## レセコン連携

レセコンは「外部システム連携」のひとつとして繋ぎます。カルテ側のコードに製品名は
出てこず、製品ごとの電文や語彙は backend のアダプタ(`Integrations::Orca` など)に
閉じています。他社レセコンを足すときはアダプタを 1 つ書き、定義の `adapters` に
登録します。詳細は `docs/receipt-computer-integration.md`。

向きは次のとおりです。

| | 向き | 中身 |
|---|---|---|
| 患者基本情報 | レセコン → カルテ | 氏名・生年月日・性別・住所・連絡先 |
| 保険・公費 | レセコン → カルテ | FHIR の `Coverage`(JP_Coverage)として保持 |
| 受付 | レセコン → カルテ | 当日受付と同じ形の `Appointment` |
| 病名・診療行為(会計) | カルテ → レセコン | レセプト電算コードで送る |

- **患者・保険・受付はレセコンが正本**です。カルテから登録しません。
  保険は取り込んだ `Coverage` を読むだけで、レセコンを画面から直接見に行くことはありません。
- 「同時に使う保険の組」はレセコンが採番した**不透明なキー**として `Coverage.class` に
  持ちます。カルテは中身を解釈せず、会計送信のときにそのまま返します。
- CLAIM は 2026 年 3 月末で廃止済みのため、連携は API(HTTP/XML)と PUSH 通知で行います。

### 通知の受け取り

レセコンの変更は PUSH 通知(WebSocket)で届きますが、**backend は WebSocket も
製品ごとのイベント形式も知りません**。院内に常駐するエージェント
(別リポジトリ `fhir-client-agent`、Go 製の単一バイナリ)が通知を中立なイベントに直し、
`POST /integrations/receipt/events` へ HTTP で渡します。

通知は「患者番号が変わった」としか言わない(保険の中身は入らない)ので、
カルテは必ずレセコンの API で取り直します。**未接続中の通知は再配達されない**ため、
取りこぼしは次のどちらかで回復します。

- カルテ > 患者情報 > 保険・公費 の「医事会計から再取込」(患者 1 人)
- `bin/rails integrations:receipt:resync`(取り込み済みの患者を洗い直す)

### 画面

連携が無効な環境では連携 UI は出ません。

| 画面 | できること |
|---|---|
| 管理 > 外部システム連携 > 医事会計 | 接続先・資格情報・受信トークン。接続テストで医療機関名と保険医療機関番号を確認できる |
| 管理 > 外部システム連携 > 医事会計 > コード変換設定 | 診療科・医師のコード対応付け(種別はアダプタが宣言する) |
| カルテ > 患者情報 > 保険・公費 | 取り込んだ保険・公費の参照と再取込 |
| 外来一覧 > 行メニュー > 医事会計へ会計送信 | 送る内容を確認してから病名と診療行為を送る。送り直し・取消も同じ場所 |

送る診療行為は、実施入力を持つ種別(放射線・生理・内視鏡・処置・手術)では実施記録
(Procedure とその配下の薬剤・材料・コメント)から組み、実施記録の無い当日のオーダーは
「未実施」として送りません。実施入力を持たない種別(検体検査・細菌検査)と処方はオーダーの
実施(予定)日で集めます。剤の区分は診療行為マスタの章から決めるので、実施入力で足した
加算・薬剤・材料はそのまま同じ剤に載ります(設計は `docs/receipt-billing-design.md`)。
送信は同期で行い(ジョブ基盤は使いません)、記録は
`log/integrations/receipt_computer.log` に JSON Lines で残ります。送信済みかどうかは
レセコンに訊くので、カルテ側に控えは持ちません。

**レセプト電算コードが設定されていない項目は送りません。** 黙って落とさず、
送信前のプレビューに「送れない項目」として理由つきで出します。

検証環境は別リポジトリ `weborca-dev` にあります(WebORCA オンプレ版を Docker で起動)。
無償ソース利用では改定パッチもマスタ更新も取得できないため、**点数計算の正確性は検証対象外**で、
電文の形と往復の確認に使います。エージェントごと通しで動かす手順は
[任意の profile(院内エージェント / 日レセ)](#任意の-profile院内エージェント--日レセ)を参照。
...
- カルテのファイル取込: 表・サムネイルとも診療日の見出しで区切られること、表とサムネイルの切り替え(選んだ側がリロード後も残ること、画像だけサムネイルが出て PDF・Word・Excel・CSV・PowerPoint・ZIP・テキストはそれぞれ別のアイコンになること)、カテゴリの追加・並べ替え、PDF・PNG・テキストの一括登録(診療日とカテゴリの一括設定)、一覧が診療日の降順で並ぶこと、カテゴリでの絞り込み(上流の `category` 検索)、PDF・画像・テキストのプレビューとダウンロード、表示名・診療日・カテゴリの編集で並びが変わること、削除、7MB 超のファイルが取込時に弾かれて他のファイルは残ること、ドロップでの追加と枠の強調を Docker 環境の画面と上流 REST で確認
