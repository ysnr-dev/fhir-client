...
- **経過表(フローシート)**: POMR 5 要素の 1 つで、「経過表」タブに出します
  ...
  - **横軸は基準日から 1 週間**(左が古く、右端が基準日。紙の温度板と同じ向き)。基準日は日付欄と
    「◀ ▶」(週送り)・「今日」で動かします。日付の見出しは `MM/DD(曜)` で、土曜は青・日曜は赤、
    基準日は下線です。**測定の無い日も 1 列置く**ので、空いている日が見えます。
  - **1 日の中は測定 1 回ごとに列**が分かれます(同じ日の朝夕を 1 列に潰すと読めないため)。
    日付の下に時刻を出します。列幅はパネルの幅を列数で割って使い切り、列が多い週だけ横に送ります。
  ...
  - 取得は期間(基準日の 6 日前 〜 基準日)で `date=ge/le` を付けて引き、ページングで取り切ります。
  - **読み取り専用**です。編集はカルテのバイタルカードから行います(編集の導線を 2 つ持つと
    同期の負債になるため)。
  - グラフは温度板にならって表の最上段に常設し、体温・血圧・脈拍・呼吸数を表の列位置に揃えて
    描きます(`FlowsheetChart`)。**血圧は収縮期 ▼・拡張期 ▲ の 2 系列**に分けます。
  - **異常値の強調**: 施設設定「バイタルの異常値」のしきい値(収縮期・拡張期・体温・脈拍・SpO2・
    呼吸数の下限・上限)で、上限以上を H(赤)・下限以下を L(青)として表のセルとグラフの点を
    色付けします。判定は表示時に行い `Observation.interpretation` には書かないので、しきい値を
    変えれば過去の測定にもそのまま効きます(`vitalInterpretationOf`)。
  - **見出しの固定**: 縦に送っても年・日付・時刻の見出しが、横に送っても測定項目・単位の列が
    残ります(表のラップだけを送り、`thead` と左 2 列を sticky にしています)。
  - **イベントの帯**: グラフの真上に、入院・退院・転棟・外出泊・帰院(`Encounter`)と手術の実施
    (ハブ `Procedure`)を ▼ と短いラベルで出し、グラフに破線の縦線を落とします
    (`fhir/flowsheetEventHelpers.ts`)。印は**その日の列のまとまりの中央**に置きます(1 日の中の
    位置は測定の並びで決まり時間に比例しないため)。同じ日のイベントは縦に積み、3 件を超えたら
    「他N件」に畳みます。帯を選ぶとその日のイベント一覧をモーダルで出し、手術は「詳細」から
    **カルテと同じオーダー詳細モーダル**(`detail=` パラメータ)を開きます。
  - **注射の欄**: 測定項目の下に、薬剤の組(RP)ごとの行を出します(`fhir/flowsheetInjectionHelpers.ts`)。
    ...
    - 印を選ぶと、その日の注射の予定・実施を一覧で出します。同じ日に施用が複数あると
      一覧は同じ内容になるので、押した 1 件は見出しの時刻と行の色で示します。
      「詳細」から注射内容モーダルへ、「実施入力」からカルテと同じ実施入力モーダルへ飛べます
      (施用するのは病棟なので、経過表からその場で書けます。中止した注射には出しません)。
      同じ日に重なる印は少しずらして、本数が見えるようにしています。
  - **検査の欄**: 注射の下に、放射線・内視鏡・生理検査の種別ごとの行を出し、オーダー 1 件を
    印 1 つで置きます。検査名はヘッダの `ServiceRequest` ではなく明細にあるので、`_revinclude:iterate`
    で一緒に取って一覧の「内容」に出します。「詳細」から各検査のオーダー詳細モーダルへ飛べます。
    **検体・細菌・病理検査は出しません**。患者が動かないうえ、毎日の採血で行が埋まります
    (時系列は検体検査時系列タブに表があります)。
  - **病日・術後日数**: 時刻の見出しの下に日ごとに出します。病日は入院日を 1 日目とし、入院前・
    退院後の日は空です。術後は手術当日を「当日」とし、90 日を超えたら出しません(既往になるため)。
    どちらも該当する記録が無ければ行ごと出ません。
