...
- **同じグループで医療機関が選ばれていれば、その医療機関に所属する医療従事者だけを表示**します
  （`PractitionerRole?organization=…&_include=PractitionerRole:practitioner`）。モーダル上部に
  絞り込み中の医療機関名が出て、「すべて表示」で解除できます。保存済み回答を開き直した直後は
  医療機関の id が分からないため、医療機関名の項目の値から名称の完全一致で引き当てます
  （同名が複数あるときは絞り込みません）。
- モーダルでは **職種で絞り込み**できます。テンプレート側の
  **「職種の初期値」**（`…/questionnaire-practitioner-role-default`）を設定しておくと、その職種が
  フィルタの初期値になります（例: 担当医師名の欄なら「医師」）。
...
- 医療従事者のテンプレートへの一括入力: 職種の初期値がモーダルのフィルタに反映されること、同じグループで選択済みの医療機関に所属する医療従事者だけが表示されること(「すべて表示」での解除も)、選択した医療従事者の氏名が項目に入ることを画面から確認
