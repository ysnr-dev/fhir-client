...
- **左ペイン(診療日 / 種別)**: 左端のペインは 2 つの探し方を切り替えて使います
  ...
  - **診療日**: データが存在する診療日をツリー表示し(既定は閉じた状態)、クリックでタイムラインの
    該当位置までスクロールします。
  - **種別**: 情報の種別(診療記録・処方・注射・検体検査・細菌検査・放射線検査・生理検査・内視鏡・テンプレート)を
...
部門業務の各種一覧（検体検査・放射線検査・生理検査・内視鏡・処方）は「入院病棟」の列とフィルタを持ちます。値は
...
- 書き込み口はオーダー 7 種別（処方・注射・検体検査・放射線検査・生理検査・内視鏡・細菌検査）が共通で通る
