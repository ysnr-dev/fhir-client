...
- **診療記録の「治療計画(P)」セクションに『テンプレート記載』で貼り付けて使います**。初期計画は
  「どのプロブレムに対する計画か」が要で、診療記録には対象プロブレムの紐付けがあるためです
  （単独登録のテンプレート回答はプロブレムの紐付けが未実装で、絞り込みにも乗りません）。
  プロブレムを選んでから「診療記録」を開けば対象が初期選択されるので、その状態で貼れば
  計画もそのプロブレムのものになります。
