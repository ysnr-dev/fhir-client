...
RSpec.describe "Master::JlacItems", type: :request do
  def valid_attrs(overrides = {})
    { jlac11_code: "C9999999999999999", fhir_item_name: "テスト検査項目" }.merge(overrides)
  end

  describe "POST /master/jlac_items/import" do
    it "imports the uploaded file" do
      file = fixture_file_upload("jlac_items_sample.csv", "text/csv")

      post "/master/jlac_items/import", params: { file: file }
    ...
    it "returns 422 when file is missing" do
      post "/master/jlac_items/import", params: {}

      expect(response).to have_http_status(:unprocessable_content)
    end
  end

  describe "GET /master/jlac_items (検索)" do
    before do
      Master::JlacItem.create!(
    ...
    def names_for(params)
      get "/master/jlac_items", params: params
  ...
  describe "GET /master/jlac_items (並び順)" do
    it "display_order の数値順に並べる(桁数が揃っていない値も正しく並ぶ)" do
      Master::JlacItem.create!(jlac11_code: "C1", fhir_item_name: "三番目", display_order: "1000")
      Master::JlacItem.create!(jlac11_code: "C2", fhir_item_name: "一番目", display_order: "100")
      Master::JlacItem.create!(jlac11_code: "C3", fhir_item_name: "二番目", display_order: "200")

      get "/master/jlac_items"
    ...
    it "display_order が同値なら収載順(id)で並べる" do
      Master::JlacItem.create!(jlac11_code: "C1", fhir_item_name: "先", display_order: "100")
      Master::JlacItem.create!(jlac11_code: "C2", fhir_item_name: "後", display_order: "100")
  ...
  describe "GET /master/jlac_items/filter_options" do
    before do
      Master::JlacItem.create!(
      ...
      Master::JlacItem.create!(jlac11_code: "C5", category_name: "")
    end

    def options_for(params = {})
      get "/master/jlac_items/filter_options", params: params
  ...
  describe "CRUD" do
    it "creates, reads, updates, lists, and deletes a record" do
      post "/master/jlac_items", params: valid_attrs, as: :json
      expect(response).to have_http_status(:created)
      id = JSON.parse(response.body)["id"]

      get "/master/jlac_items/#{id}"
      expect(JSON.parse(response.body)["fhir_item_name"]).to eq("テスト検査項目")

      patch "/master/jlac_items/#{id}", params: { fhir_item_name: "更新後" }, as: :json
      expect(JSON.parse(response.body)["fhir_item_name"]).to eq("更新後")

      get "/master/jlac_items", params: { name: "更新後" }
      expect(JSON.parse(response.body)["total"]).to eq(1)

      delete "/master/jlac_items/#{id}"
      expect(response).to have_http_status(:no_content)
    end

    it "returns 422 when jlac11_code is missing" do
      post "/master/jlac_items", params: { fhir_item_name: "無効" }, as: :json
