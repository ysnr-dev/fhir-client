...
RSpec.describe "Master::PhysioDatasetDetails", type: :request do
  def body
  ...
  before do
    ...
    Master::PhysioDataset.create!(dataset_code: "000002", name: "気道可逆性試験セット")
  ...
  describe "GET /master/physio_dataset_details" do
    before do
      ...
      Master::PhysioDatasetDetail.create!(dataset_code: "000002", detail_type: "material",
                                          code: "710010004", default_quantity: 1, display_order: 1)
    end

    it "データセットコードをカンマ区切りで複数指定して一括で引ける" do
      get "/master/physio_dataset_details", params: { dataset_code: "000001,000002" }
    ...
    it "器材は特定保険医療材料マスタから直接引く(施設内の器材マスタを挟まない)" do
      get "/master/physio_dataset_details", params: { dataset_code: "000002" }
    ...
    it "種別で絞り込める" do
      get "/master/physio_dataset_details",
    ...
    it "参照先マスタが未取込でも明細は返る(名称は空)" do
      ...
      get "/master/physio_dataset_details", params: { dataset_code: "000001", detail_type: "procedure" }
  ...
  describe "POST /master/physio_dataset_details" do
    it "表示順を省略すると追加順に採番する" do
      post "/master/physio_dataset_details", params: { dataset_code: "000001", detail_type: "procedure",
                                                       code: "160067510" }
      expect(body["display_order"]).to eq(1)

      post "/master/physio_dataset_details", params: { dataset_code: "000001", detail_type: "medicine",
    ...
    it "使うこともある明細は初期選択を外して登録できる" do
      post "/master/physio_dataset_details", params: { dataset_code: "000001", detail_type: "material",
    ...
    it "別のデータセットには同じ明細を登録できる" do
      ...
      post "/master/physio_dataset_details", params: { dataset_code: "000002", detail_type: "medicine",
                                                       code: "620000237" }
    ...
    it "未知の種別は登録できない" do
      post "/master/physio_dataset_details", params: { dataset_code: "000001", detail_type: "device",
                                                       code: "710010004" }
  ...
  describe "PATCH /master/physio_dataset_details/:id" do
    it "既定数量と経路を変更できる" do
      record = Master::PhysioDatasetDetail.create!(dataset_code: "000001", detail_type: "medicine",
                                                   code: "620000237", default_quantity: 2)

      patch "/master/physio_dataset_details/#{record.id}", params: { default_quantity: "1",
                                                                     route_code: "IV" }
    ...
    it "初期選択を切り替えられる" do
      ...
      patch "/master/physio_dataset_details/#{record.id}", params: { default_selected: "false" }

      expect(record.reload.default_selected).to be(false)
    end
  end

  describe "DELETE /master/physio_dataset_details/:id" do
    it "削除できる" do
      record = Master::PhysioDatasetDetail.create!(dataset_code: "000001", detail_type: "medicine",
                                                   code: "620000237")

      delete "/master/physio_dataset_details/#{record.id}"

      expect(response).to have_http_status(:no_content)
      expect(Master::PhysioDatasetDetail.count).to eq(0)
    end
  end
end
