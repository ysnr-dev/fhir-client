...
RSpec.describe "Master::EndoscopyItems", type: :request do
  def body
    JSON.parse(response.body)
  end

  def create_item(code, overrides = {})
    Master::EndoscopyItem.create!({ item_code: code, name: "項目#{code}" }.merge(overrides))
  end

  describe "GET /master/endoscopy_items" do
    before do
    ...
    it "表示順で返す" do
      get "/master/endoscopy_items"
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0002 P0001 P0003])
    end

    it "コードのカンマ区切りで一括取得できる" do
      get "/master/endoscopy_items", params: { item_code: "P0001,P0003" }
      expect(body["items"].map { |i| i["item_code"] }).to match_array(%w[P0001 P0003])
    end

    it "kind・検査種別で絞り込める" do
      get "/master/endoscopy_items", params: { kind: "set" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0002])

      get "/master/endoscopy_items", params: { exam_type_code: "01" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0001])
    end

    it "オーダー単位(グループ化/単独)で絞り込める" do
      create_item("P0006", name: "大腸内視鏡", groupable: false, display_order: 40)

      get "/master/endoscopy_items", params: { groupable: "false" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0006])

      get "/master/endoscopy_items", params: { groupable: "true" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0002 P0001 P0003])
    end

    it "active=true は有効期間内の項目だけ返す" do
      get "/master/endoscopy_items", params: { active: "true" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0002 P0001])
    end

    it "名称・略称・カナで検索できる" do
      get "/master/endoscopy_items", params: { name: "egd" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0001])

      get "/master/endoscopy_items", params: { name: "じょうぶないしきょう" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0001])
    end

    it "keyword は名称・検査種別のどちらかに当たる項目を返す" do
      Master::EndoscopyExamType.create!(exam_type_code: "02", name: "下部消化管内視鏡", short_name: "CS")
      # 名称に「下部」が入っていない項目でも検査種別から引ける。
      create_item("P0007", name: "S状結腸内視鏡", exam_type_code: "02")

      get "/master/endoscopy_items", params: { keyword: "下部" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0007])

      get "/master/endoscopy_items", params: { keyword: "cs" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0007])

      get "/master/endoscopy_items", params: { keyword: "じょうぶないしきょう" }
      expect(body["items"].map { |i| i["item_code"] }).to eq(%w[P0001])
    end
    ...
    it "一覧に載っている検査種別の名称を添える" do
      ...
      expect(body["exam_types"]).to eq("01" => "上部消化管内視鏡")
    end

    it "レセ電算コードに対応する医科診療行為の名称を添える" do
      Master::MedicalProcedure.create!(procedure_code: "160067810", name: "胃・十二指腸ファイバースコピー")
      create_item("P0008", name: "経鼻内視鏡", receipt_code: "160067810")

      get "/master/endoscopy_items", params: { item_code: "P0008" }

      expect(body["items"].first["receipt_procedure_name"]).to eq("胃・十二指腸ファイバースコピー")
    end
  end

  describe "GET /master/endoscopy_items/:id" do
    it "検査種別の名称とセット構成を添えて返す(コードでも引ける)" do
      Master::EndoscopyExamType.create!(exam_type_code: "01", name: "上部消化管内視鏡")
      create_item("P0002", name: "胃・大腸セット", kind: "set")
      create_item("P0004", name: "上部消化管内視鏡(経口)", exam_type_code: "01")
      Master::EndoscopySetItem.create!(set_item_code: "P0002", member_item_code: "P0004", display_order: 1)

      get "/master/endoscopy_items/P0002"

      expect(body["name"]).to eq("胃・大腸セット")
      expect(body["set_items"].map { |m| m["member_name"] }).to eq(["上部消化管内視鏡(経口)"])
    ...
    it "参照している実施入力用データセットの名称を添えて返す" do
      create_item("P0004", name: "上部消化管内視鏡", dataset_code: "000001")
      Master::EndoscopyDataset.create!(dataset_code: "000001", name: "上部内視鏡標準セット")

      get "/master/endoscopy_items/P0004"

      expect(body["dataset_code"]).to eq("000001")
      expect(body["dataset_name"]).to eq("上部内視鏡標準セット")
    end
  end

  describe "POST /master/endoscopy_items" do
    it "項目コードを省略すると自動採番する" do
      create_item("000012")

      post "/master/endoscopy_items", params: { name: "自動採番の項目" }
    ...
    it "既定はグループ化で、単独オーダーの項目も登録できる" do
      post "/master/endoscopy_items", params: { item_code: "P0001", name: "既定の項目" }
      expect(body["groupable"]).to be(true)

      post "/master/endoscopy_items", params: { item_code: "P0006", name: "大腸内視鏡", groupable: false }
      expect(response).to have_http_status(:created)
    ...
    it "既定は実施入力ありで、実施入力なしの項目も登録できる" do
      ...
      post "/master/endoscopy_items", params: { item_code: "P0007", name: "とるだけの項目",
    ...
    it "既定は予約不要で、予約必須の項目は単独オーダーなら登録できる" do
      ...
      post "/master/endoscopy_items", params: { item_code: "P0008", name: "ERCP",
    ...
    it "予約必須の項目は予約枠を紐づけて登録でき、予約不要に変えると外れる" do
      post "/master/endoscopy_items", params: { item_code: "P0010", name: "ERCP",
                                             requires_appointment: true, groupable: false,
                                             appointment_schedule_id: "schedule-1" }
      expect(response).to have_http_status(:created)
      expect(body["appointment_schedule_id"]).to eq("schedule-1")

      patch "/master/endoscopy_items/P0010", params: { requires_appointment: false }
      expect(response).to have_http_status(:ok)
      expect(body["appointment_schedule_id"]).to be_nil
    end

    it "予約必須の項目はグループ化のままでは登録できない" do
      post "/master/endoscopy_items", params: { item_code: "P0009", name: "予約必須なのにグループ化",
    ...
    it "有効終了日が有効開始日より前なら登録できない" do
      post "/master/endoscopy_items", params: { item_code: "P0001", name: "期間おかしい",
  ...
  describe "PATCH /master/endoscopy_items/:id" do
    it "実施入力なしに変えるとデータセットの参照も外れる" do
      item = create_item("P0004", dataset_code: "000001")

      patch "/master/endoscopy_items/P0004", params: { requires_perform_input: false }
    ...
    it "セットの構成項目は単独オーダーにできない" do
      ...
      patch "/master/endoscopy_items/P0004", params: { groupable: false }
  ...
  describe "DELETE /master/endoscopy_items/:id" do
    it "ぶら下がるセット構成も片付ける" do
      create_item("P0002", kind: "set")
      create_item("P0004")
      Master::EndoscopySetItem.create!(set_item_code: "P0002", member_item_code: "P0004")

      delete "/master/endoscopy_items/P0002"
    ...
    it "参照していたデータセット本体は他の項目でも使うので残す" do
      ...
      delete "/master/endoscopy_items/P0004"

      expect(Master::EndoscopyItem.count).to eq(0)
      expect(Master::EndoscopyDataset.count).to eq(1)
    end
  end
end
