module Master
  # 細菌検査結果の薬剤感受性の検査方法(JANIS 測定法コード)。配布ファイル由来の
  # 標準コード(source=official)は取込で洗い替えるためこの画面では読むだけ。
  # 書けるのは施設追加分(source=local)だけ。
  class MicroSusceptibilityMethodsController < BaseController
    before_action :set_record, only: %i[update destroy]

    def index
      scope = Master::MicroSusceptibilityMethod.all
    ...
    # 画面から作れるのは施設追加コードだけ。
    def create
      record = Master::MicroSusceptibilityMethod.new(record_params)
      record.source = Master::MicroSusceptibilityMethod::LOCAL
      if record.save
    ...
    def import
      ...
      result = MasterImport::MicroSusceptibilityMethodImporter.call(params[:file])
      render json: {
    ...
    def set_record
      @record = Master::MicroSusceptibilityMethod.find(params[:id])
    end

    def record_params
      params.permit(Master::MicroSusceptibilityMethod.column_names - %w[id search_name created_at updated_at])
    end
  end
end
