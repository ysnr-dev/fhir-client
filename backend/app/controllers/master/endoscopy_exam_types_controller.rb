module Master
  ...
  class EndoscopyExamTypesController < BaseController
    ...
    def index
      scope = Master::EndoscopyExamType.all
    ...
    def create
      record = Master::EndoscopyExamType.new(record_params)
    ...
    def destroy
      ...
      Master::EndoscopyExamType.transaction do
        Master::EndoscopyItem.where(exam_type_code: code).update_all(exam_type_code: nil)
        @record.destroy!
      end
      head :no_content
    end
    ...
    def next_exam_type_code
      max = Master::EndoscopyExamType.where("exam_type_code ~ '^[0-9]+$'")
    ...
    def set_record
      ...
      @record = Master::EndoscopyExamType.find_by(exam_type_code: params[:id]) ||
                Master::EndoscopyExamType.find(params[:id])
    end
  end
end
