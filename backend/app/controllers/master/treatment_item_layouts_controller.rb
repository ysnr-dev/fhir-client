module Master
  ...
  class TreatmentItemLayoutsController < BaseController
    ...
    def index
      scope = Master::TreatmentItemLayout.order(Arel.sql("display_order NULLS LAST"))
    ...
    def update
      removed = 0
      Master::TreatmentItemLayout.transaction do
        @record.update!(record_params)
        removed = Master::TreatmentItemLayoutCell
    ...
    def destroy
      Master::TreatmentItemLayout.transaction do
        Master::TreatmentItemLayoutCell.where(layout_id: @record.id).delete_all
        @record.destroy!
      end
      head :no_content
    end
    ...
    def cells_for(layout_id)
      Master::TreatmentItemLayoutCell
        .where(layout_id: layout_id)
        .joins("LEFT JOIN master_treatment_items ON master_treatment_items.item_code = " \
               "master_treatment_item_layout_cells.item_code")
        .select(
          "master_treatment_item_layout_cells.*",
          "master_treatment_items.name AS item_name",
          "master_treatment_items.short_name AS item_short_name",
          "master_treatment_items.kind AS item_kind",
        )
