module Master
  # 処置オーダーレイアウトの1マス。レイアウト編集画面から操作する。
  class TreatmentItemLayoutCellsController < BaseController
    ...
    def update
      ...
      occupant = Master::TreatmentItemLayoutCell
        .where(layout_id: @record.layout_id, grid_row: target_row, grid_column: target_column)
        .where.not(id: @record.id)
        .first

      Master::TreatmentItemLayoutCell.transaction do
        if occupant
