module Master
  ...
  class EndoscopyItemLayoutsController < BaseController
    ...
    def index
      scope = Master::EndoscopyItemLayout.order(Arel.sql("display_order NULLS LAST"))
    ...
    def update
      removed = 0
      Master::EndoscopyItemLayout.transaction do
        @record.update!(record_params)
        removed = Master::EndoscopyItemLayoutCell
    ...
    def destroy
      Master::EndoscopyItemLayout.transaction do
        Master::EndoscopyItemLayoutCell.where(layout_id: @record.id).delete_all
        @record.destroy!
      end
      head :no_content
    end
    ...
    def cells_for(layout_id)
      Master::EndoscopyItemLayoutCell
        .where(layout_id: layout_id)
        .joins("LEFT JOIN master_endoscopy_items ON master_endoscopy_items.item_code = " \
               "master_endoscopy_item_layout_cells.item_code")
        .select(
          "master_endoscopy_item_layout_cells.*",
          "master_endoscopy_items.name AS item_name",
          "master_endoscopy_items.short_name AS item_short_name",
          "master_endoscopy_items.kind AS item_kind",
        )
