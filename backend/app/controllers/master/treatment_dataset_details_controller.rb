module Master
  ...
  class TreatmentDatasetDetailsController < BaseController
    ...
    def index
      scope = Master::TreatmentDatasetDetail.with_names
      # カンマ区切りで複数指定可(オーダーに載っている処置項目に紐付く
    ...
    def create
      record = Master::TreatmentDatasetDetail.new(record_params)
      # 追加順に並べる(明示されていれば従う)。
    ...
    def next_display_order(dataset_code)
      (Master::TreatmentDatasetDetail.where(dataset_code: dataset_code).maximum(:display_order) || 0) + 1
    end
  end
end
