module Master
  # 処置の実施入力用データセット。画面から手動で登録し、明細(手技料・薬剤・
  # 器材)は treatment_dataset_details で編集する。詳細では明細に参照先マスタの名称を
  # 添えて返すので、画面はコードだけを持てばよい。
  class TreatmentDatasetsController < BaseController
    before_action :set_record, only: %i[show update destroy]

    def index
      scope = Master::TreatmentDataset.all
      # カンマ区切りで複数指定可(処置項目に紐付くデータセットをまとめて引くため)。
    ...
    def show
      render json: @record.as_json.merge(
        details: Master::TreatmentDatasetDetail.with_names
    ...
    def create
      record = Master::TreatmentDataset.new(record_params)
    ...
    # 外部キーを張っていないので、明細と処置項目からの参照も併せて片付ける。
    def destroy
      code = @record.dataset_code
      Master::TreatmentDataset.transaction do
        Master::TreatmentDatasetDetail.where(dataset_code: code).delete_all
        Master::TreatmentItem.where(dataset_code: code).update_all(dataset_code: nil)
        @record.destroy!
      end
      head :no_content
    end
    ...
    def next_dataset_code
      max = Master::TreatmentDataset.where("dataset_code ~ '^[0-9]+$'")
    ...
    def set_record
      ...
      @record = Master::TreatmentDataset.find_by(dataset_code: params[:id]) ||
                Master::TreatmentDataset.find(params[:id])
    end
  end
end
