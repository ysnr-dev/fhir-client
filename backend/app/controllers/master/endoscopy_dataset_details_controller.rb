module Master
  ...
  class EndoscopyDatasetDetailsController < BaseController
    before_action :set_record, only: %i[update destroy]

    def index
      scope = Master::EndoscopyDatasetDetail.with_names
      # カンマ区切りで複数指定可(オーダーに載っているオーダー項目に紐付く
    ...
    def create
      record = Master::EndoscopyDatasetDetail.new(record_params)
      # 追加順に並べる(明示されていれば従う)。
    ...
    def next_display_order(dataset_code)
      (Master::EndoscopyDatasetDetail.where(dataset_code: dataset_code).maximum(:display_order) || 0) + 1
    end
  end
end
