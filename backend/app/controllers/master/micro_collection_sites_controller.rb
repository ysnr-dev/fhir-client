module Master
  ...
  class MicroCollectionSitesController < BaseController
    ...
    def create
      record = Master::MicroCollectionSite.new(record_params)
      if record.save
    ...
    def set_record
      @record = Master::MicroCollectionSite.find(params[:id])
    end

    def record_params
      params.permit(Master::MicroCollectionSite.column_names - %w[id created_at updated_at])
    end
  end
end
