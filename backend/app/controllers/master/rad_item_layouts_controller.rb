module Master
  ...
  class RadItemLayoutsController < BaseController
    ...
    def index
      scope = Master::RadItemLayout.order(Arel.sql("display_order NULLS LAST"))
    ...
    def update
      ...
      Master::RadItemLayout.transaction do
        ...
        removed = Master::RadItemLayoutCell
    ...
    def destroy
      Master::RadItemLayout.transaction do
        Master::RadItemLayoutCell.where(layout_id: @record.id).delete_all
        @record.destroy!
      end
      head :no_content
    end
    ...
    def cells_for(layout_id)
      Master::RadItemLayoutCell
        .where(layout_id: layout_id)
        .joins("LEFT JOIN master_rad_items ON master_rad_items.item_code = " \
               "master_rad_item_layout_cells.item_code")
        .select(
          "master_rad_item_layout_cells.*",
          "master_rad_items.name AS item_name",
          "master_rad_items.short_name AS item_short_name",
