module Master
  # 内視鏡オーダーレイアウトの1マス。レイアウト編集画面から操作する。
  class EndoscopyItemLayoutCellsController < BaseController
    ...
    def update
      ...
      occupant = Master::EndoscopyItemLayoutCell
        .where(layout_id: @record.layout_id, grid_row: target_row, grid_column: target_column)
        .where.not(id: @record.id)
        .first

      Master::EndoscopyItemLayoutCell.transaction do
        if occupant
