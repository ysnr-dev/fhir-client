module Master
  ...
  class LabOrderItemLayoutsController < BaseController
    ...
    def create
      record = Master::LabOrderItemLayout.new(record_params)
      if record.save
    ...
    def set_record
      @record = Master::LabOrderItemLayout.find(params[:id])
    end

    def record_params
      params.permit(Master::LabOrderItemLayout.column_names - %w[id created_at updated_at])
    end
  end
end
