module Master
  # 内視鏡オーダー項目のメンテナンス。画面から手動で登録する。
  # 放射線の RadItemsController から JJ1017 の要素解決と頻用コードからの一括作成を
  # 落とし、代わりに検査種別(master_endoscopy_exam_types)の名称を添える。
  class EndoscopyItemsController < BaseController
    before_action :set_record, only: %i[show update destroy]

    def index
      scope = Master::EndoscopyItem.all
    ...
    def show
      ...
      # 項目そのものも含める。
      members = Master::EndoscopyItem.where(item_code: set_items.map(&:member_item_code)).to_a
    ...
    def create
      record = Master::EndoscopyItem.new(record_params)
    ...
    def destroy
      ...
      Master::EndoscopyItem.transaction do
        Master::EndoscopySetItem.where(set_item_code: code).delete_all
        Master::EndoscopySetItem.where(member_item_code: code).delete_all
        @record.destroy!
      end
      head :no_content
    end
    ...
    ITEM_SEARCH_COLUMNS = %w[
      master_endoscopy_items.search_name
      master_endoscopy_items.search_short_name
      master_endoscopy_items.search_kana
    ].freeze

    # 名称・略称・カナに加えて、検査種別の名称にも当てる(「上部」で経鼻内視鏡を
    ...
    def keyword_match(scope, query)
      ...
      Master::SearchNormalizer.tokenize(query).each do |token|
        ...
        clauses = %w[search_name search_short_name search_kana].map do |column|
          "master_endoscopy_items.#{column} LIKE #{pattern}"
        end
        clauses << "master_endoscopy_items.exam_type_code IN " \
                   "(SELECT exam_type_code FROM master_endoscopy_exam_types " \
    ...
    def with_receipt_name(scope)
      scope
        .joins("LEFT JOIN master_medical_procedures " \
               "ON master_medical_procedures.procedure_code = master_endoscopy_items.receipt_code")
        .select("master_endoscopy_items.*",
    ...
    def exam_type_names_for(items)
      ...
      Master::EndoscopyExamType.where(exam_type_code: codes).pluck(:exam_type_code, :name).to_h
    end

    def set_items_for(code)
      Master::EndoscopySetItem
        .where(set_item_code: code)
        .joins("LEFT JOIN master_endoscopy_items " \
               "ON master_endoscopy_items.item_code = master_endoscopy_set_items.member_item_code")
        .select(
          "master_endoscopy_set_items.*",
          "master_endoscopy_items.name AS member_name",
          "master_endoscopy_items.short_name AS member_short_name",
          "master_endoscopy_items.exam_type_code AS member_exam_type_code",
        )
        .order(Arel.sql("master_endoscopy_set_items.display_order NULLS LAST"))
        .order(:id)
    end
    ...
    def dataset_name_for(code)
      ...
      Master::EndoscopyDataset.where(dataset_code: code).pick(:name)
    end
    ...
    def next_item_code
      max = Master::EndoscopyItem.where("item_code ~ '^[0-9]+$'").maximum(Arel.sql("item_code::bigint"))
      ((max || 0) + 1).to_s.rjust(6, "0")
    end

    def set_record
      # id ではなく項目コードでも引けるようにする。
      @record = Master::EndoscopyItem.find_by(item_code: params[:id]) || Master::EndoscopyItem.find(params[:id])
    end

    def record_params
      params.permit(Master::EndoscopyItem.column_names - %w[id created_at updated_at])
    end
  end
end
