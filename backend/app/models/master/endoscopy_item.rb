module Master
  # 内視鏡オーダー項目。医師がオーダー画面で選ぶ単位のオーダー項目で、
  # セットの構成は master_endoscopy_set_items が持つ。
  # 検査種別はコードで master_endoscopy_exam_types に緩く紐づく。
  # dataset_code は実施入力の初期明細(master_endoscopy_datasets)。1項目1つで、
  # 同じデータセットを複数の項目から参照してよい。実施入力をしない項目
  # (requires_perform_input = false)は初期明細を持たない。
  #
  # 生理検査の PhysioItem と同型。JED はオーダー項目を一意に定義するコード体系を
  ...
  class EndoscopyItem < ApplicationRecord
    ...
    # (逆向きの防止は EndoscopySetItem 側にある)。
    def solo_item_is_not_a_set_member
      return if groupable || item_code.blank?
      return unless EndoscopySetItem.exists?(member_item_code: item_code)
