module Master
  # セット(1オーダー → 複数の検査)の構成。set も member も内視鏡オーダー項目。
  # 放射線のセット構成(master_rad_set_items)と同じ考え方。
  class EndoscopySetItem < ApplicationRecord
    self.table_name = "master_endoscopy_set_items"
    ...
    # FK が無いのでコードで引いて確かめる(逆向きの防止は EndoscopyItem 側にある)。
    #
    # 画面にそのまま出る文なので、属性名が頭に付かない :base に載せる。
    def member_is_not_solo
      return if member_item_code.blank?

      member = EndoscopyItem.find_by(item_code: member_item_code)
