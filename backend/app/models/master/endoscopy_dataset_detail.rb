module Master
  ...
  #
  # material の参照先は生理検査(PhysioDatasetDetail)と同じで、算定コードそのもの
  # である master_medical_materials を直接参照する(施設内コードの段が無い)。
  # 内視鏡は止血クリップ等の特定保険医療材料が実在するので、この構成が活きる。
  class EndoscopyDatasetDetail < ApplicationRecord
    self.table_name = "master_endoscopy_dataset_details"
    ...
    scope :with_names, lambda {
      joins("LEFT JOIN master_medical_procedures " \
            "ON master_endoscopy_dataset_details.detail_type = 'procedure' " \
            "AND master_medical_procedures.procedure_code = master_endoscopy_dataset_details.code")
        .joins("LEFT JOIN master_medicines " \
               "ON master_endoscopy_dataset_details.detail_type = 'medicine' " \
               "AND master_medicines.medicine_code = master_endoscopy_dataset_details.code")
        .joins("LEFT JOIN master_medical_materials " \
               "ON master_endoscopy_dataset_details.detail_type = 'material' " \
               "AND master_medical_materials.material_code = master_endoscopy_dataset_details.code")
        .select(
    ...
    scope :in_display_order, lambda {
      order(Arel.sql("master_endoscopy_dataset_details.display_order NULLS LAST")).order(:id)
    }
