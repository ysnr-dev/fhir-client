module Master
  ...
  #
  # 放射線(RadDatasetDetail)との違いは material の参照先。放射線は施設内の器材
  # マスタ(master_rad_materials)を経由して算定コードに繋いでいるが、生理検査で
  # 個別算定できる特定器材はほとんど無いので、算定コードそのものである
  # master_medical_materials を直接参照する(施設内コードの段が無い)。
  class PhysioDatasetDetail < ApplicationRecord
    self.table_name = "master_physio_dataset_details"
    ...
    #
    # 放射線にある receipt_material_code は添えない。material の code 自体が
    # 算定用の特定器材コードなので、別に持つ必要がない。
    scope :with_names, lambda {
      joins("LEFT JOIN master_medical_procedures " \
            "ON master_physio_dataset_details.detail_type = 'procedure' " \
            "AND master_medical_procedures.procedure_code = master_physio_dataset_details.code")
        .joins("LEFT JOIN master_medicines " \
               "ON master_physio_dataset_details.detail_type = 'medicine' " \
               "AND master_medicines.medicine_code = master_physio_dataset_details.code")
        .joins("LEFT JOIN master_medical_materials " \
               "ON master_physio_dataset_details.detail_type = 'material' " \
               "AND master_medical_materials.material_code = master_physio_dataset_details.code")
        .select(
          "master_physio_dataset_details.*",
    ...
    scope :in_display_order, lambda {
      order(Arel.sql("master_physio_dataset_details.display_order NULLS LAST")).order(:id)
    }
