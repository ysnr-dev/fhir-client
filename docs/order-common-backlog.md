...
> **2026-09-11 に対応済み(相互作用・用量上限・腎機能・年齢を除く)。** 処方・注射のフォームの薬剤行に
> アレルギー照合・重複投与・規制区分の印を出すようにした。器は `fhir/medicationSafetyHelpers.ts`
> (FHIR にも React にも依存しない純関数)と `components/MedicineWarnings.tsx`
> (`useMedicationWarnings` フックと表示部品)の 2 つで、処方・注射・医薬品検索モーダルが同じものを使う。
> **登録は止めない**(判断は医師のもの。妊娠の注意と同じ扱い)。
>
> - アレルギー … `matchMedicationAllergies` を薬剤行に当てた。銘柄一致・成分一致の両方。
> - 重複投与 … 同一成分をフォーム内の別の行と、投与中の他の処方(`useActiveMedications`)から探す。
>   成分キーは YJ コードの上 7 桁(薬効分類 + 成分)。投与中かどうかは投与終了日(`rpEndDate`)で見て、
>   中止した処方と編集中のオーダー自身は数えない。
> - 印 … 麻薬 / 毒薬 / 覚醒剤原料 / 向精神薬(`master_medicines.narcotic_category`)、
>   生物由来製品、造影剤。医薬品 API が返していた値を `Medicine` 型に出して使う。
>
> **残っている限界**: 成分キーが薬効分類を含むので、アスピリン(解熱鎮痛)とバイアスピリン(抗血小板)の
> ように同じ成分でも効能が違うと重複と出ない。アレルギー照合(J-FAGY の YCM / GCM)も同じ制約を持つ。
> YJ コードを持たない薬剤(HOT コードマスタに無いもの)は黙って通る。
...
### 4.5 持参薬(MedicationStatement) — **入院の持参薬は対応済み(2026-09-26)**

> 登録・薬剤部の鑑別(鑑別一覧・通知)・医師の継続/休止/中止・継続分の院内処方化(処方区分
> 「持参」。調剤・処方箋・請求の対象外)・退院での終了・重複チェックへの参加まで。
> 設計は `docs/brought-medication-design.md`。上流は MedicationStatement に対応済みだった
> (JP_MedicationStatement)ので、プロキシの許可リストに足しただけ。
> **残り**: 外来の他院処方・お薬手帳の服用薬、院内採用薬に絞った代替薬の選択(医薬品マスタに
> 採用区分が無い)、持参薬の返却・廃棄の記録、退院処方で休止中の持参薬を再開する導線
> (設計書 §8)。
