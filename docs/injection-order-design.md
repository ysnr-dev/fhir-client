...
RP ごとに「その日の何時に始めて何時に終えるか」を複数組もてる。

| | 置き場所 | 備考 |
|---|---|---|
| 開始 | `dosageInstruction[0].timing.event`(dateTime の配列) | 日付は注射日。従来どおり |
| 終了 | `dosageInstruction[0].extension` の `injection-scheduled-period` | `start` と `end` の複合拡張 |

**終了をローカル拡張にした理由**: FHIR に置き場所が無い。`timing.event` は時点の配列で
終了を持てず、`repeat.duration` は施用ごとに違う長さを表せない(朝は 2 時間・夜は 6 時間が
書けない)。開始は `timing.event` に残したまま、**開始の値を鍵にして**終了を引き当てる
(配列の位置で対応させると、終了の無い施用が混ざったときにずれる)。終了を入れていない
施用には拡張を付けないので、この機能より前のオーダーもそのまま読める。

**終了が開始以下なら翌日**。夜からの持続点滴が日をまたぐのは普通なので、前後関係の
エラーにはしない。24 時間ちょうどは開始と同じ時刻になる。

**終了時刻の初期値は自動で入れる**(`infusionDurationHours` / `infusionEndTime`)。
投与時間を選んでいればその時間、選んでいなければ総投与量 ÷ 投与速度で 1 回ぶんの時間を
出し、開始時刻に足す。投与時間・投与速度・投与量を変えると追従するが、**手で直した
終了時刻は上書きしない**(変更前の条件での計算値と一致する行だけを差し替える)。
ワンショットに終了は無いので入力欄を伏せ、値も落とす。
...
- **連日オーダーを束ね単位で 1 枚のカードにまとめる表示**(いまは日ごとにカード。14 日展開で 14 枚)。
  ...
  「オーダーした日に本体を置く」案も選べる。
- ~~**経過表(フローシート)への注射欄**~~: 実装済み(2026-09-03)。`VitalFlowsheetPanel` の
  グラフの下に薬剤の組(RP)ごとの行を出し、予定・実施を並べる(`fhir/flowsheetInjectionHelpers.ts`)。
  終了予定時刻を持たせた(下記)ので、持続点滴の予定はバーで出る。
