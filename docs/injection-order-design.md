...
- ~~**経過表(フローシート)への注射欄**~~: 実装済み(2026-09-03)。`VitalFlowsheetPanel` の
  グラフの下に薬剤の組(RP)ごとの行を出し、予定を空丸・実施を塗り丸(終了時刻があればバー)で
  並べる(`fhir/flowsheetInjectionHelpers.ts`)。**終了予定時刻を保存していない**(§2、
  投与速度だけを持つ)ので、予定はバーにできず点になる。持続点滴の予定を線で見せたければ、
  先に投与時間か終了予定時刻を持たせる必要がある。
