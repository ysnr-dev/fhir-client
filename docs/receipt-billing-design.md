...
842 → `Medication_Number` に数値、850 → `Medication_Number` に年月日(`YYYY-MM-DD`。西暦 4 桁可で
展開時に和暦になる)、851 → `HH-MM`、852 → 分、831 → 診療行為コード(日レセ API 仕様 comment85-831-api)、
830 → `Medication_Name` に文だけ(全角 50 文字ごとに行を分ける)、820 → コードだけ(名称は参考)、
810000001 → 80 バイト(全角 2 バイト)で切る。日レセは中途終了データの登録時にコメントの中身を検査しない
(形式の誤りはレセプト点検で出る)。

## 2-1. 診療報酬用コメントの自動生成(2026-09-23)

［決定］コメントは UI から入力させず、**カルテが既に持つデータからルールで生成する**。作れないものは
送らず「送れない項目」に理由を出し、医事側(日レセ)で入力する。日レセのレセプト点検が欠落を拾う。

| コメント | コード | 値の出どころ | コードの置き場 |
|---|---|---|---|
| 撮影部位(単純撮影・CT など) | 820 選択式 | 放射線項目マスタ `site_comment_code`(項目ごと) | マスタの列 |
| 疾患名(疾患別リハ) | 830 | リハオーダーの対象疾患名(拡張) | 施設設定 `receipt_codes.rehab[区分].disease_name_comment` |
| 発症年月日(疾患別リハ) | 850 | リハオーダーの起算日(拡張) | 施設設定 `receipt_codes.rehab[区分].onset_date_comment` |
| 照射部位(放射線治療管理料) | 830 | 治療処方の標的(名称：部位)を「、」で並べる | 施設設定 `receipt_codes.radiotherapy.site_comment` |
| 実施時の自由記載 | 810000001 | 実施入力の備考 | 固定 |

コメントコードの一覧と形式は **コメントマスタ**(`master_comments`、配布ファイル c_ALL*.csv の全置換取込。
マスタ取込画面)で参照する。

**コメント関連テーブル**(`master_comment_relations`、ck_ALL*.csv)は「必須コメントの表」ではなく、
診療行為ごとに条件つきで関係するコメントの候補表(条件区分 01 = 算定したら要る / 02 = 入外 /
03 = 複数回 / 00 = 記載要領の文言で決まる)。用途は **設定画面での候補提示**: 施設設定のリハの
コメント欄はその行の療法士のコード、照射部位コメントは照射技法マスタの放射線治療管理料のコード、
放射線項目マスタの撮影部位コメントはその項目のレセ電算コードに関係するコメントを datalist で出す
(`CommentCandidates`。絞り込みではなく案内で、表に無いコードも入力できる)。自動生成の根拠には
使わず(値の出どころは表に無い)、送信も止めない。送信前の「要るのに無いコメント」の注意表示は
条件区分 01 と入院外の 02 に限って出せるが、日レセのレセプト点検が同じ表で拾うので後回し。
...
- **Phase 4**(2026-09-23 済み)外用の日数(総量 × 1)、一般名処方(一般名コード → 薬価最安の
  ...
  処方区分「持参」(患者の持参薬を使う処方)は送らない(2026-09-26。skipped にも積まない)、
...
- **Phase 5** 放射線のフィルム(枚数-分画。実施入力に分画数を足す。特定器材のフィルムは
  `material_category` では区別できないので名称/コード帯で見分ける)。撮影部位コメントと
  コメントの自動生成は 2026-09-23 に §2-1 で対応。コメント関連テーブルの取込は次の段階。
