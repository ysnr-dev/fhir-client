...
- 検査共通: 対象プロブレム / 入外区分
  - ［提案］**伝票共通の依頼コメントは持たない**。1 オーダーの単位が撮影項目ごとに変わり
    （単独の項目は 1 件で 1 オーダー）、伝票全体への申し送りが載る先を持たなくなったため。
    申し送りは GP 単位の**特別指示**に書く。
  - 至急区分・撮影日・撮影時刻も伝票共通ではなく、選択中のオーダー枠ごとに入力する
  - 予約必須の項目は、撮影日時の代わりに枠を選ぶ（枠の日時がそのまま撮影日時）。
    ［提案］**予約日時の変更もこの画面（カルテカード→編集）から**行い、予約タブからは
    行わない。オーダーヘッダの撮影日時と予約（Appointment＋Slot）は必ず一緒に動かす
    必要があり、入口を 1 つにした方が同期の面倒を 1 か所に閉じ込められるため。
    更新は 1 つの transaction（ヘッダ・明細の書き戻し＋予約の付け替え＋旧枠 free /
    新枠 busy）で書く。予約タブに残るのは診察予約の日時変更だけ（検査予約の行は
    「日時変更・取消は放射線オーダーから」と出す）
