...
- パネルには**絞り込み**を置く(コースは数十件並ぶ): 装置・進捗・患者(氏名/患者番号の部分一致)・
  「予定が足りないコースのみ」。読んだ行に対して画面側で効かせる(他の部門一覧と同じ)。
...
治療処方を一緒に取る)。右のパネルの回数と累積線量は `useRadiotherapyProcedures`(全コースの照射記録を
ページを送って読む)。後者は終了したコースのぶんも読むので、件数が増えたら進行中のコースに絞る(§8)。
...
- **右のパネルの照射記録は全コースぶんを読む**(`useRadiotherapyProcedures`。上限 1 万件)。
  コースが増えたら、進行中のコースの id で絞るか、集計を上流に持たせる
...
17. 右のパネルの絞り込み: 装置(リニアック1号機 / 装置未定)、進捗、患者「次郎」「22」、
   「予定が足りないコースのみ」。見出しが「2 / 3 件」になり、「絞り込みを外す」で戻る
