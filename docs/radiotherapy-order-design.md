...
- 入口はカルテの治療処方カードのメニュー「週次レビュー」。右ペインにそのコースの診察が並び、その下に
  テンプレートの記入欄が出る。**2 回目からは前回と同じテンプレートを選択済みで開く**
  (`defaultQuestionnaireCanonical`)。毎回プルダウンから探さずに済む
...
- **Grade を付けて追うものは有害事象に記録する**(上記)。週次レビューは「その日に診てどう判断したか」で、
  役割が違う
- ［申し送り］既定テンプレートの設定と、診察が空いたコースの督促 Task は後続(§8)
...
- **週次レビューの様式は毎回選ぶ**(2 回目からは前回のテンプレートが選択済み。§6.3)。施設設定に
  「放射線治療の既定テンプレート」を持たせれば初回から選ばずに済む(他科依頼・術式マスタと同じ形)
- **診察が空いたコースの督促が無い。** 部門一覧で強調するだけなので、気づくには一覧を見に行く必要が
  ある。通知に出すならレジストリに 1 要素(`notification-task-registry` と同じ形)
