...
休止)、第3段階(治療終了サマリー)、第4段階(照射予定の一括登録と放射線治療カレンダー。§7)を
実装済(2026-09-21)。** 週次レビュー・計画 CT・会計は後続フェーズ(§6)で、構造だけを定める。
...
### 4.1 部門の画面(放射線治療カレンダー)

`/radiotherapy-worklist`(部門業務メニュー「放射線治療カレンダー」)。格子と照射予定は §7。
治療コースの一覧は画面の右のパネルにあり、status で切る。

- **進行中** … `status=active,on-hold`。いま抱えているコースなので有限
- **終了・中止** … `status=completed,revoked` の直近ぶん(登録日の降順)

「終了」は終了日を、「中止」「休止」は日付・理由(マスタ)・補足を聞いてから進める。取消(終了を取消・
中止を取消)や再開で戻すと、それらは落ちる(`buildRadiotherapyOrderStatusEntry`)。

［改訂］当初は他科依頼一覧と同じ表形式の一覧だった。照射予定を一括登録するようにしたのに合わせて
カレンダーに置き換えた(日々の仕事は「今日この装置で誰を照射するか」で、コースの表からは読めない)。
...
- **［決定］照射しなかった回も記録に残す**(`status=not-done` ＋ 理由)。休止の判断と治療期間の評価に
  要るため。線量も実施者も持たない
- ［改訂］当初は照射入力に「この回は照射しなかった」のチェックを置いていた。**やめた** —— 照射入力は
  実施したときに入れるもので、そこに「しなかった」の入口を混ぜると操作の意味が濁る。**予定のカードの
  ケバブの「この回を中止」**(理由はマスタから)で記録し、押し間違いは「中止を取消」で予定に戻す。
  コースそのものの中止(revoked)とは別物なので、カードの文言は「この回を中止」にしてある
- ［導出］中止した回は**番号を空ける**。5 回目を中止したら、次に組むのも 5 回目(別の日への振り替え)。
  「未実施でない件数 + 1」で採番すると、残っている予定と番号がぶつかる(§7.2 の `draftRadiotherapyPlan` は
  使用済みの番号を避けて空きから埋める)
...
### 6.3 有害事象(CTCAE)【実装済】と治療中の診察(週次レビュー)

照射による有害事象は、化学療法と**同じ器**(`category=adverse-event` の Observation)に記録する
(`docs/chemo-regimen-design.md` §8.7)。当初は化学療法専用で、適用ヘッダへの参照を `regimen-order`
拡張の中に持っていたため放射線治療では読めなかった。**原因となった治療を標準の `basedOn` に移して
一般化した。**

```text
Observation
  category   adverse-event
  code.text  CTCAE 用語(自由記述)
  valueInteger  Grade(1〜5)
  effectivePeriod  発現日 〜 回復日(継続中は end 無し)
  note       対処・経過
  basedOn    原因となった治療のヘッダ(治療処方 / レジメン適用)
  extension[treatment-context]
    type     radiotherapy | chemo-regimen   ＋ name(治療の名前の写し)、化学療法は cycle
  performer  記録した医療従事者
```

- ［決定］**参照は拡張ではなく `basedOn`。** 拡張の中の参照は検索できず、1 コースぶんを見るのに
  患者の有害事象を全部読むしかなかった。上流は `Observation?based-on=` に対応済みで改修は要らない
  (`useTreatmentAdverseEvents`)
- ［決定］**クールに当たる区切りは持たない。** 化学療法はクールが記録の単位だが、照射は毎日続くので
  コース単位で並べる。何回目の頃に出たかは発現日と照射記録を突き合わせれば追える
- 入口はカルテの治療処方カードのメニュー「有害事象」→ 右ペイン(`RadiotherapyAdverseEventPanel`、
  `radiotherapy-adverse`)。フォームは化学療法と共通(`AdverseEventEditor`)で、用語は CTCAE 用語マスタから
  選ぶか自由入力。放射線治療マスタに「想定される有害事象」が無いので候補は出さない(部位別の候補は後続)
- 表示は治療処方の詳細(カルテの詳細モーダルと部門一覧の両方)。終了サマリーの「急性有害事象」は
  医師が書くまとめで、こちらは 1 件ずつの記録
- 旧形式(`basedOn` の無い化学療法の記録)は読みで受ける。上流の backfill
  (`20260922000001_backfill_adverse_event_based_on`)で `basedOn` と `treatment-context` を足したので、
  **移行が済んだ環境では化学療法も `based-on` で引ける**(クライアントはまだ患者単位で読んでいる。
  旧クライアントのために `regimen-order` 拡張を残してあるので、消せるのは全環境の更新後)

#### 治療中の診察(週次レビュー)【実装済】

照射は数週間続き、そのあいだ放射線治療医が週に 1 回ほど患者を診て、治療を続けられるかを判断する。
書く中身は施設ごとに違うので、**テンプレート(Questionnaire)で様式を決め、回答を治療処方に
`basedOn` で結ぶ**(`RadiotherapyReviewPanel`、`radiotherapyReviewHelpers.ts`)。

- ［決定］**診療記録(Composition)には寄せない。** 上流の Composition 検索は type / subject / date /
  author / entry だけで `event.detail` が無く、「そのコースの診察」を集められない —— 集められないと
  診察の抜けを見つけられない。QuestionnaireResponse は `based-on` 検索に対応済みで上流の改修が要らない
- ［決定］**専用のリソースは足さない。** テンプレート回答はカルテのタイムラインにも出て、平文表示も
  そのまま効く。様式の入れ替えも施設の手で済む
- 入口は**カルテの治療処方カードのメニュー「週次レビュー」**と、**部門一覧のコースのメニュー**
  (カルテへ飛び、右ペインが開いた状態になる。`open=radiotherapy-review:<id>`)、それに**通知の
  「カルテ」**。気づく場所(部門一覧・通知)から書く場所へ直に行けるようにしてある
- 右ペインにはそのコースの診察が並び、その下にテンプレートの記入欄が出る。**2 回目からは前回と
  同じテンプレート**、初回は**施設の既定テンプレート**(`radiotherapy_review.template`)を選択済みで
  開く。毎回プルダウンから探さずに済む
- 部門一覧(放射線治療カレンダー)のコースに **最後の診察日と経過日数**を出す。間隔を超えたコースと、
  **一度も診ていないコース**は強調する(照射が始まっているのに一度も診ていない状態がいちばん
  見落としたくない)。見るのは**治療中**のコースだけ —— 処方済・計画中はまだ照射が始まっていない
- ［事実］間隔の既定を 7 日にしたのは、診療報酬の**外来放射線照射診療料(B001-2-8)**が
  「7 日間に 1 回に限り算定」し、**算定日に放射線治療医(放射線治療の経験 5 年以上)の診察**を
  要件にしているため。ただし同点数は「**入院中の患者以外**」が対象で、入院の患者や施設の運用まで
  縛るものではない。［決定］そこで **7 日は既定値にとどめ、施設設定(`radiotherapy_review.interval_days`、
  管理 > 施設設定 > 放射線治療の診察間隔)で変えられるようにした**
- 様式は `docs/report-mappings/radiotherapy-review-01.md`(`RT_REVIEW_01`)。全身状態(PS・体重・食事摂取)、
  照射野の皮膚・粘膜炎・自覚症状、治療継続の判断と対応・次回予定
- **Grade を付けて追うものは有害事象に記録する**(上記)。週次レビューは「その日に診てどう判断したか」で、
  役割が違う
- **診察が空いたコースには督促の通知(Task)を出す。** 契機は**照射入力** —— 照射は治療中ほぼ毎日
  あるので、間隔を超えたことに最初に気づける場所であり、部門の手が動くところでもある(外来放射線
  照射診療料が第 2 日目以降の観察を医師に報告させているのと同じ流れ)。宛先は治療処方の依頼医
  - 同じコースの督促は**未対応のものがあれば作り直さない**(毎回の照射で増やさない)
  - **週次レビューを書くと閉じる**(`useCloseRadiotherapyReviewDue`)。通知一覧から手で閉じてもよい
  - 通知の種別はレジストリに 1 要素(`radiotherapy-review-due`)。一覧・件数・種別フィルタ・一括対応は
    それだけで動く
...
## 7. 照射予定の一括登録と放射線治療カレンダー【第4段階・実装済】

### 7.1 予約枠(Slot)を使わず、照射予定を Procedure で持つ

［提案］照射予定は **照射記録と同じ Procedure を `status=preparation` で作る**。当日はその予定を
開いて実績に書き換える(同じ Procedure への PUT で `completed` / `not-done` になる)。

- 手術と同じ判断(`docs/surgery-calendar-design.md` §1)。Schedule / Slot は枠の事前量産と先着の
  予約が前提だが、照射の時間割は部門が装置ごとに組み、休止や装置の点検のたびに残り全部がずれる
- 予定と実績が同じリソースなので、「何回目の予定がどの実績になったか」を突き合わせる必要が無い。
  Phase・回・装置・時刻は予定に入っていて、実施入力はそれをそのまま初期値にする
- **予定は照射録ではない**ので削除は物理削除でよい(実績の取消は `entered-in-error`。§6.1)
- 回数・累積線量・実施日の集計は `preparation` を数えない(`radiotherapyProgress`、
  `orderProgressHelpers.ts` の実施日、退院時サマリーの実施日)
- ［事実］上流の Procedure の `date` 検索は `performedPeriod.start` / `performedDateTime` を引くので、
  予定も同じ検索で期間を切れる。上流の改修は不要

### 7.2 一括登録(`RadiotherapyPlanModal`)

治療コースのカードの「予定登録」。処方の回数から、照射済みの回と登録済みの予定を引いた**残り**を、
Phase の並び順に、指定の曜日へ 1 日 1 回ずつ割り付ける(`draftRadiotherapyPlan`)。

- 入力は 開始日・照射する曜日(既定は月〜金)・開始時刻・1 回の所要時間(既定 15 分)・治療装置。
  開始日の既定は、予定や実績があればその最後の日の翌日
- **祝日と装置の点検日は知らない。** 割り付けた結果は表で出し、行ごとに日付を直すか外せる
  (条件を変えて割り付け直すと、直した内容は捨てる)
- 登録は 1 回 = Procedure 1 件を、1 つの transaction で作る(30 回でも 1 リクエスト)
- 予定を作れるのは受付後(計画中・治療中・休止。§7.4)。組み直すときはカードのメニューの
  「予定をすべて削除」→ もう一度「予定登録」。休止で後ろへずらすのもこの手順
- 1 日 2 回照射(加速過分割)の割り付けは未対応。1 日 1 回で割り付けてから「予定変更」で直す

### 7.3 カレンダー(`RadiotherapyCalendar`)

手術カレンダーと同じ「格子 + 右のパネル」で、スタイルも `surgery-calendar__*` を共用する。

- **日ビュー** … 列 = 治療装置(装置マスタの並び。装置を決めていない照射があるときだけ末尾に
  「装置未定」)、縦 = 時刻。カードは照射 1 回(時刻・状態[予定 / 実施済 / 未実施]・患者・`回/処方回数`)。
  同じ時刻に重なったら列の中で横に分ける。時刻を持たない記録は見出しの下の「時刻なし」の帯に置く
  (この帯と列見出しは**全列で同じ高さ**にする。1 列だけ高いと、その列の格子が下へずれて時刻の軸と
  合わなくなる)
- **週ビュー** … 行 = 治療装置、列 = 日。件数(うち実施)と患者名。日付かセルを押すと日ビューへ
- 予定のカードの「実施」で照射入力が開き、その予定を実績に書き換える。メニューから
  「コースを表示」「予定変更」(日時・装置)「予定を削除」
- 右のパネルは治療コースの一覧(§4.1)。受付・治療開始・**予定登録**・休止・再開・終了・中止・
  サマリー・照射入力(予定を作らずに 1 回ぶん入れるとき)
- パネルには**絞り込み**を置く(コースは数十件並ぶ): 装置・進捗・患者(氏名/患者番号の部分一致)。
  読んだ行に対して画面側で効かせる(他の部門一覧と同じ)。件数は絞り込み中だけ「2 / 3 件」と出す。
  装置は**予定・実績の装置**で見て、まだ日程を組んでいないコースは Phase の使用予定装置で補う ——
  そうしないと「これから組むコース」が装置で絞ったときに消えてしまう
- **照射予定は掴んで動かせる**(日ビュー: 時刻と装置。週ビュー: 日と装置で、時刻は変えない)。日ビューでは
  上下の縁を掴んで開始・終了時刻を伸縮できる。刻みは 5 分。実績(実施済・未実施)は照射の記録なので
  動かせない。書き込みは予定の Procedure への PUT(`useRescheduleRadiotherapyFraction`)で、確認は挟まない
  (手術は重なりの確認を挟むが、照射は重なりを止めていない。§8)
- 「この患者の残り全部」をずらすのは、コースのメニューの「予定をすべて削除」→「予定登録」

［実装］格子は `useRadiotherapyCalendar(from, to)`(Procedure を `date` で切り、`_include` で患者と
治療処方を一緒に取る)。右のパネルの回数と累積線量は `useRadiotherapyProcedures` で、**そのとき表示して
いるコースの id を `based-on` に並べて引く**(20 コースずつに分けて並列)。category だけで引くと終了した
コースのぶんまで読むことになり、コースが増えるほど重くなる。

- 格子に出ているだけのコースは id に足さない。足すと**日付を動かすたびに**照射記録を引き直すことに
  なるし、カードは `useRadiotherapyCalendar` の結果だけで描ける
- 入力するモーダル(実施入力・一括登録・サマリー)は**開いた時点の照射記録で初期値が決まる**ので、
  一覧の写しではなく `useRadiotherapyCourseFractions` でそのコースだけを引き直し、**届いてから開く**。
  一括登録の直後に続けて実施入力を開くような場面で、採番が古い写しに引きずられないように。詳細の
  表示は初期値を持たないので待たずに開く
- 回数・累積線量は届くまで伏せる(0 回と読めてしまう)。装置の絞り込みも照射記録を見るので、
  届いてから効かせる

### 7.4 空き枠とコースのドロップ(新しいオーダーは作らない)

［改訂］当初は手術カレンダーをなぞって、空き枠を掴むと新しい放射線治療(治療処方)を登録していた。
**やめた。** 治療処方は放射線治療医がカルテで書くもので、その時点では装置も時刻も決まっていない。
決まるのは計画 CT と治療計画が済んで部門が日程を組むときなので、格子から始まるのは新しいオーダーでは
なく**既にあるコースの日程**。手術で言えば「未確定の手術を格子に落として日程を確定する」に当たる。

入口は 3 つで、どれも同じ一括登録(§7.2)に行き着く。

- **治療コースのカードを掴んで格子へ落とす** … 落とした装置・時刻(週ビューは日と装置)を初期値に
  して一括登録が開く。掴めるのは予定を足せるコースだけ
- **空き枠を掴む**(縦に引けば長さも決まる)… `RadiotherapySlotModal` で、予定を足せるコースから選ぶ。
  「予定を組む」は一括登録へ、**「1 回だけ追加」**は次の回をその枠に 1 件だけ入れる(振替・飛んだ日の
  埋め合わせ。どの Phase の何回目かは一括登録と同じ数え方)。
  ［提案］**入れられるコースが無いときに、対象外のコースは並べない。** 進行中が何十件もある施設では
  押せない行で埋まるだけなので、件数と次の一手だけを出す(「進行中の 15 件のうち 3 件は受付がまだです。
  右の一覧で『受付』を押すと…」)。コースそのものは右の一覧にある
- カードの「予定登録」… 枠を決めずに開く

マウスの下の枠は薄く塗って時刻を出す(手術カレンダーと同じ下見)。刻みは 5 分だが、帯は既定の長さ
(15 分)で塗る —— 5 分ぶんでは細すぎて時刻が読めない。下見と登録される時刻は同じ計算(切り捨て)。

［決定］**予定を足せるのは受付後(計画中・治療中・休止)のコースだけ。** 処方済は部門がまだ引き受けて
いないので、先に受付する(`canPlanCourse`)。カルテのカードは進捗に関わらず照射予定を出す。

［申し送り］空き枠の使い道として、装置を止める帯(朝の QA・定期点検・故障。手術室ブロックに当たる)と、
計画 CT・位置決めの予約(§6.5)が残っている。どちらも保存先が新しく要る。

---

## 8. 未決事項・申し送り

- **右のパネルの照射記録はコースの数だけリクエストが増える**(20 コースで 1 本。§7.3)。進行中が
  数百件並ぶ施設では本数が効いてくるので、そのときは集計(回数・累積線量・次回予定)を上流に持たせる
- **同じ装置・同じ時刻の重なりを止めない。** カレンダーで横に並んで見えるだけ
...
- **督促は照射入力が契機**(§6.3)。照射のない日が続くコース(休止中など)では作られないので、
  診察の間隔だけで督促したい施設には足りない。定時に回す仕組みが要る
- **診察の間隔は施設に 1 つ**(§6.3)。部位や入外区分で変えたいときは持ち方から要検討。外来放射線
  照射診療料は外来の患者だけを対象にしているので、入院の患者に別の間隔を当てる施設はありうる
...
12. 治療コースの「予定登録」→ 残り 9 回が平日に割り付けられる。祝日の行の日付を直し、装置と時刻を
   入れて登録 → カードが「照射 1 / 10 回 予定 9 回（次回 …）」、カレンダーの装置の列に予定が並ぶ
13. 予定の「実施」→ 日時・Phase・回・装置・線量が入った状態で開く → 登録すると同じカードが
   「実施済」になり、照射 2 / 10 回・予定 8 回に進む。週ビューの件数、「予定変更」での時刻の移動も確認
14. 日ビューの空き枠を掴む → 予定を足せるコースの表が開く →「予定を組む」で一括登録が掴んだ枠
   (日付・時刻・長さ・装置)を初期値にして開く。「1回だけ追加」で次の回がその枠に 1 件入る。
   右のパネルのコースを掴んで格子へ落としても、落とした枠で一括登録が開く
15. 予定のカードのケバブから「この回を中止」(理由: 患者の体調不良)→ 未実施になり、回数に数えられず
   残りが 1 増える。「予定登録」を開くと空いた番号(4 回目)が振替として提案される。「中止を取消」で
   予定に戻る
16. 予定のカードを別の装置の 11:00 へ掴んで動かす(長さは保たれる)、下の縁で 30 分 → 15 分に縮める、
   週ビューで別の日へ動かす(時刻は変わらない)
17. 右のパネルの絞り込み: 装置(リニアック1号機 / 装置未定)、進捗、患者「次郎」「22」。
   見出しが「2 / 3 件」になる
18. `tsc -b`、backend の rspec(`spec/requests/master/radiotherapy_masters_spec.rb`、
   施設設定の spec)
19. (2026-09-22)照射記録を id で絞る読み方(§7.3): 一覧を開くと `Procedure?based-on=…` が表示中の
   コースぶん 1 本だけ飛ぶ。日付を動かしても格子の検索しか飛ばない。予定の「実施」ではそのコース
   だけの `based-on` が 1 本飛び、何回目・線量が入った状態で開く。「終了・中止」へ切り替えると
   そのコースを足した 1 本に変わり、サマリー編集も集計が入って開く。装置の絞り込みは「2 / 3 件」
20. (2026-09-22)有害事象(§6.3): テスト太郎の第2コースのカードのメニュー「有害事象」→ 右ペインに
   「第2コース 左第5肋骨転移」。「放射線皮膚炎 G2、対処つき」を追加 → 一覧に「G2 放射線皮膚炎
   2026-09-22 〜（継続中）」、パネルは開いたまま。保存された Observation は `basedOn` に治療処方、
   `treatment-context` に type=radiotherapy と名前。詳細モーダルの Phase と照射記録のあいだに
   「有害事象」の表。化学療法(大腸太郎の mFOLFOX6 第 2 クール)も同じフォームで記録でき、クール
   一覧の「有害事象」列に出る。上流 backfill は旧形式の行を 1 件作って `basedOn` が付くのを確認。
   `tsc -b`、上流の `spec/requests/observations_spec.rb`
21. (2026-09-22)週次レビュー(§6.3): テンプレート `RT_REVIEW_01` を取り込み、カテゴリ「放射線治療」を
   作った。テスト太郎の第2コースのカードのメニュー「週次レビュー」→ 右ペインに「診察なし」。
   テンプレートを選ぶと PS・体重・皮膚・判断の欄が出て、記入して登録 → カルテのタイムラインに
   テンプレート回答のカードが出る。保存された QuestionnaireResponse の `basedOn` は治療処方。
   部門一覧のカードが「診察 09-22（0 日）」になり、詳細に「診察記録 (1 件)」。治療中で診察が無い
   コース(リニアック一郎を治療開始にして確認)は「診察なし」が強調され、計画中に戻すと消える
22. (2026-09-22)診察間隔の施設設定: 診察日を 09-10 にずらすと「診察 09-10（12 日）」が強調され、
   施設設定の `radiotherapy_review.interval_days` を 14 にすると強調が消える(既定は 7)。
   backend の `spec/models/facility_settings_spec.rb` と `spec/requests/facility_settings_spec.rb`
23. (2026-09-22)導線・既定テンプレート・督促: 部門一覧のコースのメニュー「週次レビュー」→ カルテが
   開き右ペインが週次レビューになる(URL の `open` は読んだら消える)。診察の無いコース(リニアック
   一郎)を治療開始 → 照射入力すると、同じ transaction で督促の通知ができる(「第1コース 肺 診察の
   記録がありません」、宛先は依頼医)。通知一覧の「カルテ」からそのコースの週次レビューが開き、
   **診察が 1 件も無くても施設の既定テンプレートが選択済み**。登録すると督促が `completed` になり
   note に「診察を記録しました。」が残る
