...
［提案］**治療中の診察(週次レビュー)そのものは未着手。** 記録は診療記録(Composition)に書けるが、
上流の Composition 検索に `event.detail` が無いので「そのコースの診察」を集められず、診察の抜けを
見つけられない。テンプレート回答(QuestionnaireResponse)は `basedOn` を持てて上流も `based-on` 検索に
対応しているので、週次レビューの様式をテンプレートで作り、回答を治療処方に結ぶのが素直(§8)。
...
- **治療中の診察(週次レビュー)を残す手段が無い**(§6.3)。テンプレート回答を治療処方に `basedOn` で
  結べば「最終診察日」「7 日以上空いたコース」を部門一覧に出せる。督促は通知レジストリに 1 要素
