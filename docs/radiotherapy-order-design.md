...
- 予定を作れるのは受付後(計画中・治療中・休止)。組み直すときはカードのメニューの
...
- ［提案］手術と違い、カードを掴んで動かす操作は持たない。照射は 1 日に何十件も同じ長さで並び、
  動かす単位は「この患者の残り全部」なので、個別は「予定変更」、まとめては組み直しで行う
...
14. `tsc -b`、backend の rspec(`spec/requests/master/radiotherapy_masters_spec.rb`、
   施設設定の spec)
