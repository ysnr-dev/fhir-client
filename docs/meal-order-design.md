...
**取込 API は無い**。配布マスタが存在しないので、施設の約束食事箋を見ながら画面で直す
運用にする(処置と同じ)。ただし空から 1 件ずつ作るのは重いので、**代表的な食種・主食・
副食形態・種別を seed に同梱した**(§3.4)。
...
### 3.2 種別は「給与形態」を固定コードで持つ

実装日: 2026-08-30。

［事実］参考仕様 §1 は食種を「普通食及び治療食 / 経管･経口食 / 調乳食 / 欠食」に分類し、
**分類ごとに入力項目が違う**と定めている(§2 / §3 / §4 / §5)。普通食・治療食には主食と
副食形態、経管・経口食には濃度と配ぜん量、調乳食には商品名と本数、という具合。

［導出］ところが**種別マスタの名称は施設が自由に付ける**。「一般食」「特別食」で運用する
施設もあれば「普通食」「経管栄養」「乳児食」の施設もあり、名称の文字列からは画面が
入力欄を切り替える判断ができない。

［提案］種別マスタに**給与形態**(`nutrition_form`)を 1 列足し、システムが解釈できる
固定コードにした。施設が種別を作るときに 1 回選ぶだけで済む。

| 値 | 表示 | 参考仕様 | 主食・副食形態 | 塩分制限 |
|---|---|---|---|---|
| `oral_diet`(既定) | 普通食・治療食 | §2 | あり | あり |
| `enteral_formula` | 経管・経口食 | §3 | 無し(濃厚流動食) | あり |
| `infant_formula` | 調乳食 | §4 | 無し(調製乳) | 無し |

［提案］名前は **FHIR `NutritionOrder` の要素名**(`oralDiet` / `enteralFormula`)に寄せた。
§7-2 の載せ替えを検討するとき、この軸がそのまま移せる。

［提案］**「欠食」は値に入れない**。食止めは食種マスタの `is_fasting` が担当しており
(§2.5)、ここにも置くと二重管理になる。参考仕様が 4 分類にしているのは、食種と食止めを
同じ「食種」の平面で扱っているためで、こちらは既に別の軸に分けてある。

［提案］**食種ではなく種別に持たせた**。経管栄養の種別に経口の食種が混ざることはないので
種別の性質そのものであり、種別は数件・食種は数十件なので設定コストが 1 桁小さい。

［実装］`null: false` + `default: "oral_diet"` で足したので、**この列を入れる前の種別は
すべて普通食・治療食**になる(migration だけで後方互換)。未分類の食種と、種別マスタが
まだ届いていない画面も同じく `oral_diet` として扱う。

［実装］オーダー画面(`MealOrderForm`)の効き方:

- 主食のセレクトは**欄ごと残し、主食マスタの選択肢だけ落とす**。「欠食(その食事を
  出さない)」はどの給与形態でも指示しうるため。見出しは「主食」→「食事」に変わる
- 副食形態の欄は `oral_diet` のときだけ出す。塩分制限は `infant_formula` でだけ落とす
- 食種を切り替えたときに前の入力が残らないよう `normalizeForNutritionForm` で落とす
  (欠食は残す)。同じ値なら同じ参照を返して effect が回り続けないようにしてある

［申し送り］経管・経口食と調乳食の**専用の入力欄(濃度・全エネルギー・配ぜん量・商品名・
本数・乳首サイズ)はまだ無い**。いまはコメント(`note`)に書くことになる。作るときは
この軸で分岐すればよい(§7-8)。

### 3.4 seed

［提案］**代表的な内容を seed に同梱する**(2026-08-31)。術式マスタ(K 章 67 件)と同じ
「初期値を入れて画面で直してもらう」形。既存行は**上書きしない**ので、施設が直した
あとに再投入しても消えない。

| CSV | 件数 | 内容 |
|---|---|---|
| `meal_categories.csv` | 4 | 一般食 / 特別食(ともに `oral_diet`)/ 経管栄養(`enteral_formula`)/ 調乳食(`infant_formula`) |
| `meal_diets.csv` | 34 | 一般食 10(常食 4 段階・全粥・七分/五分/三分粥・流動食)、特別食 18(糖尿病 4 / 腎臓 3 / 心臓・減塩 2 / 肝臓 / 膵臓 / 胃潰瘍 / 低残渣 / 貧血 / 脂質異常症 / 痛風 / 嚥下調整 / 妊娠高血圧)、経管栄養 4、調乳 1、食止め 1 |
| `meal_items.csv` | 21 | 主食 14(米飯 4 段階・軟飯・全粥・七分/五分/三分粥・重湯・パン 2・うどん・おにぎり)、副食形態 7 |

- コードは SS-MIX2 の給食オーダ(OMD^O03)の例示に寄せた形。`A001xx`=一般食 /
  `A002xx`=特別食 / `E001xx`=経管 / `M001xx`=調乳 / `NPO`=食止め / `105Axx`=主食 /
  `F0x`=副食形態。**施設独自採番(`000001` の連番)とぶつからない**(§3 の採番規則が
  英字混じりを計算から外すため)
- ［事実］**コードは kcal の昇順になっていない**(`A00201` が糖尿病食1600kcal で
  `A00203` が1200kcal)。既に運用・ドキュメントで使っているコードを動かさないことを
  優先し、並びは `display_order` で付けている
- ［提案］**主成分量は代表値**。熱量と三大栄養素の整合(蛋白 4 + 脂質 9 + 糖質 4)は
  全行 3% 以内に取ってあるが、実際の値は施設の献立で決まる。**栄養課が置き換える前提**で
  投入するもので、seed の comment と本節に明記しておく。適応・備考も同じ
- 調乳食(`M00101`)は成分を空にしてある。商品名・濃度・本数で決まり、1 日総量として
  持てないため(§7-8 の入力欄ができるまではコメントに書く)
...
| seed | `db/seed_data/meal_categories.csv`(4)/ `meal_diets.csv`(34)/ `meal_items.csv`(21)と `db/seeds.rb` の投入(§3.4) |
...
### 6.4 給与形態(§3.2)の追加 — 2026-08-30

| 層 | 変更 |
|---|---|
| migration | `20260831120000_add_nutrition_form_to_master_meal_categories`(`null: false` + `default: "oral_diet"`) |
| モデル | `Master::MealCategory::NUTRITION_FORMS` + inclusion 検証 |
| spec | `master/meal_categories_spec.rb` に既定値・指定・不正値の 4 例(9 examples) |
| 画面 | `MealCategoryPage`(一覧の列 + 編集モーダルのセレクト) / `MealOrderForm`(主食の選択肢・副食形態・塩分制限の出し分けと `normalizeForNutritionForm`) / `masterClient.ts` / `mealItemOptions.ts`(`MEAL_NUTRITION_FORM_OPTIONS`) |

FHIR は無変更。給与形態は**マスタ側の属性で、オーダーには写していない**
(`is_fasting` と同じ扱い。§2.5)。

検証(開発環境):

1. 種別マスタに「経管栄養」(`03`、経管・経口食)を足し、一覧に「給与形態」列が出て
   既存の 2 件が「普通食・治療食」になっていること(migration の default が効いている)
2. 編集モーダルの「給与形態」セレクトに保存値が復元されること
3. 回帰: `meal_categories_spec` 9 examples green、`tsc -b` clean、`oxlint` 既存 4 warning のみ

［申し送り］**オーダー画面での出し分けは画面から通していない**。開発環境に経管栄養の
食種(`E00101 経腸栄養剤1200kcal`、種別 `03`)は入れてあるので、次に触るときに
...
8. **経管・経口食と調乳食の専用の入力欄**: 給与形態(§3.2)で分岐する土台はできたが、
   濃度・全エネルギー・配ぜん量(§3)、商品名・容量・本数・乳首サイズ(§4)の欄はまだ無く、
   コメントに書くことになる。作るなら `enteral_formula` の入力群は FHIR
   `NutritionOrder.enteralFormula` がそのままの構造なので、§7-2 の載せ替えを先に
   検討する価値がある
9. **参考仕様 §2 の残りの入力項目**: 成分調製・飲料区分・飲物(本)・主食量(g)・主食形態
   ...
   いまの運用で足りているかを先に確かめる
10. **欠食理由(§2.9)の自動入力**: いまは外出泊の連動が `leave` を入れるだけで、`ope`(手術絶食)・
