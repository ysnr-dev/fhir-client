...
**実装済み**: 「病名」タブの区分ラジオに「既往歴」がある(readme の「病名の区分」を参照)。
基礎データの既往歴はそちらで登録する。
...
- **カルテのタイムラインに時系列のカードとして並ぶ**。基礎データは「その時点の
  出来事」ではなく患者に紐づく現在の状態なので、回答を重ねるとタイムラインに
  同じテンプレートのカードが積み上がる。左ペインの「種別」からテンプレートを
  選べば、そのテンプレートの回答だけを最新順に読めるので、最新値の確認と経過の
  確認はここで行う(readme の「左ペイン(診療日 / 種別)」を参照)。
