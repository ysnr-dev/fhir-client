...
院内共通のチャート「歯周病×糖尿病」として初期値に入っています(`backend/db/seed_data/chart_definition_presets.json`)。
項目コード・名称・単位・選択肢はこのテンプレートと同じ値をそこへ書き写しているので、テンプレートの項目を変えたときは両方を直してください。
