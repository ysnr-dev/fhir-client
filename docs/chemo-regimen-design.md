...
- **C-1 クールとレジメンの完了**: 日オーダーの Task がすべて completed / cancelled になったクールを「完了」と表示し、
  レジメン詳細に「完了にする」(ヘッダ `completed`)と「休止 / 再開」(`on-hold` ↔ `active`)を置く。
  予定クール数に達しても自動で completed にはしない(医師が決める)。
- **C-2 中止理由**: レジメンの中止(`useRevokeRegimen`)と日オーダーの中止に理由(区分 + 自由記述: 病勢進行 / 有害事象 /
  患者希望 / 完遂 / その他)を残す。ヘッダの `regimen` 拡張に持ち、治療歴(C-4)に出す。
...
- **C-4 治療歴(治療サマリー)**: 患者の全適用(完了・中止を含む)を「レジメン / 期間 / クール数 / 状態 / 中止理由」で
  一覧にする。化学療法タブのビューを「カレンダー / レジメン詳細 / 治療歴」の 3 つにする。レジメンの変更(中止 → 別の
  レジメンを適用)はこの並びで読めれば足りる。
...
- **E-1 薬剤部のレジメン監査**: 承認画面には「化学療法」で出るが、注射ワークリストでは識別できず、中身は力価の
  コメントだけ。化学療法のバッジ、ヘッダの体格からの mg/m² の逆算、前クールとの差分、B-1 の率・理由を見せる。
...
3. **C-1 + C-2 + C-4**: 完了・休止・中止理由・治療歴。状態機械を閉じる。
