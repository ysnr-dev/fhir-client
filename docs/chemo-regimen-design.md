...
#### A. 適用時・次クール登録時の安全性チェック(いまは何も出ない)

- **A-1 投与前チェック(検査値)**: `master_regimen_lab_criteria`(分析物 5 桁 + 下限 / 上限)と直近の検査結果を
  突き合わせ、適用フォーム・次クール登録フォームの上に「適応基準」の表(項目 / 基準 / 直近値 / 採取日 / 判定)を出す。
  基準外・未検は警告に留め、登録は止めない(医師判断)。CCr のような分析物の無い計算値は名称で計算値(B-3)を当てる。
  再利用: `useRenalResults`(Observation lab を新しい順 200 件)と `bodyMeasureHelpers.analyteOf` の分析物突き合わせ
  (腎機能・感染症の読み取りと同じ流儀。上流は分析物 5 桁で検索できないので画面側で前方一致)。
- **A-2 マスタの臨床情報を適用画面へ**: 制吐リスク・適応疾患・中止基準・減量基準・想定される副作用・参考文献を
  折り畳みで表示する。`RegimenDetail` に既に載っているので安い。
- **A-3 体格の変化の警告**: 次クール登録で前回(ヘッダの height / weight / bsa)と今回を並べ、体重が ±10% 以上
  変わっていれば注意を出す(外来化学療法の標準的な運用)。次クールでヘッダの体格を更新するか(いまは初回の値のまま)も
  このとき決める。
...
- **B-2 前クールの投与量を引き継ぐ**: 次クール登録で「前クールと同じ(減量を保つ)/ 体格から出し直す」を選ぶ。
  ...
  率(B-1 の拡張)を読む。
- **B-3 Calvert 式(AUC)**: `planDrugDose` の `case "auc"` に AUC × (GFR + 25) を実装する。GFR は Cockcroft-Gault の
  CCr(未実装。`bodyMeasureHelpers.ts` に足す。年齢・性別・体重・Cr)を既定にし、eGFR(JSN 式、実装済)も選べる。
  GFR の頭打ち(125 mL/min など)は施設ルールなので画面で直せる値にする。
...
1. **A-1 + A-2 + B-3(CCr)**: 適用画面に「安全性の面」を足す。同じフォームに同時に入れるのが効率的で、マスタが既に持つ
   データを使い切る。
2. **B-1 + B-2 + A-3**: 減量と次クールの引き継ぎ。実運用(2 クール目以降)で最初に困る点。
