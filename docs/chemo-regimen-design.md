...
**C-1 / C-2 / C-4 は実装済み(2026-09-06、§8.6)。** 残るのは C-3 / C-5。

- ~~**C-1 クールとレジメンの完了**~~: 実装済み。§8.6。
- ~~**C-2 中止理由**~~: 実装済み。§8.6。
- **C-3 有害事象(CTCAE Grade)の記録**: 患者 × クール × 用語 × Grade × 発現日。上流に AdverseEvent が無いので、
  第一候補は **Observation**(category = 独自の `adverse-event`、code = CTCAE 用語(自由記述。マスタは F)、
  valueInteger = Grade、`regimen-order` 拡張でクールに紐付け)。上流改修なしで済み、患者コンパートメントの検索も効く。
  入口はレジメン詳細のクール行と投与日パネル。次クール登録(B-2)と減量(B-1)の判断材料として前クールの有害事象を並べる。
...
4. **C-3**: 有害事象(Observation)。3 の治療歴と 2 の判断材料に載せる。
