...
- **保存**: 薬剤 1 件の投与量は MedicationRequest の拡張 `regimen-dose`(`drug` = レジメンマスタの薬剤 id、
  ...
  - ［決定］オーダーに載る `doseQuantity` は製剤数(瓶・錠)なので、**それだけでは前クールを再現できない**
    (何 mg だったか、標準量の何%だったかが読めない)。レジメンマスタの薬剤 id を持たせて、クールをまたいでも
    薬剤を取り違えない。RP 番号と RP 内の順で焼く(`stampRegimenDrugs`)。
