...
- **B-1 減量(dose level)**: 適用・次クールで薬剤ごとに「投与率(%)」と減量理由を入れ、力価を 基準 × 体格 × 率 で出す。
  薬剤コメントに「85 mg/m² × 80%」を残し、率・理由は日オーダーの `regimen-order` 拡張に保存する(次クールが読む)。
  マスタの減量レベル表(レベル × 薬剤 × %)は後回しでよい(率の手入力で運用は回る)。
- **B-2 前クールの投与量を引き継ぐ**: 次クール登録で「前クールと同じ(減量を保つ)/ 体格から出し直す」を選ぶ。
  いまは出し直すだけなので、手で直した減量が次クールで消える。前クールの MedicationRequest から力価(コメント)と
  率(B-1 の拡張)を読む。
...
- **B-4 次クール登録の注射区分・処方区分・プロブレムの引き継ぎ**(いまは選び直す)。
...
2. **B-1 + B-2**: 減量と次クールの引き継ぎ。実運用(2 クール目以降)で最初に困る点。
   A-3 の体格の変化が出るようになったので、「変わったから出し直す / 減量を保つ」の選択がここに繋がる。
