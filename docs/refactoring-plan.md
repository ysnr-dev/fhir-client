...
### 第 3 段の実施結果（2026-09-15）

実装済み。コード上の変更はコメントと字下げだけ（`git diff -w` でコメント以外の差分が無いことを確認）。
`tsc -b` 通過、変更した Ruby ファイルは `ruby -c` 通過。計画から変えた点は次のとおり。

- **レジメンの JSDoc**（「適用ヘッダを id で引く」）は孤立ではなく、説明の無い `useRegimenHeaders` のものだった。削除せず移した。
- **削除 hook から切り出した本体関数**は、計画の 2 件（栄養指導・他科依頼）だけでなく 13 件すべてが同じ形だった
  （hook の説明が本体の JSDoc の上に積み重なり、本体は 6 桁字下げのまま）。説明は本体の処理を書いているので、
  hook へ移すのではなく本体の 1 つの JSDoc にまとめ、字下げを直した。
- **backend で残したもの**: `fhir_gateway.rb`（Host ヘッダ指定と FlatParamsEncoder が今必要な理由）、
  `master/base_controller.rb`（`count(:all)` の理由）、`nursing_acts_controller.rb`・`jlac_items_controller.rb`
  （`reorder(nil)` の理由）、`wakeup_controller.rb`・`upstream_warmup.rb`（上流を起こす役をブラウザに渡す理由）。
  どれも現状の制約の説明で、過去の経緯ではなかった。
- **3-2 の言い換え**は計画の一覧に加え、`labTaskHelpers.ts`・`rxTaskHelpers.ts`（「一覧を作る前に登録された」）と
  `masterQueries.ts` の「旧行」（無効になった行）も直した。
- **残したもの**:
  - 「まだ読み込んでいない古いデータ」（`KarteProblemSummary.tsx`・`karteTimeline.ts`）はタイムラインの古い側の意味で、対象外。
  - 看護マスタ migration の「旧行（変更区分 7）」はマスタの用語。
  - `LeaveReturnModal.tsx` の画面文言「(時刻を付ける前の登録)」はコメントではなく利用者向けの文言なので触っていない。
  - `［決定］`・`［提案］`・`［事実］`・`［実装］` の印は約 50 か所で設計メモの書き方として使われている。
    日付の付いた 2 か所（`PatientHeader.tsx`・`locationHelpers.ts`）だけ直し、印そのものは残した。
- **任意の項目**（実施取消の JSDoc の三重複）は第 1 段で関数ごと 1 つにまとめたので解消済み。
...
### R-1〜R-4 の実施結果（2026-09-15）

実装済み。backend の rspec は全件通過（1306 件、0 failures）。開発 DB で、旧実装と新実装のレスポンス
（id と日時を除く）と SQL の発行回数を突き合わせた。39 件すべてレスポンスが一致した。

| # | 対応 | SQL の回数（開発 DB） |
|---|---|---|
| R-1 | パスの複製を、子テーブルを pathway_code で一括ロードし、階層ごとに `insert_all!` で入れる形にした。親の id の付け替えは親の中で一意なキー（病日＋パスステップ、病日 id＋unit_key、ユニット id＋assessment_key）で引き直す | 146〜348 回 → 19 回（サンプル 4 件） |
| R-2 | 保存後に病日・ユニット・観察項目・タスク（レジメンはステップ・薬剤）を 1 回読み、検証と応答の組み立てで共有した | 1〜2 回減るだけ |
| R-3 | `/facility_settings` の show で `FacilitySettings.current` を 1 回だけ読む。キャッシュは入れていない（管理画面の保存が他プロセスへすぐ反映されなくなるため） | 7 回 → 2 回 |
| R-4 | 看護行為の行為一覧の並べ替えとページングを SQL（`ORDER BY COALESCE(MIN(sort_key), 0), level3_code COLLATE "C"` と LIMIT/OFFSET）に移した。総数は GROUP BY の副問い合わせで数える | 2 回 → 3 回（読み込む行は全行為の約 550 件から 1 ページぶんに減る） |

- R-2 は見込み（中）ほど効かなかった。パス承認の保存は 124〜278 回の SQL を投げており、大半は `replace_children` が
  子を 1 行ずつ `create!` し、そのたびに一意性検証の SELECT を投げている分だった（下の R-5 で直した）。
- R-4 の並びは、ソートキーが無い行為を 0 として扱う点と、行為コードをバイト順で比べる点を Ruby の実装に合わせた。

### R-5. 子の置換（`replace_children`）の実施結果（2026-09-16）

実装済み。backend の rspec は全件通過（1306 件、0 failures）。開発 DB で、旧実装と新実装のレスポンスと SQL の
発行回数を突き合わせた。32 件（パス・レジメンの保存 18 件と、重なり・不正値を弾く 14 件）すべて、状態も
エラーの文言も一致した。

| 保存 | SQL の回数（開発 DB） |
|---|---|
| パス（サンプル 4 件の保存・承認） | 123〜278 回 → 16〜17 回 |
| レジメン（サンプル 9 件の保存） | 19〜35 回 → 16〜17 回 |

- 行ごとの検証は `model.new(...).valid?` で Ruby 側で済ませ、INSERT は階層ごとに 1 回（`insert_all!`）にした。
  親の id の付け替えは、親の中で一意な列（病日＋パスステップ、病日 id＋unit_key、ユニット id＋assessment_key、
  ステップの display_order）で引き直す。
- 識別子の重なりは、送られてきた配列の中だけを見ればよい（置換はそのパスの子を全部消して入れ直すため）。
  モデルの一意性検証は `on: :create`（1 行ずつ作る seed 用）に寄せ、置換では配列の中の重なりを見る。
  エラーの文言はモデルの宣言をそのまま使うので、1 行ずつ作るときと同じ。DB の一意インデックスが最後の砦。
- `insert_all` は列の揃った行を要るので、`model.new(...)` の `attributes` から全列ぶんの値を作る
  （パスステップのように既定値のある列も埋まる）。子テーブルに保存時のコールバックは無い。
