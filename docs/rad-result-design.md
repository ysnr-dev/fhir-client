...
［実装］**実施入力をするかは撮影項目マスタが決める**
(`master_rad_items.requires_perform_input`、画面の「実施入力有無」)。「なし」の項目は
「実施」でモーダルを開かず、Task を `completed` にするだけ(実施記録を作らないので
カルテカードにも実施情報は出ない)。オーダーに複数の撮影項目が載っているときは、
1 つでも「あり」があればモーダルを開く。撮影して終わりの項目で、毎回空のモーダルを
閉じさせないための逃げ道。
...
［実装 2026-08-25］**「造影剤を追加」「器材を追加」「診療行為を追加」の検索モーダルは、
データセットの候補から選ぶモードで開く**。データセット明細の初期値ON は最初から明細に
並ぶが、初期値OFF(使ったときだけ足す造影剤・器材)は従来どこにも出ず、レセ電算の全件
検索から名称で引き当てるしかなかった。候補が1件以上あればデータセット側から開き、
そこに無いものだけ「全件検索」に切り替える。仕組みは `DatasetPickList.tsx` に共通化し、
放射線・生理検査・内視鏡・処置の4部門で同じ形。設計の詳細は
`docs/endoscopy-order-design.md` §3.1。
...
### 6.3 即実施(オーダー登録と同時の実施。実装済み 2026-08-16)

［実装］診察室でその場で撮る運用のために、放射線オーダー画面(カルテ右ペイン)の
「選択中」の下に**「即実施」チェック**を置く。入れると「実施入力」ボタンが出て、
放射線検査一覧と同じ実施入力モーダル(`RadPerformInputModal`)が開く。入れた内容は
「選択中」に要約(実施時刻 / 手技・造影剤・器材・線量の件数)で出す。

「登録」で作るリソースは、オーダー(ヘッダ + 明細)・実施記録一式・実施済の Task を
**1 つの transaction** にまとめたもの(`buildRadOrderWithPerformBundle`)。オーダーだけ
登録されて実施が落ちる状態を作らない。ヘッダはこの時点でまだ採番されていないので、
`Procedure.basedOn` と `Task.focus` には同じ Bundle 内の fullUrl(`urn:uuid`)を指す
(上流 `BundleProcessor` が transaction 内で実 id に解決する)。

設計判断:

- **実施記録はオーダーごとに作る**。単独オーダーの項目を混ぜて選ぶとオーダーが
  分かれる(§ `splitRadOrderValues`)ので、実施入力もその単位で入れる。1 回の入力を
  全オーダーに写すと、造影剤・器材・手技料が複数のオーダーに二重に載って算定を
  誤らせる。オーダーが 1 件のときは行が 1 つなので、普段の見え方は変わらない
- **実施入力の有無は一覧の「実施」と同じ判定**(`requires_perform_input`)。
  実施入力をしない項目だけのオーダーは、ボタンを出さず Task の完了だけを積む
- **未入力のまま登録させない**。実施入力が要るオーダーが未入力なら検証エラーにする。
  即実施は実施記録まで作る操作なので、押し忘れを黙って通さない
- **編集(更新)では出さない**。既に登録済みのオーダーの進捗は放射線検査一覧が持ち、
  実施の取消もそちらで行う
