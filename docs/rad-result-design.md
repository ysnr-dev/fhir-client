...
## 5. 上流 fhir-server に必要な追加

［事実］いずれも定義の追加だけで、マイグレーションは不要。ServiceRequest に
`category` を足した 2026-08-13 の変更と同じ手順(検索定義 + `_include`/`_revinclude`
許可リスト + トークン抽出があれば `rake fhir:reindex_tokens`)。
...
- カルテの放射線オーダーカード: 「実施情報表示」を追加(細菌検査の
  「検査結果表示」と同じ置き方)。`RadResultDetailPanel` をモーダルで開く
