...
6. **実施の取消で Procedure が残る**。実施済の行の「取消」は Task を受付済へ
   戻すだけで、実施記録は消していない。取消 → 再実施を通すと Procedure が
   二重に残る。§5 で `Procedure?based-on` が入ったので「このオーダーの実施記録」は
   特定できるようになった。取消時に実施記録も片付けるか
   (`status = entered-in-error` にするか)を決めて実装する。
   ［実装］それまでの間、カルテカードは**進捗が実施済のときだけ実施情報を出す**
   (§6.2)。取消後も残る実施記録を出すと、取り消したはずの検査が実施済に見える。
   再実施でハブが複数になったときは、丸めずに実施ごとに並べる(どちらが生きている
   記録かをこちらで決められないため)。取消時に実施記録を片付ければ、この出し分けは
   要らなくなる。
