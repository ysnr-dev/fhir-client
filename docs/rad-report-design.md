...
- **確認**は緊急異常値と同じく、通知を完了にして `Task.note` に誰がいつを残す。
- **検査結果確認と重ねない**(§6.3)。

### 6.3 緊急の通知と検査結果確認を 1 件にまとめる(2026-09-17)

［事実］最終報告で重要所見ありにすると、同じ依頼医あてに重要所見と検査結果確認の 2 件が届き、
重要所見を確認しても検査結果確認が未確認のまま残った。検体検査の緊急異常値も同じ作りだった。

［実装］緊急の通知(`lab-panic` / `rad-critical-finding`)が保存後も未確認で残るなら、検査結果確認は
作らず、未確認のものは取り下げる(`resultReviewHelpers` の `urgentAwareReviewTaskEntries`)。

- 緊急の通知を**通知一覧で確認**すると、レポートが最終報告(訂正を含む)なら既読の来歴を書き、未確認の
  検査結果確認があれば対応済みにする(`api/notificationActions.ts` の `urgentConfirmationEntries`)。
  暫定報告なら既読は残さない。後で最終報告にしたとき、緊急の通知は確認済みなので検査結果確認が作られる。
- **カルテで結果を確認**すると(`useMarkResultReviewed`)、未確認の緊急の通知も対応済みにする。
- 訂正で緊急の通知が未確認に戻れば検査結果確認は取り下げ、戻らなければ検査結果確認だけが未確認に戻る。
  どちらでも未確認は 1 件。
- ［導出］細菌検査・病理は緊急の通知を持たないので、従来どおり検査結果確認だけが出る。

検証(開発環境): 読影で「最終報告+重要所見の登録で 1 件」「通知一覧の確認で既読の来歴が残る」「同じ要点で
訂正すると検査結果確認だけ」「要点を変えると重要所見だけ(検査結果確認は取り下げ)」「カルテの確認で両方済み」
「暫定+重要所見を確認しても既読は残らず、最終報告にすると検査結果確認が出る」を確認した。検体検査は
組み立て関数の出力で同じ 6 通りを確認した(緊急異常値の確認操作は、開発環境の見本データを残すため画面では行っていない)。
