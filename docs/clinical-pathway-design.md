...
経過に応じて途中で道を変えられるよう、病日の上に**フェーズ**(連続する病日のまとまり)を置き、患者への適用を
フェーズ単位で進める(レジメンのクール単位の登録と同じ考え方)。`path_step`(同じ病日を術前・術後に分ける
ePath のパスステップ)とは別の概念。

```
master_pathway_phases          pathway_code, phase_key(uuid), display_order, name, note   UNIQUE(pathway_code, phase_key)
master_pathway_phase_branches  pathway_code, from_phase_key, to_phase_key(NULL = ここでパスを終了), criteria(選ぶ目安), display_order
master_pathway_events          + phase_key(NOT NULL)。一意性は (pathway_code, phase_key, elapsed_days, path_step)
```

- ［決定］結び付けは id ではなく `phase_key`(uuid、画面が採る)。置換で id が変わっても引き直しが要らず、複製でもそのまま写せ、
  適用後の病日の CarePlan に焼く値にもなる(unit_key と同じ方針)。
- ［決定］開始フェーズは表示順の先頭(列は持たない)。分岐の行を持たないフェーズは最後のフェーズ。分岐の**先頭が標準の経路**。
- ［決定］病日は通し番号。分岐先どうしは同じ病日から始まる(同時には適用されないので、適用後の識別子
  「適用 ID.病日[-ステップ]」は重ならない)。このため病日の一意性をフェーズの中へ緩めた。
- ［決定］フェーズが 1 つのパスは今までと同じ(画面にフェーズの帯を出さず、適用は一括、病日の歯抜けも承認できる)。
  フェーズを送らない payload は、既定のフェーズを 1 つ作って全部の病日を結ぶ(旧クライアント・seed の互換)。
- API は `phases[].branches[]` の入れ子で受け・返す。`phases` も子の 1 種別で、来たときだけ置換し、承認済では送れない(凍結)。
- 検証(下書きでも): 病日が結ぶフェーズが無い / 分岐先が無い・同じフェーズ。
  承認時: 各フェーズに病日がある。フェーズが 2 つ以上なら、フェーズ名がある / 分岐が循環しない / 開始フェーズから全フェーズに
  進める / フェーズの中の病日が連続(-1 の次は 1)/ 分岐先の最初の病日 = 分岐元の最終病日の次。
  予定日数は**標準の経路**(分岐の先頭を辿る)の最終病日以上(合併症の経路が予定日数を超えるのは許す)。
...
- ［サンプル］2026-09-13 に 3 件を足した(どれも下書き。雛形の値は 900001 の同じ種別の値と開発 DB のマスタの行から作った)。
  ...
  - 2026-09-14 に 900002・900004 を画面で承認して適用した(900003 は先に承認・適用済み)。900004 の細菌検査の雛形は採取部位が
    空で、細菌検査のフォームの必須に掛かるので、喀痰は「気管内」、血液培養は「末梢静脈」を入れた。900002 の大腸EMR は
    予約必須の項目で、開発環境に内視鏡の予約枠が無いため、適用時は項目マスタの予約必須を一時的に外した(適用後に戻した)。
...
- **パスタブ**(`components/KartePathwayTab.tsx`、カルテ左ペインの「パス」、`KARTE_TABS` の `pathway`): 適用したパスを
  **日めくり**(1 病日ずつ、`components/PathwayDayView.tsx`)と、紙のパスシートと同じ**オーバービュー**(病日 × OAT ユニットのシート)で見る。
  ...
  - 見出し帯: パス名 / 入院日 / 今日が何病日目か(パスの期間外なら病日数)/ 状態(進行中・終了・中止と終了日)/
    進み具合の件数(未評価・バリアンス・未実施。オーバービューのときだけ)。
  - ［決定］**操作は見出しの行(「クリニカルパス」の行)の「全画面」の右にアイコンで並べる**。よく使うので目立たせるが、見出し帯は
    折り返すので帯には置かない(帯の行を増やさない決まり)。並びは「次のフェーズを適用」(候補があるときだけ、強調色)§7.11・
    「予定外を追加」§7.6・「日程の変更」§7.8・「終了・中止」§7.5(終わったパスは「終了の記録」)。名前はすぐ出るツールチップで示す(CSS の `data-tooltip`。ネイティブの title は出るまでが遅い)。
    消す操作(「適用の取り消し」§7.9・「フェーズの取り消し」§7.11)だけ、その右の小さなケバブに畳む。ボタンの高さは表示の切り替えに揃える。
  ...
  - **オーダー詳細からの導線**: 下に「編集」と「実施入力」を添える(`KarteDetailModal` の `actions`)。
    ...
    - 実施入力 → 注射・輸血と、部門の一覧に実施入力がある種別(放射線・生理・内視鏡・処置・手術・リハビリ・栄養指導。2026-09-14 に拡大)。
      ...
      - 手術: 手術一覧は予定日時(occurrenceDateTime)の日で引くので、日程未定の申込は開けない(その旨を出す)。
...
- ［決定］最初の適用は開始フェーズだけを登録する(CarePlan の木 + オーダー)。続きはパスタブの見出しのアイコン
  「次のフェーズを適用」→ 分岐の候補(フェーズ名 + 目安、「パスを終了」)から選ぶ → 右ペインの同じ適用フォームを
  次のフェーズとして開く(`PathwayPhasePanel`、レジメンの `RegimenCyclePanel` と同じ作り)。「パスを終了」は終了・中止の入力へ進む。
- ［決定］フェーズ階層の CarePlan は足さない。**病日の CarePlan に印を焼く**:
  拡張 `pathway-phase`(valueCoding: code = phase_key、display = フェーズ名)。partOf・識別子・根の検索は変わらない。
  分岐を選んだときの記録(目安の写し + コメント)は、フェーズの最初の病日の拡張 `pathway-phase-note` に残し、
  来歴(Provenance)もその病日を対象に付ける。適用の CarePlan は PUT しない。シートのフェーズの段で読める。
- ［決定］「どこまで適用したか」は保存せず、適用済みの病日の印から逆算する(`nextPhasesOf`)。最後に適用したフェーズ =
  最も進んだ病日の印。印の無い適用(フェーズ導入前)は先頭のフェーズとして読む。進行中でない適用には候補を出さない。
- ［決定］次のフェーズの開始日の既定は前のフェーズの最終日の翌日で、変えられる(適用済みの最後の病日より後)。病日の実日付は
  病日の CarePlan の period が正なので、ずらした結果は「日程の変更」(§7.8)と同じ表現になる(`phaseEventDate`)。
- ［決定］オーダーの続きのまとめ(§5.1)はフェーズの中で閉じる(`pathwayOfPhase` で絞った定義を渡す)。境界をまたいで同じ
  task_key が続いても、次のフェーズでは新しいオーダーを出す。前のフェーズから続いている**食事・安静度**(終わりを決めていないか、
  新しい開始より後まで続くもの)は、次のフェーズが同じ種類を出すなら、その直前で終える PUT を同じ transaction に積む
  (`planPhaseOrderClosings`。フォームに「終了する指示」として出す。終わった・取り下げたオーダーは触らない)。
- ［決定］オーダーの印(`pathway-instance`)は同じ適用の uuid で束ねるので、適用の取り消しは全フェーズのオーダーを拾う。
- **フェーズの取り消し**: 選び間違えた分岐を戻す。最後に適用した 2 つ目以降のフェーズだけが対象で、条件と経路は適用の取り消し(§7.9)と
  同じ(`planPathwayCancel` に phaseKey を渡す。記録の有無はそのフェーズの病日だけで見る)。適用の CarePlan と前のフェーズは残す。
  前のフェーズの食事・安静度に入れた終了は戻さない(次のフェーズを適用し直せば、また直前で終える。食事・安静度を出さない
  フェーズへ進み直すときは手で直す)。
- 適用の木は 1 ページ(500 件)を超えうるので、`usePathwayApplicationTree` は total から残りのページも読む。
...
- 上流の `Task/$validate` に、focus = Observation・basedOn = CarePlan・input に valueDate を持つ `pathway-variance` の Task を送り、
  「valid」が返る。
- パス 一郎(900001、主治医 児玉 義憲)の病日 1「★身体的準備ができている」を 達成 → 未達成で記録 → Task 1 件(requested、
  priority urgent、owner / requester 児玉 義憲、focus Observation、basedOn OAT ユニット)。通知一覧に「注意 / パスのバリアンス /
  腹腔鏡下胆嚢摘出術(4泊5日) 病日 1 入院日 身体的準備ができている」、ベルが 2 → 3 件。種別の絞り込みに「パスのバリアンス(1)」。
- 「カルテ」→ パスタブが日めくり「病日 1 入院日 2026-09-13」で開く。「確認」→ completed(note「バリアンスを確認しました。」)。
- Task の履歴: 版 1 requested(作成)→ 版 2 completed(確認)→ 未達成のまま記録し直し・達成に直す では版が増えない →
  未達成に戻すと版 3 requested(同じ Task を書き換え)→ 達成に直すと版 4 cancelled。
- 検証の途中で、ブラウザのレジストリに新しい種別が入らず件数が変わらなかった。Vite が古い変換結果を配信していたため
  (コンテナ内で touch して解消。コードの問題ではない)。
...
- パス 次郎(900003、受付前): 胸部X線・心電図・創処置は部門の実施入力(手技料・データセットの初期値入り)から「実施を登録」、
  手術は入室・退室の時刻を入れて登録。どれも詳細を閉じるとセルが「☑ 実施済」になり、今日の病日の未実施が 13 → 11。
- 入院 士郎(900004): 胸部X線の項目マスタの「実施入力あり」を一時的に外すと「実施入力の項目はありません。実施済にしますか？」が出て、
  「実施済にする」で ☑ 実施済(未実施 13 → 12)。確かめた後にマスタは元に戻した。
- 山田 太郎(900002): 大腸EMR(内視鏡)を実施入力から登録 → ☑ 実施済。予定外で足したリハビリのタスク(依頼済)を実施入力から登録 →
  ☑ 実施中(Task は受付済)。日程の変更・適用の取り消しに「実施済みのオーダーがあります(リハビリ(検証))」「…(内視鏡的大腸ポリープ切除術)」。
- パス 一郎(900001): 4 日目の栄養指導(依頼済)を実施日 09-16 で登録 → 4 日目のセルだけ「☑ 実施中」(Task は受付済)。
- 見つけて直したもの: 手術は予定日時を `occurrenceDateTime` に持つのに `occurrencePeriod` を見ていて「実施日が決まっていない」になった。
  登録直後も開いたままの詳細に「実施入力」が残り、押すと実施が二重に入るので、実施入力を閉じたときに木を読み直すようにした。
- 項目マスタ(`use…ItemsByCodes`)は staleTime が無期限なので、同じ画面でマスタを書き換えても読み直すまで前の判定のまま。
...
- 名前だけの形(2026-09-14): 一覧の「パス」列にパス名だけ(長い名前は 196px で省略)、パスの無い患者は「-」。パス 次郎の名前から
  カルテのパスタブがオーバービューで開き、患者帯に「パス 鼠径ヘルニア根治術(2泊3日)」。
...
### 9.27 検証したこと(フェーズ単位の適用と分岐、2026-09-19、児玉 義憲)

- backend: `spec/requests/master/pathways_spec.rb` 32 件(フェーズと分岐 9 件を追加)。既存のパスは migration で既定のフェーズ 1 つに移行。
- 定義画面: 900005(市中肺炎・3 日目に効果判定で分岐)でフェーズの帯 3 つ、見出しに分岐の要約、概要表にフェーズの段。承認の検証を通る。
- 適用: 900005 は初期治療(病日 1〜3)だけが展開・登録され、シートにフェーズの段。メニューに「次のフェーズを適用」。
  900001 のような 1 フェーズのパスと、フェーズ導入前の適用(テスト太郎)は今まで通りで、メニューに出ない。
- 次のフェーズ: 候補 2 つ + 目安。「抗菌薬変更・点滴継続」を開始日 1 日遅れ(09-21)+ コメントで適用 → 病日 4〜10 が 09-21〜09-27、
  フェーズの段に目安とコメント。前のフェーズの常食は 09-21 朝で終わり、昼から新しい食事。
- フェーズの取り消し → 初期治療だけに戻る。標準の「内服切替・退院準備」を適用 → 前のフェーズの安静度が 09-19 で終わる。
  最後のフェーズではメニューに「次のフェーズを適用」が出ない。
