...
- **パスタブ**(`components/KartePathwayTab.tsx`、カルテ左ペインの「パス」、`KARTE_TABS` の `pathway`): 適用したパスを
  ...
  - **セルを押したときに開くものは行の種類で決まる**(2026-09-13 に変更)。
    - アウトカムの行 → 右ペインの評価入力(その病日 × OAT ユニット)。記録するとシートに達成 / 未達成(バリアンスは赤)、
      実績値、☑ が出る(§7.4)。
    - タスクの行 → そのタスクに結んだオーダーの画面。看護指示なら指示簿と同じ実施入力(`NursingPerformModal`、
      過去の病日から開いたときはその日の時刻で記録を始める)、他の種別ならそのオーダーの「〜編集」の右ペイン。
      オーダーを持たないタスクは右ペインの「クリニカルパス(タスク)」(`PathwayTaskPanel`)で実施 / 未実施を 1 件だけ記録する
  ...
  - ［決定］**全画面のときは同じ内容をモーダルで開く**。全画面はシートをビューポートいっぱいに広げるので、右ペインが
    後ろに隠れて見えない。中身は右ペインと同じ `PathwayEvaluatePanel` / `PathwayTaskPanel` をそのまま置く(2 つ目の画面を
    作らない)。Escape は重なりの外側から閉じる(モーダル → 全画面)。記録すればモーダルだけ閉じ、全画面のまま次のセルへ進める。
    オーダーの「〜編集」は右ペインにしか無いので、全画面でオーダー付きのタスクを押したときは全画面を抜けてから開く。
...
### 9.9 検証したこと(セルの振り分け、2026-09-13、テスト太郎・テスト次郎)

- テスト太郎(雛形なしの適用): オーダーを持たないタスクのセル → 右ペイン「クリニカルパス(タスク)」。「実施済にする」で
  セルが ☑ に変わりペインが閉じ、開き直すと「実施済 2026-09-13 00:13」、「未実施に戻す」で ☐ に戻る。観察項目のセルは押せない。
- テスト次郎(雛形つきの適用を入院予定 2026-09-20 に当てて確認後に削除): 看護指示のタスク「臍処置・除毛」→ 実施記録の
  モーダルにその指示だけが並ぶ。検体検査 → 「検体検査編集」、手術 → 「手術編集」、食事 → 「食事編集」、放射線 →
  「放射線検査編集」が右ペインに開く。アウトカムのセル → 「クリニカルパス(評価)」。全画面ではオーダー無しのタスクがモーダル、
  Escape で閉じても全画面のまま。オーダー付きのタスクは全画面を抜けて右ペインが開く。
