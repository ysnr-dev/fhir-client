...
- ［決定］予定外のタスクに**オーダー雛形は持たせない**。必要なオーダーは通常どおり右ペインから出す(パスの印は付かない)。
...
- 予定外のアウトカムに付けたタスクにはオーダー雛形を持たせられない(パスの印の付いたオーダーにはならない)。
  予定外でもオーダーから出したいなら、適用パネルと同じ器(`ORDER_SET_TYPES`)をこのモーダルにも積むことになる。
