...
- **パスタブ**(`components/KartePathwayTab.tsx`、カルテ左ペインの「パス」、`KARTE_TABS` の `pathway`): 適用したパスを
  ...
  - タスクのセルの ☑: チェックリストはタスクの Procedure の状態。［決定］**看護指示を結んだタスクは、その日の実施記録があれば ☑**
    (`useNursingPerformsOf`)。続く病日にまたがる 1 件の指示を日ごとに記録するため。
...
- 続き(§3.1)にする前に作ったパス・適用は、同じ名前のアウトカムでも識別子が日ごとに違うので、概要表・シートで別の行になる。
  定義は概要表のセルで続きを足し直す(元の日のアウトカムは外す)。適用済みのものは当て直さない限りそのまま。
...
- 看護指示以外のタスク(チェックリスト)の ☑ はタスク 1 件の状態なので、同じチェックリストを続く病日に置いても日ごとに記録する。
  評価パネルのタスクのチェックは、看護指示を結んだものでも Procedure の状態のまま(実施記録は見ない)。
