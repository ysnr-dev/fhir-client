...
- 行の操作は「日程を確定」(`SurgeryScheduleModal`)。予定手術日・入室予定時刻・
  ...
  確定後の日程変更は編集フォーム(カルテカード、または手術室カレンダーのカードのケバブ)から
  行うので、モーダルは「未定 → 確定」の一方向だけ。
...
3. **手術室カレンダー**: **実装済み**(2026-08-28、`docs/surgery-calendar-design.md`)。
   手術一覧の 3 つ目のタブに日ビュー(手術室 × 時刻)と週ビュー(曜日 × 手術室)、
   ブロックスケジュール(曜日ごとの科割り当て)のマスタ、登録前のダブルブッキング
   確認を入れた。予約枠(Slot)を使わない判断は変えていない(部屋の空きの真実の源は
   手術オーダーの `occurrenceDateTime` + 所要時間のまま。上流改修も無し)。
   **ダブルブッキングは「重なりを承知で登録」の確認を挟んで通す**形にした。
   §5.5 の「止めると運用が回らない」と、ここでの「そもそも作れなくする」を
   両立させるため。§5.5 の表示はカレンダーが入っても残している(日程を入れている
   最中にカレンダーは見えないため)。
   残っているのは多資源(執刀医・麻酔科医)の同時空き判定と月ビューで、
   `surgery-calendar-design.md` §6 に送った
